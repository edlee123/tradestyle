...
if( len(grep('INDICATORS', txt, ignore.case = T)) == 0 ) {
	...
	return(all.data)
		...
		data = extract_table_from_webpage(txt, 'INDICATORS', has.header = T)
		...
		temp = extract_table_from_webpage(temp, 'All amounts', has.header = T)
...
{			
	index = get_fund_data_index(label, fund)		
