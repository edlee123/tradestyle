...
)
{
	...
	constraints = add_variables(2*nt + 1, constraints, lb = c(rep(0,2*nt),-Inf))
		# Aw < b => Aw1 - bt < 0
		...
		if( len(index) > 0 ) {			
			...
			constraints = add_constraints(a[, index], rep(0, len(index)), '<=', constraints)			
		}
	...
	# [ SUM <over j> 1/T * d.j ] = 1	
	constraints = add_constraints(c( rep(0,n), rep(0,nt), (1/nt) * rep(1,nt), 0), 1, '=', constraints)				
			
	# objective : Omega
	# [ SUM <over j> 1/T * u.j ]
	constraints = add_constraints(c(rep(0, n), (1/nt) * rep(1, nt), rep(0, nt), 0), value, type[1], constraints)	
			
	return( constraints )	
}

#' @export 
portfolio_omega <- function
(
	weight,		# weight
	ia			# input assumptions
)	
{	
	...
	return( apply(portfolio_returns, 1, function(x) mean(pmax(x - omega,0)) / mean(pmax(omega - x,0)) ) )
}	
...
)
{
	...
	if(type == 'mixed'	|| type == 'lp') {
		sol = optimize_portfolio(ia, constraints, add_constraint_omega, portfolio_omega, 'max', T)
	...
	if((type == 'mixed' && (sol$status !=0 || any( u*d != 0 ) )) || type == 'nlp') {
		...
		fn <- function(x){
			...
			mean(pmax(portfolio_returns - omega,0)) / mean(pmax(omega - portfolio_returns,0))
		}
		
		x = optimize_portfolio_nlp(ia, constraints, fn, direction = 'max')
		#portfolio_omega(t(x),ia)
	}
...
)
{
	...
	for(i in 1:nportfolios ) {
		...
		out$weight[i, ] = max_omega_portfolio(ia, constraints)
...
{	
	...
	threshhold = quantile(portfolio_returns, probs = c(0.05, 0.95))
	threshhold = seq(threshhold[1], threshhold[2], length.out = 100)

	par(mar = c(4,4,2,1), cex = 0.8)
	for(i in 1:nrow(weight)) {	
		data = sapply(threshhold, function(L) omegafn(portfolio_returns[i, ], L))
	...
	grid()
	plota_legend(rownames(weight) ,1:nrow(weight), x = 'bottomleft')
}
