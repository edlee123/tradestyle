...
#
#' @export 
bl_compute_risk_aversion <- function(bench, risk.free = 0)
{
...
#' @export 
bl_compute_optimal <- function(risk.aversion, mu, cov)
{
...
aa_black_litterman_examples <- function()
{
	# He & Litterman: The intuition Behind Black- Litterman Model Portfolios. 
...
EqRiskPrem = bl_compute_eqret(RiskAversion, Covmat, MktWeight)
...
post = bl_compute_posterior(EqRiskPrem, Covmat, P, Q, tau)		
	ExpRet = post$expected.return
	SigmaP = post$cov
	
	OptimalWeights = (1/RiskAversion) * solve(SigmaP) %*% ExpRet
OptimalWeights = bl_compute_optimal(RiskAversion, ExpRet, SigmaP)	
...
post = bl_compute_posterior(mu, varCov, pick, c(0.0525, 0.0025, 0.02), tau = 0.025)		    
