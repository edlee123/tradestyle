...
#' @export
variable_number_arguments <- function(...) {
	out = lst(...)
...
#' @rdname EnvironmentFunctions
###############################################################################
env_del = function(names, env) {
	warning('env_del is depreciated as of Apr 25, 2016 please use env_rm function instead')
	env_rm(names, env)
}

#' @export
env_rm = function(names, env) {
...
#' @export 
###############################################################################
check_args = function(default.args, args=NULL) {
...
#' @export
###############################################################################
test_equality = function(..., eps = 1e-10, na.rm=T, type=c('first', 'all')) {
	values = list(...)
	n = len(values) 
	if(n == 0) return
...
map2monthly <- function(equity) 
{
  ...
  if(compute_annual_factor(equity) >= 12) return(equity)
  
  dates = index(equity)
  equity = coredata(equity)

  temp = as.Date(c('', 10000*date_year(dates) + 100*date_month(dates) + 1), '%Y%m%d')[-1]
  new.dates = seq(temp[1], last(temp), by = 'month')    
  
  map = match( 100*date_year(dates) + date_month(dates), 100*date_year(new.dates) + date_month(new.dates) ) 
  temp = rep(NA, len(new.dates))
  temp[map] = equity
  
  #range(a - temp)
  
  return( make_xts( ifna_prev(temp), new.dates) )
}
...
#' @examples
#' \dontrun{ 
#' create_monthly_table(monthly.ret)
#' }
#' @export 
###############################################################################
create_monthly_table <- function(monthly.data) 
{
  nperiods = nrow(monthly.data)
  
  years = date_year(index(monthly.data[c(1,nperiods)]))
  ...
  # align months
  index = date_month(index(monthly.data[c(1,nperiods)]))
...
)
{
  if(nc == 1) m
    ...
    if(is.xts(m))
      make_xts(matrix(coredata(m), nr=len(m), nc=nc, byrow=F), index(m))
    else
...
#' @export 
col_name2row = function(x, move.name=T) {
...
#' @export 
row_name2col = function(x, move.name=T) {
...
#' @param details_flag to provide details, \strong{defaults to FALSE}
#'
#' @return new matrix
#' 
#' @examples
...
#' lookup_index(data, which(data > 4))
#' }
#' @export 
# play with following example: update 1 %% 4  
###############################################################################
lookup_index <- function
(
  data,   # matrix
...
###############################################################################
#' @export 
to_nice = function(out,nround=2,sprefix='',eprefix='') {
  if( !is.null(dim(out)) ) {    
...
#' @export 
to_percent = function(x, nround=2) to_nice(100*x,nround,'','%')

#' @export 
to_cash = function(x, nround=2) to_nice(x,nround,'$')
...
#' @examples
#' \dontrun{ 
#' make_xts(1:101,seq(Sys.Date()-100, Sys.Date(), 1))
#' }
#' @export 
###############################################################################
make_xts <- function
...
#' @export 
as_xts_list <- function(data) { for(n in names(data)) colnames(data[[n]])=n; do.call(cbind, data)}
...
###############################################################################
xts2ts = function(x) {  
  annual.factor = compute_annual_factor(x)

  map = c(date_day, date_week, date_month, date_quarter)
  ...
  start = date_year(first.date)
  end = date_year(last.date)
  if( !is.null(date.fn) ) {
...
flip_xts <- function(x)
{
  ...
  out = make_xts(coredata(x)[dates.index,], dates[dates.index])
...
#' @examples
#' \dontrun{ 
#' write_xts(make_xts(1:101,seq(Sys.Date()-100, Sys.Date(), 1)), 'temp.csv')
#' }
#' @export 
###############################################################################
write_xts <- function
...
# data = out[, eval(rest.columns.expr)]
  ...
  out = make_xts(data[dates.index,,drop=F], dates[dates.index])
...
)
{
  ...
  out[,1] = temp[[2]] 
    ...
    out = make_xts(out[dates.index,],  dates[dates.index])
    indexClass(out) = index.class
  return( out )
}

read_xts_test <- function() {
  ...
  test1 <- function() {
    out = read_xts(filename, format = '%Y-%m-%d')
  }
  test2 <- function() {
    out1 = read_xts_old(filename, format = '%Y-%m-%d')
  }
  test3 <- function() {
    out2 = read_xts_yahoo_old(filename, format = '%Y-%m-%d')
  }
...
#'
#' NOTE index_xts is the same name as the index function in the XTS package
#'
#' @param x \code{\link{xts}} object
#'
#' @return dates
...
#' @examples
#' \dontrun{ 
#' index_xts(make_xts(1:101,seq(Sys.Date()-100, Sys.Date(), 1)))
#' }
#' @export 
###############################################################################
# maybe rename to bt.index_xts
index_xts <- function
(
  x     # XTS object
)
...
#' @export 
###############################################################################
find_names <- function(names, data, return.index = T) 
{ 
  names = spl(names)
...
#' @export 
############################################################################### 
# scale_one <- function(x) x / rep_row(as.numeric(x[1,]), nrow(x))  
scale_one <- function
(
  x, 
  overlay = F, 
...
) 
{
  ...
  if( overlay )
    ...
    x / rep_row(apply(x, 2, function(v) v[index[!is.na(v)][1]]), nrow(x))
}
...
#'
#' @param Symbols_vector of symbols
...
#' @param ... other parameters for getSymbols_function
#'
...
#' \dontrun{ 
#' data <- new.env()
#' getSymbols_sit(spl('SPY,IBM'), env = data, auto.assign = T)
#' }
#' @export 
######################################################################x#########
getSymbols_sit <- function
(
  Symbols, 
  env = .GlobalEnv, 
  auto.assign = TRUE, 
...
#' @export 
################################################################################
getSymbols_extra <- function 
(
  Symbols = NULL, 
  env = parent.frame(), 
  getSymbols_fn = getSymbols,
...
{
	Symbols = parse_expr(Symbols)
	if(len(Symbols) < 1) return(Symbols)
  
	map = map_symbols(Symbols)
	...
	# download
	data = new.env()
	if(len(Symbols) > 0) match.fun(getSymbols_fn)(Symbols, env=data, auto.assign = T, ...)
    ...
    if( len(map[[ s ]]) > 1)
      for(i in 2:len(map[[ s ]]))
        if(is.null(data[[ gsub('\\^', '', map[[ s ]][i]) ]]))
          ...
          env[[ s ]] = extend_data(env[[ s ]], data[[ gsub('\\^', '', map[[ s ]][i]) ]], scale=T)
...
getSymbols_extra_test <- function() 
{
  # Syntax to specify tickers:
  # * Basic : RWX
  # * Rename: REIT=RWX
  ...
  plota_matplot(data$prices)
  
  # Use extrenal data
  raw.data <- new.env()
    raw.data$GOLD = bundes_bank_data_gold()
  
  tickers = spl('GLD, GLD.LONG=GLD+GOLD')
  data <- new.env()
    getSymbols_extra(tickers, src = 'yahoo', from = '1980-01-01', env = data, raw.data = raw.data, auto.assign = T)
  bt_start_dates(data)
...
{
  if(len(Symbols) > 0) {
    match.fun(getSymbols_fn)(Symbols, env = env, auto.assign = auto.assign, ...)

    # same logic as in getSymbols_extra
	Symbols = parse_expr(Symbols)
	if(len(Symbols) < 1) return(Symbols) 
	map = map_symbols(Symbols)
		map = sapply(map, first)
	
	data.today = getQuote_yahoo_today(unique(map))
    ...
    bt_append_today(env, data.today)
  }
}


# test for getSymbols_intraday function
getSymbols_intraday_test <- function() { 
	tickers = '
	LQD + VWESX
	DBC + CRB
	VTI +VTSMX # (or SPY)
	...
	getSymbols_extra(tickers, src = 'yahoo', from = '2012-01-01', env = data.intraday,
		raw.data = data.proxy.raw, set.symbolnames = T, auto.assign = T,
		getSymbols_fn = getSymbols_intraday)
	...
	getSymbols_extra(tickers, src = 'yahoo', from = '2012-01-01', env = data,
		raw.data = data.proxy.raw, set.symbolnames = T, auto.assign = T)
...
remove_operators = function(tokens) { 
...
expr_symbols_test = function() {
	...
	expr_symbols(expression(x=2+y))
	
	expr_symbols(expression(x=2+y, a=b))
...
#' @export 
###############################################################################
log_fn <- function(p.start=0, p.end=1) {
  p.start = p.start
    p.end = p.end
...
#' @export 
###############################################################################
log_fn_msg <- function(msg, log = log_fn()) {
  log = log
    msg = msg
...
#'
#' char is 8 bits, so to generate 512 bits random_string 
...
#' @export 
#' @rdname StringFunctions
make_random_string <- function(nbits = 256) { chr( runif(nbits/8, 1, 255) ) }

#' http://ryouready.wordpress.com/2008/12/18/generate-random_string-name/
#' @export 
#' @rdname StringFunctions
random_string <- function(lenght = 12) { join(sample(c(0:9, letters, LETTERS),lenght, replace=TRUE)) }
...
############################################################################### 
ls_f <- function(env=sys.frame(-1))unlist(lapply(ls(env=env),function(x)if(is.function(get(x)))x))

#' @export 
#' @rdname ListEnvFunctions
ls_v <- function(env=sys.frame(-1))unlist(lapply(ls(env=env),function(x)if(!is.function(get(x)))x))
...
#' @export 
###############################################################################
write_file = function(..., file) cat(..., file=file)
...
#' sb=NULL
#' }
#' @rdname string_buffer
#' @export
###############################################################################
string_buffer = function() structure(list(file = rawConnection(raw(0L), open='w')), class = 'StringBuffer')
...
#' @rdname string_buffer
#' @export
add_StringBuffer = function(x,...,sep=',',end.sep='\n') {
...
#' @rdname string_buffer
#' @export
string_StringBuffer = function(x) rawToChar(rawConnectionValue(x$file))
...
#' @rdname string_buffer
#' @export
close_StringBuffer = function(x) {close(x$file); x$file = NULL}


# test string_buffer functionality
string_buffer_test = function() {
	# base example
