...
fundamental_fb_test <- function()
{
	...
	for(i in 1:len(tickers)) {
		if(is.null(data.fund[[tickers[i]]])) {
			cat(tickers[i],'\n')
			data.fund[[tickers[i]]] = fund_data(tickers.temp[i], 80)
		}
	}
	...
	for(i in tickers) {
		...
		# Earnings per Share		
		EPS = 4 * get_fund_data('Diluted EPS from Total Operations', fund, fund.date)
		if(nrow(EPS) > 3)
			EPS = rbind(EPS[1:3], get_fund_data('Diluted EPS from Total Operations', fund, fund.date, is.12m.rolling=T)[-c(1:3)])
...
png(filename = 'plot2.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		    
    plota_matplot(PE, type='b',pch=20, dates='2012::')
dev.off()					
...
fundamental_dcf_test <- function() 
{
	...
	data.fund <- new.env()
	for(i in 1:len(tickers))
		data.fund[[tickers[i]]] = fund_data(tickers.temp[i], 80, 'annual')
	...
	#****************************************************************** 				
	# Free Cash Flows
	FCF = get_fund_data('free cash flow', fund, fund.date)
	
	# Invested Capital
	IC = get_fund_data('invested capital', fund, fund.date)
		
	# Sales
	SALE = get_fund_data('total revenue', fund, fund.date)
...
png(filename = 'plot1.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		    
	...
	plota_lines(dcf.price, type='s', col='red', lwd=2)
	plota_legend('Close,Intrinsic Value', 'blue,red', list(price, dcf.price))	
dev.off()	
