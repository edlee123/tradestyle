...
seasonality_test <- function() 
{
	...
	#****************************************************************** 
png(filename = 'plot.month_year_seasonality.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
...
png(filename = 'plot.time_seasonality.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	layout(1)
	time_seasonality(data, 1 + month.ends[index], 20, ticker)
dev.off()	
}
...
png(filename = 'plot1.time.series.pattern.matching.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')			
	matches = bt_matching_find(Cl(data), main = ticker,	n.query=90, plot=TRUE)	
dev.off()		
		
png(filename = 'plot2.time.series.pattern.matching.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	out = bt_matching_overlay(matches, plot=TRUE)	
dev.off()		
...
#' @export 
#############################################################################
month_year_seasonality <- function
(
...
) 
{
	...
	group_seasonality(data_list, paste(ticker, 'Monthly', join(format(range(index(data)), '%d-%b-%Y'), ' to\n')))

}    

#' @export 
group_seasonality <- function
(
...
) 
{
	...
	plota_legend(c(paste('Avgerage for', first.year, '-', last.year),
...
(
	...
	ticker,	
	patterns = pattern_db()	# database with patterns
) 
{
	#*****************************************************************
	# Find Extrema
	...
	# Plot	
	#****************************************************************** 
		plota_control$col.border = 'gray'
	plota(sample, type='hl',col='gray')	
		plota_lines(mhat, col='magenta', lwd=2)			
		plota_lines(sample, col='blue')				
		
	if(n.index > 0) {
		plota_lines(sample[data.extrema.loc], type='p', col='blue', lwd=3, pch=19)		
		out = find_patterns(obj, patterns = patterns, silent=F, plot=T)  
	}
			
	plota_legend(c(paste(ticker, join(format(range(index(sample)), '%d%b%Y'), ' - ')),
...
) 
{
	...
	if(is.xts(x)) mhat = make_xts(mhat, index(x))
...
) 
{
	...
	if(is.xts(mhat)) {
		dates = index_xts(obj$mhat)
	} else {
		dates = 1:len(data)
	}		
	
	# Semi-transparent orange color
	col = col_add_alpha('orange', alpha=150)
...
#' @export 
#############################################################################
find_all_patterns_window <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	for(t in window.len : (len(history)-1)) {
		...
		obj = find_extrema( sample )	
		
		if(len(obj$data.extrema.loc) > 0) {
			out =  find_patterns(obj, patterns = patterns, silent=F, plot=F)  
	...
	plot_patterns(data[1:t,], window.len, ticker, patterns)	
...
bt_patterns_test <- function() 
{
	...
	# The parameter d controls_for the fact that in practice we do not observe a
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	...
	group_seasonality(data_list, '20 days after Pattern')
dev.off()	
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	...
	time_seasonality(data, all.patterns[index,'t'], 20, name)	
dev.off()
