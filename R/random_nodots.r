...
###############################################################################
randsphere_test <- function()
{
	load_packages('car,scatterplot3d')
...
# prices = asset_paths(S, c(r,r), sigma = cov.matrix, N, periods = seq(0, 1/12, 1/360))
#
# temp = prices
# dim(temp) = c(2,31*N)
#
...
#' @export 
###############################################################################
asset_paths <- function(s0, mu, sigma, 
	nsims = 10000, 
...
###############################################################################
# Test asset_paths function
###############################################################################
asset_paths_test <- function()
{
	#*****************************************************************
	...
	periods = 0:10
	prices = asset_paths(S[1], r, sigma[1], N, periods = periods)
...
dev.off()					
	...
	prices = asset_paths(S, c(r,r), cov.matrix, N, periods = periods)
...
dev.off()					
	...
	N = 1000000	
	prices = asset_paths(S[1], r, sigma[1], N, periods = Time)
	...
	N = 100000
	cov.matrix = sigma%*%t(sigma) * matrix(c(1,rho,rho,1),2,2)
	prices = asset_paths(S, c(r,r), sigma = cov.matrix, N, periods = Time)
	...
	prices = asset_paths(S, c(r,r), sigma = cov.matrix, N, periods = periods)
...
#' @export 
#******************************************************************************
mat_3d = function(mat, n3dim) { array(mat, c(dim(mat), n3dim)) }

# aperm(array(t(matrix(1:12, nr=6)),c(2,3,2)),c(2,1,3))
#' @export 
mat_slice_3d = function(mat, slice.at) { aperm(array(t(mat), c(ncol(mat), nrow(mat) / slice.at, slice.at)),c(2,1,3)) }


# assuming strategy makes all descions at period.ends
# asset_paths_at_period_ends function simulates price paths
# that resemable history at each period.end
#
# please note that to generate prices at period.end
# asset_paths_at_period_ends function examines historical
# prices prior to and including period.end
#' @export 
asset_paths_at_period_ends <- function(
	prices,
	period.ends,
	nsims = 100,
	...
	# if(any(is.na(prices))) stop('asset_paths_at_period_ends cannot handle missing values')
	...
	for(i in 2:len(period.ends.all)) {
        ...
        scenarios[index,include.index,] = mat_slice_3d(mvrnorm(n.index * nsims, mu, cov), nsims)

		#temp = asset_paths(rep(1,n), mu, cov, nsims, periods = 1:n.index) 		
	}
	...
	scenarios = scenarios * mat_3d(1+0*prices, nsims)
