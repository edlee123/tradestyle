...
#adaptive_shrinkage_paper_backtests()


###############################################################################
# Main routine to run all backtests
###############################################################################
adaptive_shrinkage_paper_backtests <- function() 
{
	stats = list()
	for(i in 0:8) {
		temp = bt_run_data_set(i)
...
if(F) {	
	...
	performance_barchart_helper(out, custom.order = !custom.order,
		custom.col=c('MV.S_SA_A'='red', 'best.sharpe'='green'))	
}		
	
	
	
	bt_summary_stats(stats)
} 

adaptive_shrinkage_paper_backtests_leverage <- function() 
{
	# FuturesForex with leverage, i.e. scale all strategies to have
	# leverage of the equal weight strategy
	temp = bt_run_data_set(0)

	vol = as.double(temp$stats['Volatility',])
	leverage = vol[1] / vol
	
	temp1 = bt_run_data_set(0, leverage = leverage, title = paste(temp$title, 'leverage'))
} 
...
#cssa_average_shrinkage(gia$hist.returns)
...
cssa_average_shrinkage <- function(hist) {		 	
	n = ncol(hist)
...
shrink_average_cssa <- function(s=NULL) { s=s; function(x, a) cov_shrink(x, cssa_average_shrinkage, s, 1)$sigma }
...
# Sample Data Sets
###############################################################################
bt_load_data_set <- function(data.set = 1) 
{	
	if(data.set == 0) {
		data <- new.env()
			getSymbols_TB(env = data, auto.assign = T, download = F)		
...
#		align='keep.all', dates='2000:08::')
#		
#	data.sets[['Dow Jones']] = dow_jones_components()	
#	data.sets[['SP500']] = sp500_components()$tickers 
...
data_clean(data, min.ratio=3)	
	...
	bt_prep(data, align = info$align, dates = info$dates)
...
###############################################################################
bt_run_data_set <- function(i = 1, dates = '::',
	leverage = 1, 
	title = ''
) 
{
...
#		periodicity = 'weeks', lookback.len = 252,
		...
		min.risk.fns = list(MV=min_var_portfolio),
		shrinkage.fns = list(
			S=sample_shrinkage,
			SA=sample_anchored_shrinkage,
			
			D=shrink_diag(1),					
			CC=shrink_const_cor(1),
			SI=shrink_single_index(1),
			A=shrink_average_cssa(1),			

			AVG=function(h,a) 1/4*(
				shrink_diag(1)(h,a) + shrink_const_cor(1)(h,a) + 
				shrink_single_index(1)(h,a) + shrink_average_cssa(1)(h,a)
			),
						
			S_SA_A=function(h,a) 1/3*(shrink_average_cssa(1)(h,a)
			+ sample_shrinkage(h,a) + sample_anchored_shrinkage(h,a)),			

			D_S=shrink_diag(),														
			CC_S=shrink_const_cor(),
			SI_S=shrink_single_index(),			
			A_S=shrink_average_cssa(),

			AVG_S=function(h,a) cov_shrink(h, 1/4 *(
				shrink_diag(1)(h,a) + shrink_const_cor(1)(h,a) + 
				shrink_single_index(1)(h,a) + shrink_average_cssa(1)(h,a)
			))$sigma,
			
			S_SA_A_S=function(h,a) cov_shrink(h, 1/3 *(
			shrink_average_cssa(1)(h,a) + sample_shrinkage(h,a) + sample_anchored_shrinkage(h,a)
			))$sigma
	...
	#****************************************************************** 		
	obj = bt_shrinkage_best_sharpe('S,SA,A', 252, obj, data)
			

	models = create_strategies(obj, data, dates = dates, leverage = leverage)$models
	list(title = title, stats = bt_summary_report(models, title, data, obj,
...
# Best Sharpe Idea
###############################################################################
bt_shrinkage_best_sharpe <- function(methods, sharpe.lookback.len, obj, data) 
{	
	...
	for(i in methods) {
		...
		global.data[[i]] = make_stock_xts(temp)
	}
	bt_prep(global.data, align='keep.all')
	
	
	#*****************************************************************
	# Strategy that invests into highest sharpe strategy	
	#****************************************************************** 		
	# compute_sharpe and select best one - if we use 60 here not very good results
	rets = global.data$prices / mlag(global.data$prices) - 1
	
	sharpe = bt_apply_matrix(rets, runMean, sharpe.lookback.len) / bt_apply_matrix(rets, runSD, sharpe.lookback.len)
...
#######################################################################
bt_summary_stats <- function(stats) 
{
	# notes: lower turnover is better, lower vol is better, higher sharpe is better
	names = spl('Portfolio Turnover,Sharpe,Volatility,Composite Diversification Indicator(CDI)')
	custom.order = c(T,F,T,F)
...
pdf(file = 'summary.pdf', width=8.5, height=11)	
	performance_barchart_helper(temp, 'Composite Standardized Score (higher is better),Composite Rank Score (lower is better)', c(T,F), nc.plot=1,
		custom.col=c('MV.S_SA_A'='red', 'best.sharpe'='green'))
dev.off()   	

png(filename = 'plot2.png', width = 600, height = 400, units = 'px', pointsize = 12, bg = 'white')

	performance_barchart_helper(temp, 'Composite Rank Score (lower is better)', c(F), nc.plot=1,
		custom.col=c('MV.S_SA_A'='red', 'best.sharpe'='green'))
