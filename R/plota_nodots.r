...
#'
#' Set the color theme, the defaul theme is plota_theme_green_orange()
#'
...
#' @examples
#' \dontrun{ 
#' plota_theme_blue_red()
#' }
#' @export
#' @rdname PlotaColorTheme
###############################################################################
plota_theme <- function
(
	col.border = 'black',
	col.up = 'green',
	col.dn = 'red',
...
) 
{
	...
	if(!is.na(alpha)) col = col_add_alpha(col, alpha)
	
	plota_control$col.border = col[1]
	plota_control$col.up = col[2]
	plota_control$col.dn = col[3]
	plota_control$col.x.highlight = col[4]
	plota_control$col.y.highlight = col[5]
}


#' @export 
#' @rdname PlotaColorTheme
plota_theme_blue_red <- function(alpha=NA) 
{
	plota_theme(
		col.border = 'black',
		col.up = 'blue',
...
#' @export 
#' @rdname PlotaColorTheme
plota_theme_green_orange <- function(alpha=NA) 
{
	plota_theme(
...
#' @export 
#' @rdname PlotaColorTheme
plota_theme_gray_orange <- function(alpha=NA) 
{
	plota_theme(
		col.border = '#444444',
		col.up = '#BEBEBE',
...
plota_control = new.env()
	plota_control$col.border = 'black'
	plota_control$col.up = 'green'
	plota_control$col.dn = 'red'
	plota_control$col.x.highlight = 'orange'
	plota_control$col.y.highlight = 'orange'
	plota_control$xaxis.ticks = c()

# set default theme	
plota_theme_green_orange();
...
)
{
	...
	plot( temp.x, y1, xlab = xlab, ylab = ylab, main = main,
		...
		plota_control$xaxis.ticks = axis.POSIXct(1, temp.x,labels = plotX, tick = plotX)
		
				
	# highlight logic
	if( !is.null(x.highlight) ) plota_x_highlight(y, x.highlight); 	
	if( !is.null(y.highlight) ) plota_y_highlight(y, y.highlight); 	
	...
	# plot data
	switch(type,
		'candle' = plota_candle(y, ...),
		'hl' = plota_hl(y, ...),
		'ohlc' = plota_ohlc(y, ...),
		'volume' = plota_volume(y, ...),
...
#'   plota_ohlc(y, col = 'red')		
#' plota_legend('SPY(rhs),IBM(lhs)', 'blue,red', list(y0,y))
#' }
#' @export 
###############################################################################
plota2Y <- function(
...
plota_grid <- function() 
{
	...
	abline( v = plota_control$xaxis.ticks, col = 'lightgray', lty = 'dotted')
}
...
#' @param yformat format Y values function, \strong{defaults to \code{\link{plota_format}}}
...
(
	...
	merge = F, 				# merge
	bty = 'n',				# box
	yformat = plota_format,	# format values
...
###############################################################################
# plota_dx - determine data spacing along X axis
###############################################################################
plota_dx <- function
(
	y	# xts object to plot
...
#' @param col highlight color, \strong{defaults to plota_control$col.x.highlight}
#'
#' @return nothing
#'
#' @export 
###############################################################################
plota_x_highlight <- function
(
	y,						# xts object to plot
	highlight,				# segments to highlight along X axis
	col = plota_control$col.x.highlight
)
{
	if(len(col)==1) {
		plota_x_highlight_helper(y, highlight, col = col)		
	} else { # do for each color
		for( icol in unique(col[highlight]) ) {
			plota_x_highlight_helper(y, iif(col == icol, highlight, FALSE), col = icol)					
		}
	}
}


plota_x_highlight_helper <- function
(
	y,						# xts object to plot
	highlight,				# segments to highlight along X axis
	col = plota_control$col.x.highlight
...
#' @param col highlight color, \strong{defaults to plota_control$col.y.highlight}
#'
#' @return nothing
#'
#' @examples
...
#'   plota_legend('SPY', 'grey70', data.spy)
#' plota(rsi, type = 'l')
#' 
#' col = col_add_alpha(spl('green,red'),80)
#' plota_y_highlight(col=col[1], highlight=c(50,100))	
#' plota_y_highlight(col=col[2], highlight=c(0,50))	
#' 	
#' abline(h = 50, col = 'gray20')
#' 
#'   col = iif(last(rsi)>50,'black','red')
#' plota_legend('RSI(20)', col, rsi, text.col=col)
#' }
#' @export 
###############################################################################
plota_y_highlight <- function
(
	y,					# xts object to plot
	highlight,			# segments to highlight along Y axis
	col = plota_control$col.y.highlight
)
{
	# see par documentation
	temp.y = par('usr')[3:4]
...
###############################################################################
plota_candle_col <- function(	y ) { 
	return( iif( Cl(y)>Op(y), plota_control$col.up, plota_control$col.dn) )
}
plota_volume_col <- function( y ) { 
	return( iif( Cl(y)>mlag(Cl(y)), plota_control$col.up, plota_control$col.dn) )
}

###############################################################################
# plota_candle - plot candles
#  plota_candle will try to plot candles if dx is sufficient
#  otherwise ohlc or bars 
###############################################################################
plota_candle <- function
(
	y,					# xts object to plot
	col = plota_candle_col(y)
)
{
	dx = plota_dx(y)
	...
	if( dxi0 < 1 ) {
		plota_hl_lwd(y, col = col, lwd = 1)
	} else if ( dxi0 < 1.75 ) {
		plota_ohlc_lwd(y, col = col, lwd = 1)
	} else {
		temp.x = attr(y, 'index')
		
		rect(temp.x - dx/10, Lo(y), temp.x + dx/10, Hi(y), 
			col = plota_control$col.border, border = plota_control$col.border)
		rect(temp.x - dx/2, Op(y), temp.x + dx/2, Cl(y), 
			col = col, border = plota_control$col.border)	
	} 
}

###############################################################################
# plota_ohlc - plot ohlc
#  plota_ohlc will try to plot ohlc if dx is sufficient
#  otherwise ohlc or bars 
###############################################################################
plota_ohlc <- function
(
	y,					# xts object to plot
	col = plota_control$col.border
)
{
	dx = plota_dx(y)
	# convert dx to line width
...
(
	y,					# xts object to plot
	col = plota_volume_col(y),
	border = plota_control$col.border
)
{
	dx = plota_dx(y)
	# convert dx to line width	
...
###############################################################################
# plota_ohlc_lwd - plot ohlc using line width
###############################################################################
plota_ohlc_lwd <- function
(
	y,					# xts object to plot
	lwd=1,				# line width
...
###############################################################################
# plota_hl_lwd - plot hl using line width
###############################################################################
plota_hl_lwd <- function
(
	y,					# xts object to plot
	lwd=1,				# line width
	...					# other parameters to segments
...
###############################################################################
# plota_volume - plot volume
#' @export
###############################################################################
plota_volume <- function
(
	y,							# xts object to plot
	col = plota_volume_col(y),	# color
	border = plota_control$col.border
)
{
	dx = plota_dx(y)
...
###############################################################################
# plota_scale_volume - scale volume, (c) quanmod package
#' @export
###############################################################################
plota_scale_volume <- function(y) 
{
	Volumes = Vo(y)
...
###############################################################################
# plota_test - test for plota functions
#' @export
###############################################################################
plota_test <- function() {
	...
	# simple example + rsi + legend with last value		
		...
		plota(y, type = 'candle', plotX = F)
			plota_legend('SPY', 'blue', y)
		y = plota_scale_volume(y)
		plota(y, type = 'volume', plotX = F)
			plota_legend('Volume', 'blue', Vo(y))
		rsi = RSI(Cl(y),2)
		plota(rsi, type = 'l', y.highlight = c(c(Inf,80),c(20,-Inf)))
			abline(h = 20, col = 'red')
			abline(h = 80, col = 'red')
			plota_legend('RSI(2)', 'black', rsi)
	...
	# daily & monthly  on the same plot
		...
		plota(y, type = 'candle')
			...
			plota_ohlc(y1, col = 'pink')
			plota_candle(y)
			plota_legend('Daily,Monthly', 'red,pink')
	...
	# daily / weekly / monthly
		...
		layout(c(1,2,3))	
		plota(y, type = 'candle', plotX = F)
			plota_legend('Daily', 'blue', y)
			
		plota(y, ylim = range(OHLC(y)), plotX = F)
			y1 = to.weekly(y)
				index(y1) = as.Date(index(y1))
			plota_candle(y1)			
			plota_legend('Weekly', 'blue', y1)		
			
		plota(y, ylim = range(OHLC(y)))
			y1 = to.monthly(y)
				index(y1) = as.Date(index(y1))
			plota_candle(y1)	
			plota_legend('Monthly', 'blue', y1)	
...
###############################################################################
# plota_colors - generate distinct colors, todo switch to ColorBrewer
# col = rainbow(N, start=0, end=.9)
#' @export
###############################################################################
plota_colors <- function(N) {
	...
	#pie(rep(1,length(1:14)), col=plota_colors(14))
	col
}

#' @export
plota_colors_experimental = function(N) {
	if( is.list(N) ) N = len(N)
...
#' @param xlab X label, \strong{defaults to ''}, for more info see \code{\link{plot}}
#' @param col colors, \strong{defaults to \code{\link{plota_colors(ncol(y))}}}
...
#' plota_stacked(index_xts(weight), weight)	
#' }
#' @export 
###############################################################################
plota_stacked <- function
(
	x,				# x data
	y, 				# matrix with y data : len(x) = nrow(y)
	xlab='',		# x axis label	
	col = plota_colors(ncol(y)), 	# colors
...
)
{
	...
	if( class(x)[1] != 'Date' & class(x)[1] != 'POSIXct') {
		...
		plota(make_xts(y[,1], x), ylim = ylim, cex = par('cex'), LeftMargin = 4, ...)
		axis(2, las = 1) 
    ...
    # legend
    if(flip.legend)
    	plota_legend(rev(colnames(y)), rev(col), cex = par('cex'))    
    else
    	plota_legend(colnames(y), col, cex = par('cex'))    
}
...
)
{
	...
	if( is.list(y) ) {
		...
		if( n > 1 ) {
			for( i in 2:n ) plota_lines(y[[i]], col = i, type = type, ...)
		}

		plota_legend(names(y), paste(1:n), y)	
		...
		plota_legend(names(y), paste(1:n), as.list(y))	
	}	
}
...
#' @export 
############################################################################### 
plota_add_copyright <- function(copyright = 'Systematic Investor') {		
...
#'
#' @param col highlight color for recession periods, \strong{defaults to \code{\link{col_add_alpha}}('gray', 50) - alpha = 50 so it's relatively transparent }
...
#' plota(SPY, type='l')
#' plota_recession()
#' plota_legend('SPY','black',SPY)
#' }
#' @author Judson Bishop
#' @export 
###############################################################################
plota_recession <- function
(
	col = col_add_alpha('gray', 50),	# Set alpha = 50 so it's relatively transparent
	highlight = NULL
)
{
	...
	plota_x_highlight(highlight, highlight != 0, col)
}
