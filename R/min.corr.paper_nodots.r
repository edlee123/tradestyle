...
#close(con)
###############################################################################
min_corr_paper_examples <- function() 
{
#*****************************************************************
...
#****************************************************************** 
	...
	#****************************************************************** 
	data <- new.env()
	getSymbols_TB(env = data, auto.assign = T, download = T)
	...
	bt_prep(data, align='keep.all', dates='1980::')
	...
	bt_prep(data, align='keep.all', dates='2003:10::')	
	...
	# plot
	#plota(make_xts(count(t(data$prices)),index(data$prices)),type='l')
...
#****************************************************************** 
	...
	for(name in names) {
		...
		obj = portfolio_allocation_helper(data$prices, periodicity, lookback.len = lookback.len, prefix = prefix,
			min.risk.fns = 'min_corr_portfolio,min_corr2_portfolio,max_div_portfolio,min_var_portfolio_risk_parity_portfolio(),equal_weight_portfolio',
	...
	for(name in names) {
		...
		custom_input_report_helper(paste('report.', name, sep=''), data)
		
		# create summary of strategies report
		load(file=paste(name, lookback.len, periodicity, '.bt', '.Rdata', sep=''))
		custom_report_helper(paste('report.', name, lookback.len, periodicity, sep=''), 
			create_strategies(obj, data))	
	}
	...
	for(name in names) {
		...
		custom_report_helper(paste('report.leverage.', name, lookback.len, periodicity, sep=''), 
			create_strategies(obj, data, leverage))	
	}
...
# Create summary of inputs report
#*****************************************************************
custom_input_report_helper <- function(filename, data) {
	...
	for(i in data$symbolnames) {
		data$weight[] = NA
			data$weight[,i] = 1	
		asset.models[[i]] = bt_run(data, silent=T)
	}		
	asset.summary = plotbt_strategy_sidebyside(asset.models, return.table=T)

		
	# plot correlations
	ret.log = bt_apply_matrix(data$prices, ROC, type='continuous')
	temp = cor(ret.log, use='complete.obs', method='pearson')
			temp[] = plota_format(100 * temp, 0, '', '%')
	plot_table(temp, smain='Correlation', highlight = TRUE, colorbar = TRUE)	
...
# Create summary of strategies report
#*****************************************************************
custom_report_helper <- function(filename, obj) {
	filename.pdf = paste(filename, '.pdf', sep='')
	filename.csv = paste(filename, '.csv', sep='')
	...
	out = plotbt_strategy_sidebyside(models, perfromance.fn = 'custom_returns_kpi', return.table=T)

	
	
	# Plot time series of components of Composite Diversification Indicator
	cdi = custom_composite_diversification_indicator(obj)	
	...
	performance_barchart_helper(out, 'Sharpe,Cagr,RC Gini,RC Herfindahl,Volatility,Portfolio Turnover,Composite Diversification Indicator(CDI)', c(T,T,F,F,F,F,T))

		
	# summary allocation statistics for each model	
	custom_summary_positions(obj$weights)
	
	
	# monhtly returns for each model	
	custom_period_chart(models)
...
# * 1 - Gini(portfolio_risk_contribution weights) and  
# * Minimum Average Correlation (from max.div) / Average Portfolio Correlation (w * Correlation Matrix * w) 			
#' @export 
#*****************************************************************
custom_composite_diversification_indicator <- function
(
...
) 
{
	cdi = 0.5 * obj$risk.gini + 0.5 * obj$degree.diversification
		if(avg.flag) cdi = bt_apply_matrix(cdi, EMA, avg.len)
	...
	if(plot.main) {	
		...
		out = obj$degree.diversification
			if(avg.flag) out = bt_apply_matrix(out, EMA, avg.len)
		plota_matplot(out, cex.main = 1, 
	...
	if(plot_table) {	
		...
		highlight = apply(temp,2, function(x) plot_table_helper_color(t(x)) )
		
		# plot temp with colorbar
		layout(1)
		plot_table(temp, smain = 'CDI Rank\nAlgo vs %D',highlight = highlight, colorbar = TRUE)
	}	
...
#*****************************************************************
custom_summary_positions <- function(weights) {
	...
	for(w in names(weights)) {
		...
		temp[] = plota_format(temp, 0, '', '')
		plot_table(temp, smain=w)
	}
}	
...
#*****************************************************************
custom_profit_chart <- function(data, main, cols) {
...
# Monhtly returns for each model	
#*****************************************************************
custom_period_chart <- function(models) {
	for(imodel in 1:len(models)) {
		...
		ret.by.month = create_monthly_table(ret)	
		...
		# plots
		temp = plotbt_monthly_table(equity)	
	
		# plot months
		cols = spl('green,red')
		custom_profit_chart(ret.by.month, 'Average Monthly Returns', cols)
		
		# plot years
		ret = 100*as.vector(ret.by.year)
			names(ret) = date_year(index(ret.by.year))
		custom_profit_chart(ret, 'Annual Returns', cols)
	}
}	
...
{	
	...
	# Consentration stats
	out[[ 'W Gini' ]] = mean(portfolio_concentration_gini_coefficient(w), na.rm=T)
	out[[ 'W Herfindahl' ]] = mean(portfolio_concentration_herfindahl_index(w), na.rm=T) 
	
	# Consentration stats on marginal risk contributions
	out[[ 'RC Gini' ]] = mean(portfolio_concentration_gini_coefficient(rc), na.rm=T)
	out[[ 'RC Herfindahl' ]] = mean(portfolio_concentration_herfindahl_index(rc), na.rm=T) 

	out = lapply(out, function(x) if(is.double(x)) round(100*x,1) else x)
	out = c(bt_detail_summary(bt)$System, out)
...
#
# The marginals_for the minimum variance and maximum diversification portfolios 
...
#
###############################################################################
min_corr_paper_numerical_examples <- function() 
{
	#*****************************************************************
