...
###############################################################################
#' Helper function to setup_cluster
#'
#'	load_packages('parallel')
#'	cl = setup_cluster({source('post.fn.r')}, 'spyRets,sma.lens,data,run.backtest',envir=environment())
#'	out = clusterApplyLB(cl, vol.lens, function(j) { run.backtest(j) } )	
#'	stopCluster(cl)	
#'
#' @export 
###############################################################################
setup_cluster <- function(expr = NULL, varlist = NULL, envir = .GlobalEnv, cores = 1000) {
	#*****************************************************************
	# Setup Parallel
	#*****************************************************************			
	load_packages('parallel')	
...
#' @export 
###############################################################################
clusterApplyLB_log <- function (cl = NULL, log = log_fn(), x, fun, ...) {
    argfun <- function(i) c(list(x[[i]]), list(...))
    dynamicClusterApply_log(cl, log, fun, length(x), argfun)
}

dynamicClusterApply_log <- function (cl = NULL, log = log_fn(), fun, n, argfun) {
