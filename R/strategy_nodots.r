...
# Nicely summarise a strategy
#' @export 
###############################################################################
strategy_performance_snapshoot <- 
  function(models, one.page = F, title = NULL, dataenv = NULL,
  ...
  for(n in spl('main,comparison,transition,monthly'))
    ...
    if(control$comparison) {
      # should always go in case.
      if(is.null(out))
        out = plotbt_strategy_sidebyside(models, return.table=T, make.plot = F)
      # Portfolio Turnover
      if(!is.null(dataenv)) {
        y = 100 * sapply(models, compute_turnover, dataenv)
        out = rbind(y, out)
        rownames(out)[1] = 'Turnover'		
        performance_barchart_helper(out, 'Sharpe,Cagr,DVR,MaxDD,Volatility,Turnover', c(T,T,T,T,F,F), sort.performance = sort.performance)
      } else	{
        # Fixed bug here.
        print("ED: in the weird section.  Fixed a bug where out was not generated in this section")
        performance_barchart_helper(out, sort.performance = sort.performance)
      }	
    }
    ...
    if(control$monthly) {	
      ...
      for(n in names(models))
        plotbt_monthly_table(models[[n]]$equity, smain=n)			
    }		
...
#' @export 
###############################################################################
strategy_load_historical_data <- function
(
	tickers = spl('DIA,SPY'),
	dates = '1900::',
	align = 'keep.all',
...
) 
{
	...
	bt_prep(data, align=align, dates=dates, fill.gaps=fill.gaps)
...
#' TODO, weird thing happens when out is not appropriate format?
#' I.e. if out cannot be cast to double, row 165, if out is a date.
#' @export 
###############################################################################
performance_barchart_helper <- function(out, 
	names = rownames(out),
...
) 
{
	...
	for(i in names) {
	  print(i)
		y = as.double(out[i,]) # out should be a matrix
		if(!is.na(y)){
...
###############################################################################
# helper function to create barplot_with_labels
#' @export 
###############################################################################
barplot_with_labels <- function(data, main, plotX = TRUE, label=c('level','name','both')) {
...
) 
{
	...
	# Load historical data 
	#****************************************************************** 
	data = strategy_load_historical_data(tickers, dates, fill.gaps=T)
	...
	models$meom.equal.weight = bt_run_share(data, clean.signal=F)
	...
	data$weight[] = NA;
		...
		data$weight[month.ends2,] = 0		
	models$meom.top2.rank1 = bt_run_share(data, clean.signal=F, trade.summary=T)		
	...
	data$weight[] = NA
		data$weight[month.ends,] = ntop(position.score[month.ends,], 2)		
		data$weight[month.ends2,] = 0		
	models$meom.top2.rank2 = bt_run_share(data, clean.signal=F, trade.summary=T)		
	...
	models$meom.top2.rank2.hold12 = bt_run_share(data, clean.signal=F, trade.summary=T)		
		
	return(rev(models))
}

meom_strategy_test <- function() 
{
	models = meom_strategy(
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part3(models$meom.top2.rank2, trade.summary=T)		
dev.off()	
...
) 
{
	...
	# Load historical data 
	#****************************************************************** 
	data = strategy_load_historical_data(tickers, dates)
	...
	sma = bt_apply_matrix(prices, SMA, ma.len)
	buy.rule = prices > sma
		buy.rule = ifna(buy.rule, F)
	...
	data$weight[] = NA
		data$weight[period.ends,]  = weight
	models$timing = bt_run_share(data, clean.signal=F, trade.summary=T)
		
	return(rev(models))
}

timing_strategy_test <- function() 
{
	models = timing_strategy(
...
) 
{
	...
	models$top = bt_run_share(data, clean.signal=T, trade.summary=T)

	
	# Seletop Top 2 funds,  and Keep then till they are in 1:6 rank
	data$weight[] = NA
		data$weight[period.ends,] = ntop_keep(position.score[period.ends,], top.n, keep.n)	
	models$top.keep = bt_run_share(data, clean.signal=T, trade.summary=T)
...
rotation_strategy_test <- function() 
{
	models = rotation_strategy(
...
#' @export
create_ia <- function(hist.returns, index=1:ncol(hist.returns), nperiod=nrow(hist.returns))
{	
...
#' @export
update_ia <- function(ia, name, cov_shrink)
{
	if(name != 'sample') {
		ia$cov = cov_shrink
...
#' @export
create_ia_subset <- function(ia.base, index=1:ia.base$n)
{	
	# setup input assumptions
	ia = list()	
...
# Create historical input assumptions
#' @export 	
create_historical_ia <- function
(
	hist.returns, 
	annual.factor
) {	
	# setup input assumptions
	ia = create_ia(hist.returns)
...
###############################################################################
#' @export 	
ia_build_hist <- function(hist.returns, lookbacks, n.lag)
{
...
#' @export 	
momentum_averaged <- function(prices, 
...
#' @export	
create_ia_averaged <- function(lookbacks, n.lag)
{
	lookbacks = lookbacks
	n.lag = n.lag
	...
	function(hist.returns, index, nperiod)
	{
		temp = ia_build_hist(hist.returns, lookbacks, n.lag)
		create_ia(temp, index, nperiod)
	}	
}
...
# static allocation
#' @export 
static_weight_portfolio <- function(static.allocation)
{
	...
	#' @export 
	equal_weight_portfolio <- function
	(
		ia,				# input assumptions
		constraints		# constraints
	)
	...
	#' @export 
	get_risky_asset_index <- function(ia) {
		if(is.null(ia$risk)) ia$risk = sqrt(diag(ia$cov))
		(ia$risk > 0) & !is.na(ia$risk)
	}
	#' @export 
	set_risky_asset <- function(x, risk.index) {
	...
	#' @export 	
	risk_parity_portfolio_basic <- function
	(
		ia,				# input assumptions
		constraints		# constraints
	)
	{
		if(is.null(ia$risk)) ia$risk = sqrt(diag(ia$cov))
		risk.index = get_risky_asset_index(ia)
				
		# re-scale weights to penalize for risk		
		x = 1 / ia$risk[risk.index]
		
		# normalize weights to sum up to 1
		set_risky_asset(x / sum(x), risk.index)
	}

# RP = risk_parity_portfolio()
# RP.CVAR = risk_parity_portfolio(function(ia) apply(ia$hist.returns, 2, compute_cvar))
# RP.MD = risk_parity_portfolio(function(ia) apply(apply(1+ia$hist.returns, 2, cumprod), 2, compute_max_drawdown)) 
# RP.CDAR = risk_parity_portfolio(function(ia) apply(apply(1+ia$hist.returns, 2, cumprod), 2, compute_cdar))
#	
# risk.parity allocation with custom risk functions
#' @export 
risk_parity_portfolio <- function(
	risk.fn = function(ia) ia$risk
)
{
	algo.map = list(
		'cvar' = function(ia) -apply(ia$hist.returns, 2, compute_cvar),
		'md' = function(ia) -apply(apply(1+ia$hist.returns, 2, cumprod), 2, compute_max_drawdown),
		'cdar' = function(ia) -apply(apply(1+ia$hist.returns, 2, cumprod), 2, compute_cdar)
	)
	...
	if(class(fn)[1] == 'try-error') stop(paste('risk_parity_portfolio', fn))
	...
	)
	{
		...
		# normalize weights to sum up to 1
		ifna(set_risky_asset(x / sum(x), risk.index), 0)
	}	
}
	...
	)
	{
		...
		set_risky_asset(sol$solution, risk.index)
	}	
	...
	)
	{
		...
		x = min_var_portfolio(ia, constraints, ia$correlation)
	...
	#' @export 	
	equal_risk_contribution_portfolio <- function
	(
		ia,                     # input assumptions
	    constraints             # constraints
	)
	{
		...
		# normalize weights to sum up to 1
		set_risky_asset(x$solution / sum(x$solution), risk.index)			
	}
	

# Based on CIR factory by mock-quant
#' @export 	
ef_portfolio <- function(percent = 0.5) 
{
	...
	)
	{
		...
		# need to set ia$expected.return for max_return_portfolio
		max.w = max_return_portfolio(ia, constraints)
		min.w = min_var_portfolio(ia, constraints)	
		
		# compute portfolio_returns
		...
		constraints = add_constraints(ia$expected.return, 
							type='>=', b=target, constraints)
							
		return(min_var_portfolio(ia, constraints))	
	}		
}	
...
#' @export 
min_te_portfolio_test <- function() 
{
	# minimum Tracking Error portfolios
	min.te.portfolio <- function(ia, constraints, index.weight)	
		min_var_portfolio(ia, constraints, dvec = index.weight %*% ia$cov)
	...
	# enforce min return
	constraints = create_basic_constraints(n, 0, 1, 1)
	constraints = add_constraints(ia$expected.return, type = '>=', b=0.95 * max(ia$expected.return), constraints)		
	x = min.te.portfolio(ia, constraints, index.weight)	
		x - index.weight
	252 * portfolio_return(x, ia)
}
...
#' @export 
#*****************************************************************
random_hist <- function(
	portfolio.fn = min_var_portfolio, # portfolio allocation algo
    nsamples = 100,         # Number of Samples to draw
    sample.len = 60         # Length of each sample	
)
{
	fn = try( match.fun(portfolio.fn) , silent = TRUE)
	if(class(fn)[1] == 'try-error') stop(paste('random_hist', fn))
...
)
{
	...
	scenarios = asset_paths_at_period_ends(data$prices, period.ends, nsamples, lookback.len=sample.len)
	#plota_matplot(scale_one(make_xts(scenarios[,,j],data$dates)),main='Asset Perfromance')
	
	for(j in 1:nsamples) {
		prices = make_xts(scenarios[,,j],data$dates)
...
#
# R Tools_for Portfolio Optimization by Guy Yollin - R/Finance
...
###############################################################################	
	...
	#' @export 
	max_sharpe_portfolio_helper <- function
	(
		ia,				# input assumptions
	...
	)
	{
		...
		if( all(excess.return <= 0) )
			constraints = add_constraints(excess.return, -1 , type = '=', constraints)		
		else
			constraints = add_constraints(excess.return, 1 , type = '=', constraints)		
		
		if( const == 'market-neutral' )
			constraints = add_constraints(rep(1,n), 0 , type = '=', constraints)		
	...
	)
	{
		const = const[1]
		const.sum = const.sum
		function(ia, constraints) { max_sharpe_portfolio_helper(ia, const, const.sum) }
	}
	
	
max_sharpe_portfolio_test <- function() 
{
	#*****************************************************************
...
png(filename = 'plot1.png', width = 500, height = 500, units = 'px', pointsize = 12, bg = 'white')
	...
	# plot_efficient frontier
	plot_ef(ia, list(ef), transition.map=F)	 
	
	# find maximum sharpe portfolio
	max(portfolio_return(ef$weight,ia) /  portfolio_risk(ef$weight,ia))
	
	# plot minimum variance portfolio
	weight = min_var_portfolio(ia,constraints)	
	points(100 * portfolio_risk(weight,ia), 100 * portfolio_return(weight,ia), pch=15, col='red')
	portfolio_return(weight,ia) /  portfolio_risk(weight,ia)
		
	# plot maximum Sharpe or tangency portfolio
	weight = max_sharpe_portfolio()(ia,constraints)	
	points(100 * portfolio_risk(weight,ia), 100 * portfolio_return(weight,ia), pch=15, col='orange')
	portfolio_return(weight,ia) /  portfolio_risk(weight,ia)
		
	plota_legend('Minimum Variance,Maximum Sharpe','red,orange', x='topright')
...
dev.off()	
	...
	#****************************************************************** 	
	weight = max_sharpe_portfolio('long-only')(ia,constraints)	
		round(weight,2)
		round(c(sum(weight[weight<0]), sum(weight[weight>0])),2)
		
	weight = max_sharpe_portfolio('long-short')(ia,constraints)			
		round(weight,2)
		round(c(sum(weight[weight<0]), sum(weight[weight>0])),2)

	weight = max_sharpe_portfolio('long-short', -1)(ia,constraints)			
		round(weight,2)
		round(c(sum(weight[weight<0]), sum(weight[weight>0])),2)
		
				
	weight = max_sharpe_portfolio('market-neutral')(ia,constraints)			
		round(weight,2)
		round(c(sum(weight[weight<0]), sum(weight[weight>0])),2)
...
max_div_portfolio_test <- function() 
{
	...
	# Create historical input assumptions
	ia = create_historical_ia(hist.returns, 12)
	
	constraints = create_basic_constraints(ia$n, 0, 1, 1)
	
	load_packages('quadprog,corpcor,lpSolve,kernlab')

	sol1 = max_div_portfolio(ia, constraints)
	...
	)
	{
		...
		constraints = new_constraints(ia$n, lb = 0)
 		constraints = add_constraints(diag(ia$n), type = ">=", b = 0, constraints)
			constraints = add_constraints(ia$risk, type = '=', b = 1, constraints)	
		x = min_var_portfolio(ia, constraints)
		
		# normalize weights to sum up to 1
		set_risky_asset(x / sum(x), risk.index)
	}
	
	sol2 = max_div_portfolio2(ia, constraints)
	
	# weights
	round(100*cbind(sol1,sol2),2)
		
	# risk contributions
	round(100*t(portfolio_risk_contribution(rbind(sol1,sol2), ia)),2)
	...
	sol = list(
	EW = equal_weight_portfolio(ia, constraints),
	ERC = equal_risk_contribution_portfolio(ia, constraints),
	MV = min_var_portfolio(ia, constraints),
	MDP = max_div_portfolio(ia, constraints),
	MDP2 = max_div_portfolio1(ia, constraints)
	)
	...
	# risk contributions
	round(100*portfolio_risk_contribution(t(sapply(sol,c)), ia))
	...
	# risk contributions
	round(100*portfolio_risk_contribution(weights.base, ia.base))
...
#' @export 
#*****************************************************************
	max_sharpe_nlp_portfolio <- function
	(
		ia,                     # input assumptions
	    constraints             # constraints
	)
...
find_portfolio_given_risk_test <- function() 
{
	...
	weight = max_return_portfolio(ia, constraints)
		weight
cat(round(100*c(portfolio_return(weight,ia), portfolio_risk(weight,ia), portfolio_return(weight,ia) /  portfolio_risk(weight,ia)),2), '\n')
...
cat(round(100*c(portfolio_return(weight,ia), portfolio_risk(weight,ia), portfolio_return(weight,ia) /  portfolio_risk(weight,ia)),2), '\n')
	...
	weight = target_return_portfolio_helper(ia,constraints, 12/100)	
		weight
cat(round(100*c(portfolio_return(weight,ia), portfolio_risk(weight,ia), portfolio_return(weight,ia) /  portfolio_risk(weight,ia)),2), '\n')

	weight = target_risk_portfolio_helper(ia,constraints, 10/100, silent=F)	
		weight
cat(round(100*c(portfolio_return(weight,ia), portfolio_risk(weight,ia), portfolio_return(weight,ia) /  portfolio_risk(weight,ia)),2), '\n')
	...
	#' @export 
min_corr_excel <- function(power.function = 1, final.scale = 'risk')
{
	...
	{					
		...
		x = min_corr_special_case(ia$risk[risk.index])
		if(!is.null(x)) return( set_risky_asset(x / sum(x), risk.index) )
		...
		rr.norm.dist.m = rep_col(rr.adjustment,n) * norm.dist.m
...
# http://cssanalytics.files.wordpress.com/2012/10/minimum-correlation-mincorr-spreadsheet.xlsx
min_corr_excel_portfolio_test <- function() 
{
	ia = list()
	ia$n = 3
	...
	min_corr_excel_portfolio(ia,constraints)				
...
#' @export 
min_corr_special_case <- function(risk) {
	n = len(risk)
	if(n == 1) 1
	else if(n == 2) 1/risk
	else NULL
...
# MinCorr by David Varadi
#' @export 
min_corr <- function(power.function = 1)
{
	power.function = as.numeric(power.function)
...
#' @export 
min_corr2 <- function(power.function = 1)
{
	power.function = as.numeric(power.function)
...
#' @export 
min_var2111 <- function(power.function = 1)
{
    power.function = as.numeric(power.function)
...
#' @export 
min_var_excel <- function(power.function = 1)
{
    power.function = as.numeric(power.function)
...
min_var_excel_portfolio_test <- function() 
{
...
'
	...
	ia = create_ia(matrix(scan(text=data), nc = len(tickers)))
...
'
	...
	min_var_excel_portfolio(ia,constraints)				
	...
	#' @export 
	min_var2_portfolio <- function(ia,constraints) { min_corr_excel(final.scale = 'vol')(ia,constraints) } 
	
	

#' @export 
min_var2 <- function(power.function = 1)
{
    power.function = as.numeric(power.function)    
    function(ia,constraints) min_corr_excel(power.function, final.scale = 'vol')(ia,constraints)      
}
	
	

	#' @export 
	min_var_excel_portfolio <- function(ia,constraints) { min_var_excel()(ia,constraints) } 	
		
	#' @export 	
	min_corr_excel_portfolio <- function(ia,constraints) { min_corr_excel()(ia,constraints) }
	
	#' @export 	
	min_corr_portfolio <- function(ia,constraints) { min_corr()(ia,constraints) }
	
	#' @export 	
	min_corr2_portfolio <- function(ia,constraints) { min_corr2()(ia,constraints) }
	
	#' @export 	
	min_cvar <- function(alpha = 0.95) { 
		alpha = alpha
		function(ia,constraints) {
			ia$parameters.alpha = as.numeric(alpha)
			min_cvar_portfolio(ia,constraints) 
		}
	}
	#' @export 	
	min_cdar <- function(alpha = 0.95) {
		alpha = alpha
		function(ia,constraints) {
			ia$parameters.alpha = as.numeric(alpha)
			min_cdar_portfolio(ia,constraints) 
		}
	}

	#' @export 	
	min_risk_downside <- function(mar = 0) { 
		mar = mar
		function(ia,constraints) {
			ia$parameters.mar = as.numeric(mar)
			min_risk_downside_portfolio(ia,constraints) 
		}
	}
	#' @export 	
	min_mad_downside <- function(mar = 0) { 
		mar = mar
		function(ia,constraints) {
			ia$parameters.mar = as.numeric(mar)
			min_mad_downside_portfolio(ia,constraints) 
		}
	}
	...
	#*****************************************************************
	#' @export 	
	sample_shrinkage <- function( hist, hist.all ) {
		cov(hist, use='complete.obs', method='pearson')
	}

	#' @export 	
	sample_anchored_shrinkage <- function( hist, hist.all ) {
		cov(hist.all, use='complete.obs', method='pearson')
	}
	
	#' @export 	
	sample_mix_shrinkage <- function( hist, hist.all ) {
		0.5 * sample_shrinkage(hist, hist.all) +
		0.5 * sample_anchored_shrinkage(hist, hist.all)
	}
	
	#' @export 	
	exp_sample_shrinkage <- function( hist, hist.all ) {
		hist = na.omit(hist)
	...
	#' @export 	
	diagonal_shrinkage <- function( hist, hist.all ) {
		n = ncol(hist)
	...
	#' @export 	
	max_shrinkage <- function( hist, hist.all ) {
		n = ncol(hist)
		correlation = cor(hist, use='complete.obs', method='pearson')
		create_cov_matrix(max(correlation-diag(n)), hist)
	}
		
	#' @export 	
	create_cov_matrix <- function( value, hist ) {
		n = ncol(hist)
		s0 = apply(hist, 2, sd, na.rm=T)
		temp = diag(n)
	...
	#' @export 	
	ledoit_wolf_shrinkage <- function( hist, hist.all ) {
		require(BurStFin)
		var.shrink.eqcor(hist, 1, compatible = T)
	}
		
	#' @export 	
	factor_model_shrinkage <- function( hist, hist.all ) {
		require(BurStFin)
	...
	#' @export 	
	cov_shrink <- function(h, prior = NULL, shrinkage = NULL, roff.method = 1) {	
		require(tawny)
	...
	#' @export 	
	cov_sample <- function(h) {		
		# center x, de-mean returns
		T = nrow(h)
		x = h - rep_row(colMeans(h), T)
	...
	#' @export 
	shrink_diag <- function(s=NULL) { s=s; function(x, a) { cov_shrink(x, cov_diag, s, 0)$sigma }}
	
	#' @export 	
	shrink_const_cor <- function(s=NULL) { s=s; function(x, a) { cov_shrink(x, cov_const_cor, s, 1)$sigma }}
	
	#' @export 	
	shrink_single_index <- function(s=NULL) { s=s; function(x, a) { cov_shrink(x, cov_market, s, 1)$sigma }}
	
	#' @export 	
	shrink_two_parameter <- function(s=NULL) { s=s; function(x, a) { cov_shrink(x, cov_2param, s, 1)$sigma }}
...
{	
	...
	# Find groups using clustering algorithm
	#' @export 	
	cluster_group_kmeans_90 <- function
	(
		ia				# input assumptions
	)
	{
	...
	# Find groups using clustering algorithm
	#' @export 	
	cluster_group_kmeans_elbow <- function
	(
		ia				# input assumptions
	)
	{		
		...
		icluster = find_maximum_distance_point(p.exp[-1]) + 1
...
cluster_group_FTCA_test <- function() {
	#*****************************************************************
	...
	portfolio_allocation_custom_stats_clusters <- function(x,ia) {
		return(list(
			clusters.FTCA = cluster_group_FTCA(0.5)(ia)			
		))
	}
	...
	plot.data = coredata(temp1)
		rownames(plot.data) = format(index_xts(temp1), '%Y%m')		
	plot_table(plot.data, highlight = plot.data + 1)
	...
	obj = portfolio_allocation_helper(data$prices, 
		...
		min.risk.fns = list(
			# cluster
			C.EW.kmeans = distribute_weights(equal_weight_portfolio, cluster_group_kmeans_90),
			C.EW.FTCA = distribute_weights(equal_weight_portfolio, cluster_group_FTCA(0.5)),

			C.RP.kmeans = distribute_weights(risk_parity_portfolio(), cluster_group_kmeans_90),
			C.RP.FTCA = distribute_weights(risk_parity_portfolio(), cluster_group_FTCA(0.5)),

			C.MD.kmeans = distribute_weights(max_div_portfolio, cluster_group_kmeans_90),
			C.MD.FTCA = distribute_weights(max_div_portfolio, cluster_group_FTCA(0.5)),

			C.MV.kmeans = distribute_weights(min_var_portfolio, cluster_group_kmeans_90),
			C.MV.FTCA = distribute_weights(min_var_portfolio, cluster_group_FTCA(0.5)),
						
			C.MVE.kmeans = distribute_weights(min_var_excel_portfolio, cluster_group_kmeans_90),
			C.MVE.FTCA = distribute_weights(min_var_excel_portfolio, cluster_group_FTCA(0.5)),

			C.MCE.kmeans = distribute_weights(min_corr_excel_portfolio, cluster_group_kmeans_90),
			C.MCE.FTCA = distribute_weights(min_corr_excel_portfolio, cluster_group_FTCA(0.5)),

			C.MS.kmeans = distribute_weights(max_sharpe_portfolio(), cluster_group_kmeans_90),
			C.MS.FTCA = distribute_weights(max_sharpe_portfolio(), cluster_group_FTCA(0.5)),
			
			C.ERC.kmeans = distribute_weights(equal_risk_contribution_portfolio, cluster_group_kmeans_90),
			C.ERC.FTCA = distribute_weights(equal_risk_contribution_portfolio, cluster_group_FTCA(0.5))		
		)
	)
...
dev.off()
	...
	###############################################################################
	# min.risk.fns = list(G0.MV = distribute_weights(min_risk_portfolio, empty_group),					
	# 					G2.MV = distribute_weights(min_risk_portfolio, cluster.group))
	#' @export 	
	distribute_weights <- function
	(
		fn,				# function that dictates how to distribute_weights both across and within groups
		...
		{		
			...
			for(g in 1:ngroups) {
				...
				if( sum(index) == 1 ) {
					...
					ia.temp = create_ia_subset(ia, index)

					constraints.temp = create_basic_constraints(ia.temp$n, 0, 1, 1)

					w0 = match.fun(fn.within)(ia.temp, constraints.temp)
						weight0[index] = w0
						# Note that: sd(return0) = portfolio_risk(weight0, ia)	
			...
			constraints.g = create_basic_constraints(ngroups, 0, 1, 1)				
	...
	#*****************************************************************
	#' @export 	
	get_algo <- function(algo.name, has.param = F) {
		algo.map = list(
			'cluster' = distribute_weights,
		...
		if(any(names(algo.map) == algo.name))
			...
			if(class(fn)[1] == 'try-error') stop(paste('get_algo', fn))
			fn
		}
	}
	
	#' @export 		
	get_group <- function(group.name) {
		group.map = list(
			'none' = empty_group,
			'hclust' = cluster_group_hclust,
			'kmeans90' = cluster_group_kmeans_90
		)	
	...
	# parse strategys
	#' @export 	
	map_min_risk_fns <- function(strategys) {	
		strategys = spl(strategys,';')
		
		min.risk.fns = list()
		#strategys = spl(";EW,Equal.Weight,;MCE,Min.Corr.Excel,1;MC,Min.Corr,1;MC2,Min.Corr2,1;C.EW,Cluster,kmeans90:min.var;Empty,Cluster,hclust:min_corr2",';')		
		...
		for(i in 1:len(strategys)) {
			...
			temp = tolower(temp)[-1]		
			if(len(temp) == 1)
				min.risk.fns[[ f.name ]] = get_algo(temp[1])
			else {
				if(temp[1] == 'cluster') {
					params = trim(spl(temp[2], ':'))
					min.risk.fns[[ f.name ]] = get_algo(temp[1], T)( get_algo(params[2]), get_group(params[1]) )								
				} else
					min.risk.fns[[ f.name ]] = get_algo(temp[1],T)(temp[-1])
			}
		}					
		min.risk.fns	
	}
	...
	#*****************************************************************
	#' @export 	
rso_portfolio <- function
(
    weight.fn,    # function that dictates how to distribute_weights
...
)
{
    ...
    constraints0 = create_basic_constraints(k, const.lb, const.ub, const.sum)
    ...
    )
    {
		...
		if(k > ia$n) {
			k1 = ia$n			
			constraints1 = create_basic_constraints(k1, const.lb, const.ub, const.sum)
		}
        ...
        for(i in 1:s){
        	ia.temp = create_ia_subset(ia, index.samples[i,])
...
#' @export 
#*****************************************************************
portfolio_allocation_helper_parallel <- function
(
	cores = 1,
	...
	min.risk.fns = 'min_var_portfolio',	# portfolio construction functions
	custom.stats.fn = NULL,
	shrinkage.fns = 'sample_shrinkage',	# covariance Shrinkage Estimator functions
	
	create_ia.fn = create_ia,
	update_ia.fn = update_ia,
...
) 
{
	cores = round(cores)
	if(cores <= 1)
		return(portfolio_allocation_helper(prices, periodicity, period.ends, lookback.len, n.skip,
			universe, prefix,
			min.risk.fns, custom.stats.fn, shrinkage.fns,
			create_ia.fn, update_ia.fn,
	...
	load_packages('foreach,doParallel')
...
#	temp[-c(chunks[i] : (chunks[i+1]-1))]
	...
	out <- foreach(i=1:cores, .packages=spl('quantmod,SIT')) %dopar% {
		...
		portfolio_allocation_helper(prices, periodicity, period.ends, lookback.len, n.skip,
...
#' @export 
#*****************************************************************
portfolio_allocation_helper <- function
(
	prices,					# prices
	periodicity = 'weeks',	#  rebalancing frequency
	period.ends = endpoints(prices, periodicity),	# rebalancing times
...
) 
{
	...
	if( !is.null(custom.stats.fn) ) {
		...
		dummy = matrix(NA, nr=nrow(weight), nc=len(weights))		
			...
			dummy = make_xts(dummy, dates)	
			
   		#temp = custom.stats.fn(1:ncol(ret), create_ia(ret))
   		temp = ret
   			temp[] = rnorm(prod(dim(ret)))
   		temp = custom.stats.fn(1:ncol(ret), create_ia(temp))
...
if(!is.na(start.i)) {
	...
	for( j in start.i:len(period.ends) ) {
		...
		if(n > 0) {
			...
			if(n > 1) {
				constraints = create_basic_constraints(n, const.lb[index], const.ub[index], const.sum)
							
				# create historical input assumptions
				ia.base = create_ia.fn(hist, index.map[index], i)
				
				for(c in names(shrinkage.fns)) {
					cov_shrink = shrinkage.fns[[c]](hist, hist.all)					
					ia = update_ia.fn(ia.base, c, cov_shrink)
				...
				ia = create_ia.fn(hist, index.map[index], i)
...
#' @export 
portfolio_allocation_custom_stats <- function(x,ia) {
	...
	return(list(
		...
		risk.gini = 1 - portfolio_concentration_gini_coefficient(risk.contributions)
	))
}



# this is a basic version of portfolio_allocation_helper function
#' @export 
portfolio_allocation_helper_basic <- function
(
	prices,					# prices
	periodicity = 'weeks',	#  rebalancing frequency
	period.ends = endpoints(prices, periodicity),	# rebalancing times
...
) 
{
	...
	if(!is.na(min.weight) && min.weight != 0) for(i in names(obj$weights)) 
		obj$weights[[i]][] = bt_apply_min_weight(coredata(obj$weights[[i]]), min.weight)
	
	if(!is.na(round.weight) && round.weight != 0) for(i in names(obj$weights)) {
		# round a few times to get more consisitent results
		obj$weights[[i]][] = bt_apply_round_weight(coredata(obj$weights[[i]]), round.weight)
		obj$weights[[i]][] = bt_apply_round_weight(coredata(obj$weights[[i]]), round.weight)
		obj$weights[[i]][] = bt_apply_round_weight(coredata(obj$weights[[i]]), round.weight)
	}
...
#			data$weight[obj$period.ends,] = as.matrix(obj$weights[[j]][1:len(obj$period.ends),])
		...
		models[[i]] = bt_run_share(data, clean.signal = clean.signal, silent = silent, ...)
...
#*****************************************************************
asset_allocation_strategy_test <- function() 
{
	...
	strategy_performance_snapshoot(models)
...
#' @export 
#****************************************************************** 				
target_vol_strategy <- function(model, weight, 
	target = 10/100, 
	lookback.len = 21,
...
# Calendar Strategy
#*****************************************************************
	#signals = calendar_signal(key.date, 0, 1, 2, -1, -2)
	#names(signals)
	#signals = calendar_signal(key.date, T0=0, 1, 2, N1=-1, -2,P2N2=-2:2)
	#names(signals)	
	# advanced ... - offsets
	#' @export 
	calendar_signal <- function(key.date, ...) {
		offsets = list( ... )
	...
	#names(models)	
	#models = calendar_strategy(data, A=signals[[1]], signals[[1]])
	#names(models)	
	# advanced ... - signals
	#' @export 
	calendar_strategy <- function(data, ..., universe = data$prices > 0, do.lag.universe = 1, commission = 0) {
		signals = list( ... )		
		...
		for(n in names(signals)) {
			...
			models[[n]] = bt_run_share(data, do.lag = 0, trade.summary=T, clean.signal=T, commission = commission, silent=T)  	
		}
		models
	}	
	...
	#' @export 
	#*****************************************************************
	last_trades <- function(..., n=20, make.plot=T, return.table=F, smain = NULL) {
	...
	#' @export 
	#*****************************************************************	
	last_signals <- function(..., n=20, make.plot=T, return.table=F, smain = NULL) {
		models = variable_number_arguments( ... )
		model = models[[1]]
		name=ifnull(names(models),NULL)[1]
