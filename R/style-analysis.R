...
############################################################################### 
#' renamed factor.rolling.regression to style_roll (shorter)

style_roll.xts <- function(equity, factors, window.len = 36, silent = F, 
...
) 
{
  ...
  carhart_reg <- function(ticker, align_ret_factor) {
    reg_formula <- as.formula(paste0(ticker, " ~ ." ))
    #  use R lm instead of SIT::ols fn.
...
style_roll_bt_plot <- function(obj) {
  ...
  models[[ticker]] = bt.run.share(xtsenv, clean.signal = F)
  
  xtsenv$weight[] = NA
  xtsenv$weight[,3:(n+2)] = t(repmat(obj$full_sample_res$estimate[1:n], 1, nperiods))
  xtsenv$weight$RF = 1
  xtsenv$weight[1:obj$window.len,] = NA
  models$all.alpha = bt.run.share(xtsenv, clean.signal = F)
  ...
  xtsenv$weight[1:obj$window.len,] = NA
  models$all = bt.run.share(xtsenv, clean.signal = F)
  
  xtsenv$weight[] = NA
  xtsenv$weight[,3:(n+2)] = obj$fl$estimate[,1:n]
  xtsenv$weight$RF = 1
  xtsenv$weight[1:obj$window.len,] = NA
  models$est.alpha = bt.run.share(xtsenv, clean.signal = F)
  ...
  xtsenv$weight$alpha = NA	
  xtsenv$weight[1:obj$window.len,] = NA
  models$est = bt.run.share(xtsenv, clean.signal = F)
