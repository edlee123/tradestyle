...
#' @param method method used to compute_correlations, please see \code{\link{cor}} for more details
#'
...
#' @export 
############################################################################### 
proxy_test <- function(data.all, names = ls(data.all), price.fn=Ad)
{
	#*****************************************************************
	# Prepare data
	#****************************************************************** 	
	...
	for(n in data$symbolnames)
		data[[n]] = make_stock_xts( price.fn( data.all[[n]] ) )	
...
layout(1)
		plota_matplot(scale_one(prices))
	...
	# compute_stats
	temp = compute_stats( as.list(rets),
			list(
...
#' @export 
############################################################################### 
plot12month_rolling_spread <- function(data.all, names = ls(data.all), price.fn=Ad)
{
	#*****************************************************************
	# Prepare data
	#****************************************************************** 	
...
#'   getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)   
#'
#' proxy_overlay_plot(data)
#' }
#' @export 
############################################################################### 
proxy_overlay_plot <- function(data.all, names = ls(data.all), price.fn=Ad)
{
	#*****************************************************************
	# Prepare data
	#****************************************************************** 	
...
#' @export 
############################################################################### 
proxy_prices <- function(data, names = ls(data)) {
	n.names = len(names)
	temp = list()
...
#	layout(1:(n.names+1))	
	layout(1:n.names)
	for(n in names) {
		plota_matplot(cbind(Cl(data[[n]]),Ad(data[[n]])),main=n)
		temp[[ paste(n, 'Price') ]] = Cl(data[[n]])
		temp[[ paste(n, 'Total') ]] = Ad(data[[n]])
	}
	
	# compute_stats
	temp = compute_stats( lapply(temp, function(x) ifna(x/mlag(x) -1,NA)),
			list(
				Mean=function(x) 252*mean(x,na.rm=T),
				StDev=function(x) sqrt(252)*sd(x,na.rm=T)
				)
...
#' @export 
proxy_map <- function(raw.data, tickers)
{
	#*****************************************************************
	# Prepare data
	#******************************************************************
  ...
  getSymbols_extra(tickers, src = 'yahoo', from = '1980-01-01', env = data, raw.data = raw.data, set.symbolnames = T, auto.assign = T)
