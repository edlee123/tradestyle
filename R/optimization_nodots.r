...
)
{
	...
	if ( binary.vec[1] == 0 ) {	
		...
		qp.data.final = solve_QP_remove_equality_constraints(Dmat, dvec, Amat, bvec, meq)	
...
)
{
	...
	qp.data.final = solve_QP_remove_equality_constraints(qp_data$Dmat, qp_data$dvec, 
...
mbqp_test <- function()
{
	...
	sol = solve_QP_bounds(Dmat=Dmat, dvec=dvec, Amat=Amat, bvec=bvec, meq=0, binary.vec = index_binvar) 
		xsol = sol$solution
		fsol = sol$value
...
solve_QP_remove_equality_constraints <- function
(
	Dmat,				# solve.QP parameters
	dvec, 				# solve.QP parameters
	Amat, 				# solve.QP parameters
...
###############################################################################
# Logic to remove reduandant equality constraints in QP problem, based on qp_solve
solve_QP_remove_equality_constraints12 <- function
(
	Dmat,				# solve.QP parameters
	dvec, 				# solve.QP parameters
	Amat, 				# solve.QP parameters
...
)
{
	...
	qp.data.temp = remove_equality_constraints_old(qp.data.temp)
	...
	if( len(qp.data.temp$var.index) == len(qp.data.temp$solution) ) {
		...
		qp.data.final = remove_equality_constraints_old(qp.data.temp)		
...
remove_equality_constraints_old <- function(qp.data)
{
...
)
{
	if( is.null(constraints) ) {
		fit = lm.fit(x, y)
		return( ols_summary(x, y, fit$coefficients) )
	} else {
    	temp = cov(cbind(y, x))
		Dmat = temp[-1,-1] 
		dvec = temp[-1,1]
	
		sol = solve_QP_bounds(Dmat = Dmat, dvec = dvec , 
				Amat=constraints$A, bvec=constraints$b, constraints$meq,
				lb = constraints$lb, ub = constraints$ub)
		return( ols_summary(x, y, sol$solution) )
	}
}
...
)
{
	...
	if(computeSummary) {
		return( ols_summary(x, y, coefficients, xinv) )
	} else {
...
#' @export 
###############################################################################
find_maximum_distance_point <- function
(
	y, 
	x=1:len(y)
)
