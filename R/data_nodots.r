...
)
{
  pos1 = 1
  if (nchar(smarker) > 0)
    pos1 = find_tokens(txt, smarker, pos, pos.start = keep.marker)
  if( pos1 < 0 ) return("")
  pos1.marker = iif(keep.marker, pos1 + nchar(last(spl(smarker))), pos1)
  
  pos2 = nchar(txt)
  if (nchar(emarker) > 0)
    pos2 = find_tokens(txt, emarker, pos1.marker, pos.start = !keep.marker) - 1
  if( pos2 < 0 ) return("")
...
{
  token = extract_token(txt, smarker, emarker, pos, keep.marker = T)
  if(nchar(token) == 0) 
    txt
  else
    replace_token(gsub(pattern = token, replacement = replacement, txt), smarker, emarker, replacement)
}
...
###############################################################################
# extract_table_from_webpage
#' @export 
###############################################################################
extract_table_from_webpage <- function
(
...
###############################################################################
# Test for extract_table_from_webpage function
###############################################################################
extract_table_from_webpage_test <- function()
{
  ...
  temp = extract_table_from_webpage(txt, 'Market Cap', has.header = F)
  ...
  plota(y, type = 'candle', main = Symbol, plotX = F)
    plota_lines(sma50, col='blue')
    plota_legend(c(Symbol,'SMA 50'), 'green,blue', list(y,sma50))
    
  y = plota_scale_volume(y)
  plota(y, type = 'volume')   
...
processTBill_test <- function() 
{
  ...
  plota(cumprod(1 + ir), type='l', log = 'y')
    plota_lines(Ad(out), type='l', col='red')
  ...
  compute_raw_annual_factor(tbill.d)
  compute_raw_annual_factor(tbill.m)
  ...
  plota(tbill.d, type='l')    
    plota_lines(tbill.m, type='s', col='blue')                
    plota_lines(SHY, type='l', col='red')
  plota_legend('Daily 3YR T-Bills,Monthly 3YR T-Bills,SHY','black,blue,red')
...
###############################################################################
# ... parameters for read.xls_function
# i.e. CRB = get_CRB(perl = 'c:/perl/bin/perl.exe')
#
# This url is not working anymore, for updated example please see
#   bt_extend_DBC_update_test in bt_test.r 
#' @export 
###############################################################################
get_CRB <- function(...)
{
  load_packages('gtools,gdata')
  ...
  out = repmat(as.double(temp[,2]), 1, 6)
      ...
      out[, 'Volume'] = 0
  #out = make_xts( out, as.Date(temp[,1], '%m/%d/%y'))
  out = make_xts( out, as.POSIXct(temp[,1], tz = Sys.getenv('TZ'), format='%m/%d/%y'))  
    indexClass(out) = 'Date'  
...
get_CRB_test <- function()  
{
  ...
  layout(1:3) 
  plota_matplot(CRB[,c('Close','Adjusted')])  
  plota_matplot(DBC[,c('DBC.Close','DBC.Adjusted')])  
  plota_matplot(GSG[,c('GSG.Close','GSG.Adjusted')])  
  
  
  layout(1)       
  comm = extend_data(DBC, CRB, scale=T)
  plota(comm, type='l', col=1)
    plota_lines(CRB*0.078, type='l', lwd=5, col=col_add_alpha(2,150))
    plota_lines(DBC, type='l', lwd=5, col=col_add_alpha(3,150))
    plota_lines(comm, type='l', col=1)
  plota_legend('comm,CRB,DBC', 1:3, list(comm,CRB,DBC))
} 
...
dow_jones_components <- function()
{
  ...
  temp = extract_table_from_webpage(temp, 'Volume', has.header = T)
  trim(temp[,'Company'])
}	

dow_jones_components_0 <- function()
{
  url = 'http://finance.yahoo.com/q/cp?s=^DJI+Components'
  txt = join(readLines(url))

  # extract table from this page
  temp = extract_table_from_webpage(txt, 'Volume', has.header = T)
  temp[, 'Symbol']
}

dow_jones_components_1 <- function()
{
  load_packages('readxl,httr')
...
#' @export 
###############################################################################
nasdaq_100_components <- function()
{
...
#' @export
###############################################################################
sector_spdr_components <- function(sector.etf = 'XLE')
{
...
sp500_components <- function()
{
  ...
  # extract table from this page  
  temp = extract_table_from_webpage(txt, 'Ticker', has.header = T)
...
sp100_components <- function()
{
  ...
  # extract table from this page  
  temp = extract_table_from_webpage(txt, 'Components', has.header = T)
...
ftse100_components <- function()
{
  ...
  # extract table from this page    
  txt = gsub('&#37;','%',txt)
  temp = extract_table_from_webpage(txt, 'Security', has.header = T)
  ...
  for(i in 1:100) { 
    ...
    # extract table
    temp.table = extract_table_from_webpage(txt, 'Price', has.header = T)
    ...
    temp = extract_table_from_webpage(temp, 'Price', has.header = T)
...
# Get Dow Jones Components
# http://finance.yahoo.com/q/cp?s=^DJI+Components
# us_ishares_components(date='2008-02-01')
#' @export 
###############################################################################
us_ishares_components <- function(Symbol = 'DVY', date = NULL, debug = F)
{
  url = paste('http://us.ishares.com/product_info/fund/holdings/', Symbol, '.htm?periodCd=d', sep='')
  if( !is.null(date) )
    url = paste('http://us.ishares.com/product_info/fund/holdings/', Symbol, '.htm?asofDt=', date_end(date), '&periodCd=m', sep='')
  txt = join(readLines(url))

  # extract date from this page
  temp = remove_tags(extract_token(txt, 'Holdings Detail', 'Holdings subject to change'))
  date = as.Date(spl(trim(temp),' ')[3], '%m/%d/%Y')
  
  # extract table from this page
  temp = extract_table_from_webpage(txt, 'Symbol', has.header = T)
...
{  
  ...
  for(i in 2:len(tokens)) {
    token = tokens[i] 
    token = extract_token(token, '<a href=', '</a>', keep.marker = T)
    url = extract_token(token, 'url\\?q=', '&amp;sa=U&amp;')  
    label = remove_tags(token) 
...
#' @export 
###############################################################################
#getQuote_google(spl('MSFT,AAPL,IBM'))
getQuote_google <- function(tickers) {
...
#getQuote_google_xml(spl('MSFT,AAPL,NYSE:RY'))
#' @export 
getQuote_google_xml <- function(tickers) {
...
# getSymbol_intraday_google('GOOG','NASDAQ')
# getSymbol_intraday_google('.DJI','INDEXDJX')
#' @export 
###############################################################################
getSymbol_intraday_google <- function
(
  Symbol, 
  Exchange,
...
#
# getQuote_yahoo_info('DJP,IEF,KIE,RHS')
#
#' @export 
###############################################################################   
getQuote_yahoo_info <- function(Symbols, fields = c(
	Name='Name',
	Symbol='Symbol',
	...
	Symbols = spl(Symbols)
	out = getQuote_yahoo_today(Symbols, fields)
	...
	out$Start = bt_start_dates(data)[out$Symbol,]	
	out
}


#' @export 
getQuote_yahoo_today <- function(Symbols, fields = c(
	Name='Name',
	Symbol='Symbol',
	Time='Last Trade Time',
	Date='Last Trade Date',
...
###############################################################################
# extend_GLD and SLV historical prices
#' @export 
###############################################################################
extend_GLD <- function(GLD) {
  extend_data(GLD, bundes_bank_data_gold(), scale=T)
}

# gold = extend_GLD(data$GLD)
# comm = extend_data(data$DBC, get_CRB(), scale=T)
#' @export 
...
) 
{
  ...
  # find Close in hist
  close.index = find_names('Close', hist)
  if(len(close.index)==0) close.index = 1 
  adjusted.index = find_names('Adjusted', hist)
  if(len(adjusted.index)==0) adjusted.index = close.index 

  if(scale) {
    cur.close.index = find_names('Close', current)
    if(len(cur.close.index)==0) cur.close.index = 1 
    cur.adjusted.index = find_names('Adjusted', current)
  ...
  #hist = make_xts( rep_col(hist[,adjusted.index], ncol(current)), index(hist)) 
  if( ncol(hist) != ncol(current) ) 
    hist = rep_col(hist[,adjusted.index], ncol(current))
  else
...
# extend_data from the previously saved proxies
#' @export 
extend_data_proxy <- function(data, data.proxy = NULL, proxy.filename = 'data.proxy.Rdata') {
  if(is.null(data.proxy) && file.exists(proxy.filename))
    ...
    for(n in ls(data.proxy))
        if( !is.null(data[[n]]) )
          data[[n]] = extend_data(data[[n]], data.proxy[[n]], scale=T)
}  
...
#
# Please use only with Adjusted time series. For example create_leveraged(data$QQQ, leverage=2)
# will produce erroneous values because QQQ had 2: 1 Stock Split on Mar 20, 2000 
# Hence at 2x leverage the value goes to zero.
#
# @example create_leveraged(tlt, 2)
# @example extend_data(data$UBT, create_leveraged(data$TLT, leverage=2), scale=T)
# @example extend_data(data$TMF, create_leveraged(data$TLT, leverage=3), scale=T)
#' @export 
create_leveraged = function(hist, leverage=2) {
  rets = 1 + leverage * (hist / mlag(hist) - 1)
  rets[1,] = 1  
  bt_apply_matrix(rets, cumprod)
} 


create_leveraged_test = function() {
    ...
    test2 = extend_data(data$UBT, create_leveraged(data$TLT, leverage=2), scale=T)
    test3 = extend_data(data$TMF, create_leveraged(data$TLT, leverage=3), scale=T)    
    proxy_test(list(TLT=data$TLT, UBT=test2, TMF=test3),price.fn=Ad)
  
    test0 = create_leveraged(data$TLT, leverage=2)    
    proxy_test(list(UBT=data$UBT, EXTEND=test0),price.fn=Ad)
    
    test0 = create_leveraged(data$TLT, leverage=3)    
    proxy_test(list(TMF=data$TMF, EXTEND=test0),price.fn=Ad)
...
#' @export 
###############################################################################  
bundes_bank_data <- function(symbol) {
  ...
  #hist = make_xts(as.double(temp[,2]), as.Date(temp[,1], '%Y-%m-%d'))    
  hist = make_xts(as.double(temp[,2]), as.POSIXct(temp[,1], tz = Sys.getenv('TZ'), format='%Y-%m-%d'))
...
#' @export 
bundes_bank_data_gold <- function() {
  bundes_bank_data('BBEX3.D.XAU.USD.EA.AC.C05')
}
...
# fx.data = rbind(fx_sauder_data(2000, 2003, base.cur, target.curs), 
#       fx_sauder_data(2004, 2007, base.cur, target.curs), 
#       fx_sauder_data(2008, 2011, base.cur, target.curs),
#       fx_sauder_data(2012, 2012, base.cur, target.curs))
#' @export 
###############################################################################  
fx_sauder_data <- function(start.year, end.year, base.cur, target.curs) {
  ...
  #hist = make_xts(as.matrix(temp[,-c(1:3)]), as.Date(temp[,2], '%Y/%m/%d'))    
  hist = make_xts(as.matrix(temp[,-c(1:3)]), as.POSIXct(temp[,2], tz = Sys.getenv('TZ'), format='%Y/%m/%d'))
    indexClass(hist) = 'Date'
...
) 
{
  ...
  for (i in 1:len(Symbols)) { 
    ...
    #out = make_xts(temp[,-1], as.Date(temp[,1],'%m/%d/%Y'))
    out = make_xts(temp[,-1], as.POSIXct(temp[,1], tz = Sys.getenv('TZ'), format='%m/%d/%Y'))
      indexClass(out) = 'Date'
...
{   
  ...
  for (i in 1:len(Symbols)) { 
    ...
    out = make_xts(temp[,spl('OPEN,LOW,HIGH,CLOSE')], 
...
'
  ...
  bt_prep(data.fx, align='remove.na')
  fx = bt_apply(data.fx, '[')
...
#' @export 
###############################################################################     
wealthsimple_portfolio = function(portfolio.number = 10) {
	# download
	...
	# extract
	temp = extract_table_from_webpage(txt, 'Breakdown', has.header = F)
...
wealthsimple_portfolio_test = function() {
	# create list of all portolios
	portfolios = list()
	for(i in 1:10)
		portfolios[[i]] = wealthsimple_portfolio(i)
		
	portfolios = t(sapply(portfolios, identity))
	
	# look at evolution of mixes
	plota_stacked(1:10, portfolios/100, flip.legend = T, type='s', xaxp=c(1,10,9), las=1,
...
) 
{
  ...
  for( i in 1:nrow(def) ) {
    ...
    if(file.exists(filename)) {                   
      ...
      fr <- make_xts(fr[,-1], as.Date(as.character(fr[,1]),'%Y%m%d'))     
...
cat(i, 'out of', nrow(def), 'Reading', symbol, format(index_xts(fr)[1],'%Y%m%d'), format(index_xts(fr)[nrow(fr)],'%Y%m%d'), '\n', sep='\t')   
    } else {
...
png(filename = 'plot_CL_2009.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')			    
	...
	plota_matplot( scale_one(plot.data['2009']), main='Crude oil, CL - 2009')
...
#
# data2 = get_fama_french_data('F-F_Research_Data_Factors', periodicity = 'weeks',download = F, clean = F)
# data3 = get_fama_french_data('6_Portfolios_2x3', periodicity = 'days',download = F, clean = F)  
#' @export 
###############################################################################
get_fama_french_data <- function(
...
{
	warning('get_fama_french_data is depreciated as of Apr 25, 2016 please use data_ff function instead')
	data_ff(name, periodicity, force.download, clean, file.suffix)
}
...
## download using curl  !!!!
  ...
  if(len(files) == 1) {
  	...
  	return( data_ff_internal_one_file(filename) )
  }
  ...
  for(i in 1:len(files))
  	data[[ names[i] ]] = data_ff_internal_one_file(files[i])
...
data_ff_internal_one_file = function(filename) {  
  ...
  for(i in 1:nrow(data.index)) {
    ...
    data[[name]] = make_xts(temp[,-1], as.Date(paste(temp[,1], date.format.add, sep=''),date.format))
...
#' @export 
###############################################################################
download_helper <- function(url,download) {
  # setup temp folder
  temp.folder = paste(getwd(), 'temp', sep='/')
  dir.create(temp.folder, F)
...
#for(y in date_year(Sys.Date()):(1+date_year(Sys.Date())))
# for(m in 1:12) {
#   temp = getSymbol_CBOE('VX', m, y, T)
#' @export 
getSymbol_CBOE <- function
(
  Symbol, 
  Month,
  Year,
...
) 
{
  ...
  filename = download_helper(url, download)   
  if(file.exists(filename) && file.info(filename)$size > 1)
    read_xts(filename, format='%m/%d/%Y')
  else
    NULL
}
...
#' @export 
cboe_volatility_term_structure_SPX <- function(make.plot = T) {
  url = 'http://www.cboe.com/data/volatilityindexes/volatilityindexes.aspx'
  txt = join(readLines(url))

  temp.table = extract_table_from_webpage(txt, 'Trade Date', has.header = T)
...
load_VXX_CBOE <- function() {
  ...
  futures = list()  
  i = 1
  for(y in 2004:(1+date_year(Sys.Date())))
    for(m in 1:12) {
      temp = getSymbol_CBOE('VX', m, y)
      ...
      if(i==1)
        ...
        futures[[ i ]]$dt = dr(dates, futures[[ i-1 ]]$settle.date)
          ...
          temp$i = i
          #number of business_days in roll period
  ...
  # debug
  # bt_apply(data, function(x) x[,'dr'])[1150:1160,48:55]
  # count(t(coredata(bt_apply(data, function(x) x[,'Settle']))))
  
  data
}
  
#' @export 
extract_VXX_CBOE <- function(data, field, index, exact.match=T) {
  map  = 1:ncol(data$prices)
  
  temp = bt_apply(data, function(x) x[,field])
    temp = coredata(temp)
...
# page PS-20
#' @export 
reconstruct_VXX_CBOE <- function(exact.match=T) {
  data = load_VXX_CBOE()

  dt = extract_VXX_CBOE(data, 'dt', 1, exact.match)[1,]
  dr = extract_VXX_CBOE(data, 'dr', 1, exact.match)[1,]
  x  = extract_VXX_CBOE(data, 'Settle', 1:2, exact.match)
  ...
  VXX = make_xts(cbind(Close,x,dt,dr,ret), data$dates)
  
  # VXZ Mid-Term: 4,5,6,7 months
  x  = extract_VXX_CBOE(data, 'Settle', 4:7, exact.match)
  ...
  Close = cumprod(1+ifna(ret,0))  
  VXZ = make_xts(cbind(Close,x,dt,dr,ret), data$dates)
  
  # debug
  # plota(VXZ,type='l',lwd=2) 
  # write_xts(VXZ, file='vix-mid.txt')  
...
(
  force.download = FALSE,
  data.filename = 'country_code.Rdata'
)
{ 
	...
	temp = extract_table_from_webpage(txt, 'Country or Area Name')
...
#' @examples
#' \dontrun{ 
#' data_ft_search_ticker('s&p 500')
#' }
#' @export
#' @rdname DataFTFunctions
###############################################################################
data_ft_search_ticker = function
(
  search.field = 'tsx',
  sec.type = 'IN'
)
{
    ...
    temp = extract_table_from_webpage(temp, 'Symbol,Exchange,Country')
...
#' @examples
#' \dontrun{ 
#' data_ft_index_members('TSEA:TOR')
#' }
#' @export
#' @rdname DataFTFunctions
###############################################################################
data_ft_index_members = function
(
  ft.symbol = 'INX:IOM',
  force.download = FALSE,
...
)
{
    ...
    temp = extract_table_from_webpage(temp, 'Equities')
		
    # 
	# Paging
	#	
	library(stringr)
	token = extract_token(txt,'<div class="wsod-paging-key">','</div>')
	...
	token = extract_token(txt,'<div class="wsodHidden">','</div>')
	...
	for(istart in seq(nstep+1,nfound,by=nstep)) {
		...
		temp = extract_table_from_webpage(temp, has.header=F)
...
#' @examples
#' \dontrun{ 
#' data_ishares_universe()
#' }
#' @export
#' @rdname DataFTFunctions
###############################################################################
data_ishares_universe = function
(
...
)
{
	...
	data = extract_table_from_webpage(temp, 'Ticker', has.header=F)
...
# 
# [get industry/sector information from yahoo finance](https://ca.finance.yahoo.com/q/pr?s=RY.TO)
# data_yahoo_profile('RY.TO')
# 
#' @export 
###############################################################################
data_yahoo_profile = function(symbol) {	
	url = paste0('http://finance.yahoo.com/q/pr?s=', symbol)
	txt = get_url(url)
	
	temp = extract_table_from_webpage(txt, 'Sector', has.header = F)
		temp = gsub(':','',temp )
		out = temp[,2]
...
# 
# get_url('http://money.cnn.com/data/dow30/')
# 
#' @export 
###############################################################################
get_url = function
...
{
	warning('get_FOMC_dates is depreciated as of Apr 25, 2016 please use data_fomc function instead')
	data_fomc(force.download, data.filename)
}
...
{ 
  ...
  sb = string_buffer()
  for(year in 2009:(1 + date_year(Sys.Date()))) {
    temp = extract_table_from_webpage(txt, paste(year,'FOMC Meetings'))
    if(nrow(temp) == 0) next
  ...
  for(year in 1936:(first.year-1)) {
    ...
    for(token in tokens[-1])
    	add(sb, colnames(extract_table_from_webpage(token, 'year'))[1])
  }
...
#[680] "june 30-july 1 "  
	...
	parse.token = function(year, token) {
		...
		n = len(parts)
		if( n > 1 ) {
			month = ifna_prev(iif(nchar(parts) > 3,
...
edgar_info <- function(ticker)
{
  ...
  # extract table from this page
  temp = extract_table_from_webpage(txt, 'seriesDiv,Filings', has.header = T)
    out$fillings= clean_table(temp)
  
  temp = extract_token(txt, 'contentDiv,mailer,Mailing Address','</div>')
  out$mailing = t(clean_table(extract_table_from_webpage(temp, has.header=F, end.marker='</span>')))
    colnames(out$mailing) = 'Mailing Address'

  temp = extract_token(txt, 'contentDiv,mailer,Business Address','</div>')
  out$business = t(clean_table(extract_table_from_webpage(temp, has.header=F, end.marker='</span>')))
    colnames(out$business) = 'Business Address'

  temp = extract_token(txt, 'contentDiv,companyInfo,>','</div>')
  temp = gsub('\\|', '</span>', replace_token(temp, '<br','>','</span>'))
  temp = clean_table(extract_table_from_webpage(temp, has.header=F, end.marker='</span>'))
  out$company = t(temp)
...
#' @export
###############################################################################
zacks_info <- function(ticker = 'IBM')
{
  ...
  for(i in spl('earnings,webcasts,revisions,splits,dividends,guidance')) {  
  	data = extract_token(txt,paste0('<script>,window.app_data_', i, ',=,"data"'),'</script>')
...
###############################################################################
# Get quantumonline_info
# www.quantumonline.com/search.cfm?tickersymbol=458140100&sopt=cusip&1.0.1=Search    
# quantumonline_info(id = '458140100', type='cusip')
#' @export
###############################################################################
quantumonline_info <- function
(
    id,
    type=c(
...
)
{
    ...
    # extract table from this page
    out$main = extract_table_from_webpage(gsub('&nbsp;', ',', txt), "Company's Online Profile", has.header = F)    
    out$address = extract_table_from_webpage( txt, 'Address:', has.header = F)
...
#' 
#' hist = read_xts(hist_quotes_url('IBM', '1992-11-01', '2016-05-05', 'quotemedia'))
#' hist = read_xts(hist_quotes_url('HOU:CA', '1992-11-01', '2016-05-05', 'quotemedia'))
#' hist = read_xts(get_url(hist_quotes_url('HOD:CA', '1992-11-01', '2016-05-05', 'quotemedia')))
#'
#' library(readr)
#' hist = read_xts(read_csv(get_url(hist_quotes_url('HOU:CA', '1992-11-01', '2016-05-05', 'quotemedia')),,na=c('','NA','N/A')))
#'
...
)
{
	...
	switch(src[1],
		yahoo = paste('http://ichart.finance.yahoo.com/table.csv?',
         's=', ticker,  
         '&a=', sprintf('%.2d', date_month(from) - 1),
         format(from, '&b=%d&c=%Y'),
         '&d=', sprintf('%.2d', date_month(to) - 1),
         format(to, '&e=%d&f=%Y'),
		...
		quotemedia = paste('http://app.quotemedia.com/quotetools/getHistoryDownload.csv?webmasterId=501&',
         'symbol=', ticker,  
         '&startMonth=', sprintf('%.2d', date_month(from) - 1),
         format(from, '&startDay=%d&startYear=%Y'),
         '&endMonth=', sprintf('%.2d', date_month(to) - 1),
...
if(min.obs > 0) {  
  ...
  for(ticker in data$symbolnames)
    data[[ticker]] = data_clean_helper(data[[ticker]], ticker, min.ratio, iqr.mult)
}

data_clean_helper <- function
(
  data, 
  ticker,
  min.ratio = 2.5, 
...
) 
{
    ...
    if(len(index) > 0)
      for(i in index) {
        ...
        for(name in find_names('Open,Close,High,Low,Adjusted', data)) 
...
make_data_proxy <- function() {
    ...
    if(file.exists(filename)) {
    	temp = extract_table_from_webpage( join(readLines(filename)), 'EODValue' )
    	temp = join( apply(temp, 1, join, ','), '\n' )
    	raw.data$CRB = make_stock_xts( read_xts(temp, format='%m/%d/%y' ) )
    }
	...
	load(file=filename)
	TB3M[] = ifna_prev(TB3M)
	#compute_raw_annual_factor(TB3M)
	raw.data$TB3M = make_stock_xts(processTBill(TB3M, timetomaturity = 1/4, 261))
	...
	load(file=filename)
	TB3Y[] = ifna_prev(TB3Y)
	#compute_raw_annual_factor(TB3Y)
	raw.data$TB3Y = make_stock_xts(processTBill(TB3Y, timetomaturity = 3, 261))
	...
	load(file=filename)
	TB10Y[] = ifna_prev(TB10Y)
	#compute_raw_annual_factor(TB10Y)
	raw.data$TB10Y = make_stock_xts(processTBill(TB10Y, timetomaturity = 10, 261))
	...
	TB20Y[] = ifna_prev(TB20Y)
  
	#compute_raw_annual_factor(TB10Y)
	raw.data$TB20Y = make_stock_xts(processTBill(TB20Y, timetomaturity = 20, 12))

	#--------------------------------
	filename = 'data/GOLD.Rdata'
	if(!file.exists(filename)) {
		GOLD = bundes_bank_data_gold()
		save(GOLD, file=filename)
	}
	load(file=filename)
	raw.data$GOLD = make_stock_xts(GOLD)
	...
	NAREIT = make_xts(temp$Index, as.Date(temp$Date)) 

	raw.data$NAREIT = make_stock_xts(NAREIT)
...
'
	...
	data.proxy = env()
	getSymbols_extra(tickers, src = 'yahoo', from = '1970-01-01', env = data.proxy, raw.data = raw.data, auto.assign = T)
...
{
	warning('load_aqr_data is depreciated as of Apr 25, 2016 please use data_aqr function instead')
	data_aqr(data.set, frequency, sheet, force.download, last.col2extract)
}
...
)
{
	...
	make_xts(data[,-1], as.Date(data[,1]))	
}
...
#' @export 
###############################################################################
data_csi_security_master = function
(
...
###############################################################################
#' Get list of FX symbols_from FRED
...
fred_fx_symbol = function() {
	...
	temp = extract_table_from_webpage(temp, 'Country', has.header = F)
...
###############################################################################
#' Get list of FX symbols_from FXHISTORICALDATA.COM
#' [FXHISTORICALDATA.COM](http://www.fxhistoricaldata.com/)
#'
#' @examples
#' \dontrun{ 
#' info = fxhistoricaldata_fx_symbol()
#' info
#' }
#' @export
###############################################################################
fxhistoricaldata_fx_symbol = function() {
	...
	# extract info
	temp = extract_table_from_webpage(temp, 'EURUSD', has.header = F)
