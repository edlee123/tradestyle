...
{	
	...
	# Buy & Hold	
	data$weight[] = 0
	buy.hold = bt_run(data, trade.summary=T)
	
	#*****************************************************************
	# Create Report
	#****************************************************************** 	
	
	plotbt_custom_report_part1( buy.hold, trade.summary =T)
	plotbt_custom_report_part2( buy.hold, trade.summary =T)
	plotbt_custom_report_part3( buy.hold, trade.summary =T)
...
###############################################################################
bt_execution_price_test <- function() 
{	 
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)			
	bt_prep(data, align='keep.all', dates='1970::')	
	...
	data$weight[] = 0
		...
		data$weight[] = 1
	models$buy.hold = bt_run_share(data, clean.signal=T)
	...
	data$weight[] = NA
		data$execution.price[] = NA
		data$weight[] = signal
	models$ma.crossover = bt_run_share(data, clean.signal=T, trade.summary = TRUE)
	...
	data$weight[] = NA
		data$execution.price[] = NA
		data$weight[] = signal
	models$ma.crossover.com = bt_run_share(data, commission = 0.1, clean.signal=T)
	...
	models$ma.crossover.enter.next.open = bt_run_share(data, clean.signal=T, trade.summary = TRUE)
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
		...
		# Plot trades
		plotbt_custom_report_part3(models$ma.crossover, trade.summary = TRUE)		
		
dev.off()					
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		

		plotbt_custom_report_part3(models$ma.crossover.enter.next.open, trade.summary = TRUE)		
...
dev.off()			
	...
	models$test.com = bt_run_share(data, clean.signal=T, trade.summary=T)

	data$weight[] = NA
		data$execution.price[] = NA
		data$weight[201,] = 1
		data$weight[316,] = 0					
	models$test.com.new = bt_run_share(data, commission=commission, trade.summary=T, clean.signal=T)
	...
	#plotbt_custom_report_part1(models)
...
{	 
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)			
	bt_prep(data, align='keep.all', dates='2013:08::2013:09')	
	...
	data$weight[] = NA
		...
		commission = 0.0
	model = bt_run_share(data, commission = commission, capital = capital, silent = T)
...
#	
    ...
    commission = list(cps = 0.0, fixed = 0.0, percentage = 0/100)        
    models = create_strategies(obj, data, capital = capital, commission = commission )$models
    ...
    write_xts(cbind(ret, ret[,1] - ret[,2]), 'diff.csv')	
    ...
    # Create Report
    #******************************************************************    
	strategy_performance_snapshoot(models, T)    
}	
...
###############################################################################
bt_timelyportfolio_visualization_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)		
	bt_prep(data, align='keep.all', dates='2000::2011')
	...
	# Strategy
	ma10 = bt_apply_matrix(prices, EMA, 10)
	ma50 = bt_apply_matrix(prices, EMA, 50)
	ma200 = bt_apply_matrix(prices, EMA, 200)
	data$weight[] = NA;
		data$weight[] = iif(ma10 > ma50 & ma50 > ma200, 1, 
						iif(ma10 < ma50 & ma50 < ma200, -1, 0))
	strategy = bt_run_share(data, clean.signal=F)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plota_lines(buy.hold$eq, type='l', col=col)		
	
		plota_legend('strategy,Long,Short,Not Invested','black,green,red,gray')
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	layout(1:2)
	drawdowns = compute_drawdown(strategy$eq)
	highlight = drawdowns < -0.1
	
	plota_control$col.x.highlight = iif(drawdowns < -0.15, 'orange', iif(drawdowns < -0.1, 'yellow', 0))	
	
	plota(strategy$eq, type='l', plotX=F, x.highlight = highlight, ylim=range(buy.hold$eq,strategy$eq))
		plota_legend('strategy,10% Drawdown,15%  Drawdown','black,yellow,orange')
		
	plota(100*drawdowns, type='l', x.highlight = highlight)
		plota_legend('drawdown', 'black', x='bottomleft')
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plotbt_custom_report_part1(strategy, buy.hold, x.highlight = highlight)
...
###############################################################################
bt_improving_trend_following_test <- function() 
{	
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(trend.following.dv1, trend.following, buy.hold)
dev.off()	


png(filename = 'plot2.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(trend.following.dv1, trend.following, buy.hold)
dev.off()	
	...
	for(i in 1:len(ma.lens)) {
		ma.len = ma.lens[i]
		mas[, i] = bt_apply(data, function(x) { SMA(Cl(x), ma.len) } )
	}
	for(i in 1:len(dv.lens)) {
		dv.len = dv.lens[i]
		dvs[,i] = bt_apply(data, function(x) { DV(HLC(x), dv.len, TRUE) } )
	}
	...
	for(ima in 1:len(ma.lens)) {
		...
		for(idv in 0:len(dv.lens)) {			
			...
			strategy = bt_run(data, silent=T)			
			
			# add 1 to account for benchmark case, no counter-trend
			idv = idv + 1
			out$Cagr[ima, idv] = compute_cagr(strategy$equity)
			out$Sharpe[ima, idv] = compute_sharpe(strategy$ret)
			out$DVR[ima, idv] = compute_DVR(strategy)
			out$MaxDD[ima, idv] = compute_max_drawdown(strategy$equity)
		}
	}
...
png(filename = 'plot3.png', width = 800, height = 600, units = 'px', pointsize = 12, bg = 'white')										
	...
	for(i in names(out)) {
		...
		plot_table(temp, smain = i, highlight = T, colorbar = F)
	}
...
{	
	...
	data$weight[] = NA
		...
		data$weight$SPY[] = iif(roc252.2 > roc5.2 & roc252.1 < roc5.1 & roc252 < roc5, -1, data$weight$SPY)
	roc.cross = bt_run(data, trade.summary=T)					
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(roc.cross, buy.hold, trade.summary=T)
dev.off()	

png(filename = 'plot2.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(roc.cross, buy.hold, trade.summary=T)
dev.off()	

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part3(roc.cross, buy.hold, trade.summary=T)
dev.off()	
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	roc.cross.share = bt_run(data, type='share', trade.summary=T, capital=capital)					
...
png(filename = 'plot4.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(roc.cross.share, roc.cross, buy.hold, trade.summary=T)
dev.off()	

png(filename = 'plot5.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(roc.cross.share, roc.cross, buy.hold, trade.summary=T)
dev.off()	
...
###############################################################################
bt_rotational_trading_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	models$equal.weight = bt_run_share(data, clean.signal=F, dates=dates)
	...
	models$top2 = bt_run_share(data, trade.summary=T, dates=dates)

	# Seletop Top 2 funds,  and Keep then till they are in 1:6 rank
	data$weight[] = NA
		data$weight[month.ends,] = ntop_keep(position.score[month.ends,], 2, 6)	
	models$top2.keep6 = bt_run_share(data, trade.summary=T, dates=dates)
	...
	layout(1)
	barplot_with_labels(sapply(models, compute_turnover, data), 'Average Annual Portfolio Turnover')

		
	# put all reports into one pdf file
	pdf(file = 'report.pdf', width=8.5, height=11)
		plotbt_custom_report(models, trade.summary=T)
	dev.off()	
...
png(filename = 'plot2.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(models)
dev.off()	

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part3(models, trade.summary=T)
dev.off()	
...
bt_timing_model_test <- function() 
{
	...
	bt_prep(data, align='remove.na', dates='1970::2011')
	...
	# find month ends
	month.ends = date_month_ends(index(prices))
	...
	equal.weight = bt_run(data, type='share', capital=capital)
		
	# BuyRule, price > 10 month SMA
	sma = bt_apply_matrix(prices, SMA, 200)
	buy.rule = prices > sma
	...
	data$weight[] = NA		
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	timing = bt_run(data, type='share', trade.summary=T, capital=capital)
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	timing.d = bt_run(data, type='share', trade.summary=T, capital=capital)	
	...
	# put all reports into one pdf file
	pdf(file = 'report.pdf', width=8.5, height=11)
		plotbt_custom_report(timing, timing.d, equal.weight, trade.summary=T)
	dev.off()
	
	#*****************************************************************
	# Code Strategies : Daily with Counter-Trend Entries by david varadi
	# see bt_improving_trend_following_test
	#****************************************************************** 
	dv = bt_apply(data, function(x) { DV(HLC(x), 1, TRUE) } )	
	...
	data$weight[] = NA
		...
		data$weight = bt_apply_matrix(data$weight, ifna_prev)
		data$weight$SHY = 1 - rowSums(data$weight)
		
		data$weight = bt_exrem(data$weight)

		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	timing.d1 = bt_run(data, type='share', trade.summary=T, capital=capital)

	# compute_turnover	
	models = variable_number_arguments(timing.d1, timing.d, timing, equal.weight)
		sapply(models, compute_turnover, data)
	...
	plotbt_custom_report_part1(timing.d1, timing.d, timing, equal.weight)
...
{	
	...
	getSymbols(tickers, src = 'yahoo', from = '1995-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)	
	bt_prep(data, align='keep.all', dates='1995::')
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	meom.equal.weight = bt_run(data, type='share', capital=capital)
	...
	# BuyRule = C > WMA(C, 89)
	buy.rule = prices > bt_apply_matrix(prices, function(x) { WMA(x, 89) } )		
		buy.rule = ifna(buy.rule, F)
	...
	# Rank1 = MA( C/Ref(C,-2), 5 ) * MA( C/Ref(C,-2), 40 )
	position.score = bt_apply_matrix(ret2, SMA, 5) * bt_apply_matrix(ret2, SMA, 40)
	...
	data$weight[] = NA;
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	meom.top2.rank1 = bt_run(data, type='share', trade.summary=T, capital=capital)
	...
	# Rank2 = MA( C/Ref(C,-2), 5 ) * Ref( MA( C/Ref(C,-2), 10 ), -5 )
	position.score = bt_apply_matrix(ret2, SMA, 5) * mlag( bt_apply_matrix(ret2, SMA, 10), 5)
		position.score[!buy.rule] = NA
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	meom.top2.rank2 = bt_run(data, type='share', trade.summary=T, capital=capital)
	...
	# put all reports into one pdf file
	pdf(file = 'report.pdf', width=8.5, height=11)
		plotbt_custom_report(meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
	dev.off()	

	
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
dev.off()	

png(filename = 'plot2.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
dev.off()	

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part3(meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
dev.off()	
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	meom.top2.rank2.hold12 = bt_run(data, type='share', trade.summary=T, capital=capital)

png(filename = 'plot4.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(meom.top2.rank2.hold12, meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
dev.off()	

png(filename = 'plot5.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(meom.top2.rank2.hold12, meom.top2.rank2, meom.top2.rank1, meom.equal.weight, equal.weight, trade.summary=T)
dev.off()	
...
bt_intraday_test <- function() 
{
	...
	EURUSD = getSymbols_fxhistoricaldata('EURUSD', 'hour', auto.assign = F, download=F)
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plota2Y(y, ylim = range(OHLC(y), na.rm=T), las=1, col='red', col.axis = 'red')
		plota_ohlc(y, col=plota_candle_col(y))
	plota_legend('SPY(rhs),EURUSD(lhs)', 'black,red', list(SPY[dates],EURUSD[dates]))
...
dev.off()	
	...
	#****************************************************************** 
	data <- new.env()
	getSymbols_fxhistoricaldata(tickers, 'day', data, download=F)
	bt_prep(data, align='remove.na', dates='1990::')
	...
	data$weight[] = NA
		data$weight[] = ntop(prices, n)
	models$equal.weight = bt_run_share(data, clean.signal=F)
	
	# Timing by M. Faber
	sma = bt_apply_matrix(prices, SMA, 200)
	data$weight[] = NA
		data$weight[] = ntop(prices, n) * (prices > sma)
	models$timing = bt_run_share(data, clean.signal=F)
...
dev.off()	
	...
	#****************************************************************** 
	data <- new.env()	
	getSymbols_fxhistoricaldata(tickers, 'hour', data, download=F)	
	bt_prep(data, align='remove.na', dates='1990::')
...
###############################################################################
bt_min_var_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	load_packages('quantmod,quadprog,lpSolve')
	tickers = spl('SPY,QQQ,EEM,IWM,EFA,TLT,IYR,GLD')
	...
	bt_prep(data, align='remove.na', dates='1990::')
	bt_prep(data.weekly, align='remove.na', dates='1990::')
	...
	# Create Constraints
	#*****************************************************************
	constraints = new_constraints(n, lb = -Inf, ub = +Inf)
	...
	for( i in week.ends[week.ends >= (63 + 1)] ) {
		...
		weight[i,] = min_risk_portfolio(ia, constraints)
	}
	...
	data$weight[] = weight		
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	min.var.daily = bt_run(data, type='share', capital=capital)
	...
	for( i in week.ends[week.ends >= (63 + 1)] ) {	
		...
		weightw[i,] = min_risk_portfolio(ia, constraints)
	}	
		
	data$weight[] = weightw		
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	min.var.weekly = bt_run(data, type='share', capital=capital, trade.summary = T)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	plotbt_custom_report_part1(min.var.weekly, min.var.daily, equal.weight)
dev.off()	

png(filename = 'plot2.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(min.var.weekly, min.var.daily, equal.weight)
dev.off()	

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	layout(1:2)
	plotbt_transition_map(min.var.daily$weight)
		legend('topright', legend = 'min.var.daily', bty = 'n')
	plotbt_transition_map(min.var.weekly$weight)
...
bt_aa_test <- function() 
{
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)							
	bt_prep(data, align='remove.na', dates='1990::2011')
...
#			period.annual.factor = 12
	...
	#min.risk.fns = spl('min_risk_portfolio,min_maxloss_portfolio,min_mad_portfolio,min_cvar_portfolio,min_cdar_portfolio,min_cor_insteadof_cov_portfolio,min_mad_downside_portfolio,min_risk_downside_portfolio,min_avgcor_portfolio,find_erc_portfolio,min_gini_portfolio')	
	min.risk.fns = spl('min_risk_portfolio,min_maxloss_portfolio')
	
	# Gini risk measure optimization takes a while, uncomment below to add Gini risk measure
	# min.risk.fns = c(min.risk.fns, 'min_gini_portfolio')
...
#hist = hist[ , include.index]
		...
		# compute_risk contributions implied by portfolio weihgts
		for(f in names(weights)) {
			risk.contributions[[ f ]][j, include.index] = portfolio_risk_contribution(weights[[ f ]][j, include.index], ia)
		}
	...
	for(i in names(weights)) {
		...
		models[[i]] = bt_run_share(data, clean.signal = F)
	}
...
png(filename = 'plot5.png', width = 600, height = 1600, units = 'px', pointsize = 12, bg = 'white')	
	...
	for(m in names(risk.contributions)) {
		plotbt_transition_map(risk.contributions[[m]], name=paste('Risk Contributions',m))
			legend('topright', legend = m, bty = 'n')
	}
dev.off()	
...
png(filename = 'plot6.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	...
	layout(1:2)	
	plota_matplot(lapply(weights, portfolio_concentration_gini_coefficient), main='Gini Coefficient')
	plota_matplot(lapply(weights, portfolio_concentration_herfindahl_index), main='Herfindahl Index')
	#plota_matplot(lapply(weights, portfolio_turnover), main='Turnover')
dev.off()	


png(filename = 'plot7.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	# Compute stats
	out = compute_stats(weights,
		list(Gini=function(w) mean(portfolio_concentration_gini_coefficient(w), na.rm=T),
			Herfindahl=function(w) mean(portfolio_concentration_herfindahl_index(w), na.rm=T),
			Turnover=function(w) period.annual.factor * mean(portfolio_turnover(w), na.rm=T)
			)
		)
	
	out[] = plota_format(100 * out, 1, '', '%')
	plot_table(t(out))
dev.off()		
...
png(filename = 'plot8.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	# Plot Portfolio Turnover for each strategy
	layout(1)
	barplot_with_labels(sapply(weights, function(w) period.annual.factor * mean(portfolio_turnover(w), na.rm=T)), 'Average Annual Portfolio Turnover')
dev.off()	

}

bt_aa_test_new <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	obj = portfolio_allocation_helper(data$prices, 
		...
		min.risk.fns = list(
			...
			# cluster
			C.EW = distribute_weights(equal_weight_portfolio, cluster.group),
			...
			MMad = min_mad_portfolio,
			MCVaR = min_cvar_portfolio,
			MCDaR = min_cdar_portfolio,
			MMadDown = min_mad_downside_portfolio,
			MRiskDown = min_risk_downside_portfolio,
			MCorCov = min_cor_insteadof_cov_portfolio
		)
	)
	
	models = create_strategies(obj, data)$models
...
png(filename = 'plot1.png', width = 1800, height = 1800, units = 'px', pointsize = 12, bg = 'white')	
		strategy_performance_snapshoot(models, T, 'Backtesting Asset Allocation portfolios')
dev.off()
	...
	#	strategy_performance_snapshoot(models, title = 'Backtesting Asset Allocation portfolios', data = data)
	#dev.off()	
...
bt_rebalancing_test <- function() 
{
	...
	getSymbols(tickers, src = 'yahoo', from = '1900-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)
	bt_prep(data, align='remove.na', dates='1900::2011')
	...
	data$weight[] = NA	
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	buy.hold = bt_run(data, type='share', capital=capital)
	...
	for(period in spl('months,quarters,years')) {
		data$weight[] = NA	
			...
			capital = 100000
			data$weight[] = (capital / prices) * data$weight
		models[[period]] = bt_run(data, type='share', capital=capital)	
	}
	models$buy.hold = buy.hold				
	
	# Compute Portfolio Turnover 
	compute_turnover(models$years, data)		
	
	# Compute Portfolio Maximum Deviation
	compute_max_deviation(models$years, target.allocation)		
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	...
	layout(1:2)
	plotbt_transition_map(models$buy.hold$weight, 'buy.hold', spl('red,orange'))
		abline(h=50)
	plotbt_transition_map(models$months$weight, 'months', spl('red,orange'))
		abline(h=50)		
dev.off()
...
dev.off()	
	...
	models$smart5.all = bt_max_deviation_rebalancing(data, buy.hold, target.allocation, 5/100, 0) 
	
	# rebalance half-way to target.allocation when portfolio weights are 5% away from target.allocation
	models$smart5.half = bt_max_deviation_rebalancing(data, buy.hold, target.allocation, 5/100, 0.5) 
...
png(filename = 'plot4.png', width = 600, height = 800, units = 'px', pointsize = 12, bg = 'white')
	...
	plotbt_transition_map(models$buy.hold$weight, 'buy.hold', spl('red,orange'))
		abline(h=50)
	plotbt_transition_map(models$smart5.all$weight, 'Max Deviation 5%, All the way', spl('red,orange'))
		abline(h=50)
	plotbt_transition_map(models$smart5.half$weight, 'Max Deviation 5%, Half the way', spl('red,orange'))
		abline(h=50)
	plotbt_transition_map(models$years$weight, 'years', spl('red,orange'))
		abline(h=50)
dev.off()	
...
png(filename = 'plot5.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	# Plot Portfolio Turnover for each Rebalancing method
	layout(1:2)
	barplot_with_labels(sapply(models, compute_turnover, data), 'Average Annual Portfolio Turnover', F)
	barplot_with_labels(sapply(models, compute_max_deviation, target.allocation), 'Maximum Deviation from Target Mix')
dev.off()
...
dev.off()	
	...
	models = list()
	for(i in 1:12) {
		index = which( date_month(index(prices)[period.ends]) == i )
		data$weight[] = NA	
			data$weight[1,] = target.allocation			
			...
			capital = 100000
			data$weight[] = (capital / prices) * data$weight
		models[[ months[i] ]] = bt_run(data, type='share', capital=capital)	
	}
...
#' @export 	
bt_max_deviation_rebalancing <- function
(
	data,
	model, 
	target.allocation, 
...
) 
{
	...
	while(T) {	
		...
		weight = model$weight
		index = apply(abs(weight - rep_row(target.allocation, nperiods)), 1, max) > max.deviation
		...
		if( len(index) > 0 ) {
			...
			if( len(index) > 0 ) {
				...
				data$weight[] = NA	
					...
					temp = rep_row(target.allocation, sum(action.index))
					data$weight[action.index,] = temp + 
						rebalancing.ratio * (weight[action.index,] - temp)					
					
				# please note the bt_run_share_ex somehow gives slighly better results
				if(fast)
					model = bt_run_share_fast(data)
				else
					model = bt_run_share_ex(data, clean.signal=F, silent=T)
	...
	model = bt_run_share_ex(data, clean.signal=F, silent=F)
	return(model)
}
...
bt_rebalancing1_test <- function() 
{
	...
	models$smart3.all = bt_max_deviation_rebalancing(data, buy.hold, target.allocation, 3/100, 0) 
	
	# rebalance half-way to target.allocation when portfolio weights are 3% away from target.allocation
	models$smart3.half = bt_max_deviation_rebalancing(data, buy.hold, target.allocation, 3/100, 0.5) 
...
###############################################################################
bt_rotational_trading_trades_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)				
	top2.d = bt_run(data, type='share', trade.summary=T, capital=capital)
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.w = bt_run(data, type='share', trade.summary=T, capital=capital)
	
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	# Plot Strategy Metrics Side by Side
	plotbt_strategy_sidebyside(top2.d, top2.w, perfromance.fn = 'engineering_returns_kpi')	
dev.off()
	...
	data$weight[] = NA
		data$weight[] = ntop_keep(position.score, 2, 4)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.d.keep4 = bt_run(data, type='share', trade.summary=T, capital=capital)
	
	data$weight[] = NA
		data$weight[] = ntop_keep(position.score, 2, 6)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.d.keep6 = bt_run(data, type='share', trade.summary=T, capital=capital)

png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')				
	# Plot Strategy Metrics Side by Side
	plotbt_strategy_sidebyside(top2.d, top2.d.keep4, top2.d.keep6, perfromance.fn = 'engineering_returns_kpi')
dev.off()
	...
	for( avg in spl('SMA,EMA') ) {
		for( i in c(3,5,10,20) ) {		
			position.score.smooth = bt_apply_matrix(position.score.ma, avg, i)	
			...
			data$weight[] = NA
				...
				capital = 100000
				data$weight[] = (capital / prices) * bt_exrem(data$weight)		
			models[[ paste(avg,i) ]] = bt_run(data, type='share', trade.summary=T, capital=capital)		
		}
	}
		
png(filename = 'plot3.png', width = 1200, height = 600, units = 'px', pointsize = 12, bg = 'white')				
	# Plot Strategy Metrics Side by Side
	plotbt_strategy_sidebyside(models, perfromance.fn = 'engineering_returns_kpi')
dev.off()
	...
	position.score.smooth = bt_apply_matrix(position.score.ma, 'EMA', 10)	
		position.score.smooth[!buy.rule,] = NA
	data$weight[] = NA
		data$weight[] = ntop_keep(position.score.smooth, 2, 6)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.d.keep6.EMA10 = bt_run(data, type='share', trade.summary=T, capital=capital)
		
	# Select Top 2 funds weekly, Keep till they are 6 rank
	data$weight[] = NA
		data$weight[week.ends,] = ntop_keep(position.score[week.ends,], 2, 6)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.w.keep6 = bt_run(data, type='share', trade.summary=T, capital=capital)
	
	# Select Top 2 funds weekly, Keep till they are 6 rank, Smooth Rank by 10 week EMA
	position.score.smooth[] = NA
		position.score.smooth[week.ends,] = bt_apply_matrix(position.score.ma[week.ends,], 'EMA', 10)	
			position.score.smooth[!buy.rule,] = NA
	
	data$weight[] = NA
		data$weight[week.ends,] = ntop_keep(position.score.smooth[week.ends,], 2, 6)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)		
	top2.w.keep6.EMA10 = bt_run(data, type='share', trade.summary=T, capital=capital)
	
		
png(filename = 'plot4.png', width = 800, height = 600, units = 'px', pointsize = 12, bg = 'white')				
	# Plot Strategy Metrics Side by Side
	plotbt_strategy_sidebyside(top2.d, top2.d.keep6, top2.d.keep6.EMA10, top2.w, top2.w.keep6, top2.w.keep6.EMA10, perfromance.fn = 'engineering_returns_kpi')
dev.off()
	...
	position.score = bt_apply(data, function(x) TSI(HLC(x)) )		
		position.score.ma = position.score
		position.score[!buy.rule,] = NA
...
###############################################################################
bt_december_trading_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	dates = index(prices)
	years = date_year(dates)	
	index = which(date_month(dates) == 12)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	...
	grid()
	plota_legend('Avg SPY,SPY Dec 2011', 1:2)
dev.off()	
	...
	index = which(date_month(dates) == 11)
	...
	data$weight[] = NA	
		...
		capital = 100000
		data$weight[] = (capital / prices) * data$weight
	december = bt_run(data, type='share', capital=capital, trade.summary=T)
	
	#*****************************************************************
	# Create Report
	#****************************************************************** 
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	plotbt_custom_report_part1(december, buy.hold, trade.summary=T)	
dev.off()	

png(filename = 'plot3.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')	
	plotbt_custom_report_part2(december, buy.hold, trade.summary=T)	
dev.off()	
...
{	
	...
	# keep only January	
	ret = ret[date_month(index(ret)) == 1, ]
...
png(filename = 'plot1.png', width = 600, height = 200, units = 'px', pointsize = 12, bg = 'white')										
	plot_table(stats[, order(stats['Positive',], decreasing =T)[1:10]])
dev.off()	
...
###############################################################################
bt_test_garch_speed <- function() 
{
	load_packages('tseries,fGarch,rbenchmark')	
...
###############################################################################
garch_predict_one_day <- function(fit, r.last) 
{
...
garchFit_predict_one_day <- function(fit, r.last) 
{
...
###############################################################################
bt_forecast_garch_volatility <- function(ret.log, est.period = 252) 
{		
	...
	for( i in (est.period + 1) : nperiods ) {
		...
		if( !is.logical( fit ) ) {
			...
			garch.vol[i] = garch_predict_one_day(fit, r.last)
		} else {
			...
			if( !is.logical( fit ) ) {
				...
				garch.vol[i] = garchFit_predict_one_day(fit, r.last)
			} 
		}			
		if( i %% 100 == 0) cat(i, '\n')
	}
	garch.vol[] = ifna_prev(coredata(garch.vol))
	return(garch.vol)
}	
...
bt_volatility_garch <- function() 
{
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)		
	bt_prep(data, align='remove.na', dates='2000::2012')
	...
	rsi2 = bt_apply_matrix(prices, RSI, 2)
	data$weight[] = NA
		data$weight[] = iif(rsi2 < 50, 1, -1)	
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	mr = bt_run(data, type='share', capital=capital, trade.summary=T)
				
		
	# Trend Following(TF) strategy - MA 50/200 crossover
	sma.short = bt_apply_matrix(prices, SMA, 50)
	sma.long = bt_apply_matrix(prices, SMA, 200)
	data$weight[] = NA
		data$weight[] = iif(sma.short > sma.long, 1, -1)
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	tf = bt_run(data, type='share', capital=capital, trade.summary=T)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plotbt_custom_report_part1(mr, tf, buy.hold, trade.summary=T)
...
dev.off()		
	...
	ret.log = bt_apply_matrix(prices, ROC, type='continuous')
	hist.vol = bt_apply_matrix(ret.log, runSD, n = 21)
	vol.rank = percent_rank(SMA(percent_rank(hist.vol, 252), 21), 250)
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	regime.switching = bt_run(data, type='share', capital=capital, trade.summary=T)
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plotbt_custom_report_part1(regime.switching, mr, tf, buy.hold, trade.summary=T)
...
dev.off()		
	...
	#****************************************************************** 		
	load_packages('tseries,fGarch')	
	garch.vol = bt_forecast_garch_volatility(ret.log, 252)	
	vol.rank = percent_rank(SMA(percent_rank(garch.vol, 252), 21), 250)
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	regime.switching.garch = bt_run(data, type='share', capital=capital, trade.summary=T)
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plotbt_custom_report_part1(regime.switching.garch, regime.switching, buy.hold, trade.summary=T)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	obj = bt_matching_find(Cl(data), plot=T)
dev.off()

png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	matches = bt_matching_overlay(obj, plot=T)
dev.off()

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	bt_matching_overlay_table(obj, matches, plot=T)
dev.off()	
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	plota(data, type='l', col='gray', main=tickers)
		plota_lines(last(data,90), col='blue')
		for(i in 1:n.match) {
		plota_lines(data[(min.index[i]-n.query + 1):min.index[i]], col='red')
		}
		text(index_xts(data)[min.index - n.query/2], reference[min.index - n.query/2], 1:n.match, 
			adj=c(1,-1), col='black',xpd=TRUE)
		plota_legend('Pattern,Match Number','blue,red')
dev.off()			
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	...
	bt_plot_dot_label <- function(x, data, xfun, col='red') {
		for(j in 1:len(xfun)) {
	...
	bt_plot_dot_label(2*n.query, temp[2*n.query,1:n.match], 
		list(Min=min,Max=max,Median=median,'Bot 25%'=function(x) quantile(x,0.25),'Top 75%'=function(x) quantile(x,0.75)))
	bt_plot_dot_label(n.query, temp[n.query,(n.match+1)], list(Current=min))
dev.off()	
	...
	# format
	temp[] = plota_format(100*(temp-1),1,'','%')
	...
	# plot_table
png(filename = 'plot4.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	plot_table(temp, smain='Match Number')
dev.off()		
...
###############################################################################
bt_matching_backtest_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	start.index = which(date_year(index(hist[month.ends])) == 1994)[1]
	weight = hist * NA
	
	for( i in start.index : len(month.ends) ) {
		#obj = bt_matching_find(hist[1:month.ends[i],], n.match=10, normalize.fn = normalize_mean, plot=T)
		#matches = bt_matching_overlay(obj, future=hist[(month.ends[i]+1):(month.ends[i]+22),], plot=T)
		#bt_matching_overlay_table(obj, matches, weights=NA, plot=T)

		obj = bt_matching_find(hist[1:month.ends[i],], normalize.fn = normalize_first)
		matches = bt_matching_overlay(obj)
	...
	data$weight[] = NA
		...
		capital = 100000
		data$weight[] = (capital / prices) * bt_exrem(data$weight)
	test = bt_run(data, type='share', capital=capital, trade.summary=T)
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
	...
	plotbt_custom_report_part1(test, buy.hold, trade.summary=T)
...
normalize_mean_sd <- function(x) { (x - mean(x)) / sd(x) }
normalize_first <- function(x) { x/as.double(x[1]) }

###############################################################################
# functions to compute distance
###############################################################################
dist_euclidean <- function(x) { stats:::dist(x) }

###############################################################################
# Find historical matches similar to the given query(pattern)
###############################################################################
bt_matching_find <- function
(
	data,				# time series
	...
	normalize.fn = normalize_mean_sd, 	# function to normalize data
	dist.fn = dist_euclidean,	# function to compute distance
...
)
{
	...
	if(plot) {	
		...
		plota(data, type='l', col='gray', LeftMargin = 1,
			...
			text(index_xts(data)[min.index - n.query/2], reference[min.index - n.query/2], 1:n.match, 
				adj=c(1,-1), col='black',xpd=TRUE)
			plota_legend(paste('Pattern: ', main, ',Match Number'),'blue,red')	
	}
...
bt_matching_overlay <- function
(
	obj, 				# object from bt_matching_find function
...
)
{
	...
	if(plot) {		
		...
		bt_plot_dot_label(n, temp[n, 1:n.match], 
			list(Min=min,Max=max,Median=median,'Bot 25%'=function(x) quantile(x,0.25),'Top 75%'=function(x) quantile(x,0.75)))
		bt_plot_dot_label(n.query, temp[n.query,(n.match+1)], list(Current=min))	
	}
...
###############################################################################
bt_matching_overlay_table <- function
(
	obj, 				# object from bt_matching_find function
	matches, 			# matches from bt_matching_overlay function
...
###############################################################################
#dist_euclidean <- function(x) { stats:::dist(x) }
dist_MOdist <- function(x) { MOdist(t(x)) }
dist_DTW <- function(x) { dtw(x[1,], x[2,])$distance }


bt_matching_dtw_test <- function() 
{
	#*****************************************************************
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	obj = bt_matching_find(Cl(data), normalize.fn = normalize_mean, dist.fn = 'dist_euclidean', plot=T)
dev.off()

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	matches = bt_matching_overlay(obj, plot.index=1:90, plot=T)
dev.off()

png(filename = 'plot4.png', width = 600, height = 800, units = 'px', pointsize = 12, bg = 'white')
	layout(1:2)
	matches = bt_matching_overlay(obj, plot=T, layout=T)
	bt_matching_overlay_table(obj, matches, plot=T, layout=T)
dev.off()
...
png(filename = 'plot5.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	obj = bt_matching_find(Cl(data), normalize.fn = normalize_mean, dist.fn = 'dist_DTW', plot=T)
dev.off()
...
png(filename = 'plot8.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	obj = bt_matching_find(Cl(data), normalize.fn = normalize_mean, dist.fn = 'dist_DTW1', plot=T)
dev.off()
...
png(filename = 'plot11.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')
	obj = bt_matching_find(Cl(data), normalize.fn = normalize_mean, dist.fn = 'dist_DDTW', plot=T)
dev.off()
...
dist_DDTW <- function(x) { 
	y = x
	x[1,] = derivative_est(x[1,])
	x[2,] = derivative_est(x[2,])
...
dev.off()
	...
	alignment0 = dtw(derivative_est(query), derivative_est(reference), keep=TRUE)
...
###############################################################################
bt_position_sizing_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	#****************************************************************** 
	atr = bt_apply(data, function(x) ATR(HLC(x),20)[,'atr'])
	...
	models$buy.hold.2atr = bt_run(data, type='share', capital=capital)					
...
###############################################################################
bt_volatility_position_sizing_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)		
	bt_prep(data, align='keep.all', dates='1994::')
	...
	#****************************************************************** 
	ret.log = bt_apply_matrix(prices, ROC, type='continuous')
	hist.vol = sqrt(252) * bt_apply_matrix(ret.log, runSD, n = 60)

	data$weight[] = 0.1 / hist.vol
	models$buy.hold.volatility.weighted = bt_run_share(data, clean.signal=T)
	...
	models$buy.hold.volatility.weighted.100 = bt_run_share(data, clean.signal=T)
	...
	models$buy.hold.volatility.weighted.100.monthly = bt_run_share(data, clean.signal=T)
...
png(filename = 'plot2.png', width = 1600, height = 1000, units = 'px', pointsize = 12, bg = 'white')		
	plotbt_custom_report_part2(rev(models))
dev.off()	
	
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	# Plot Portfolio Turnover for each strategy
	layout(1)
	barplot_with_labels(sapply(models, compute_turnover, data), 'Average Annual Portfolio Turnover', plotX = F, label='both')
dev.off()	
	...
	for(icalc in calc) {
		...
		data$weight[period.ends,] = 0.1 / vol[period.ends,]
			rs = rowSums(data$weight[period.ends,])
			data$weight[period.ends,] = data$weight[period.ends,] / iif(rs > 1, rs, 1) 			
		models[[icalc]] = bt_run_share(data, clean.signal=T)
	}
...
dev.off()	
	...
	data$weight[] = weight
	models$ma.crossover = bt_run_share(data, clean.signal=T)
		
	#*****************************************************************
	# Target 10% Volatility
	#****************************************************************** 
	ret.log = bt_apply_matrix(models$ma.crossover$equity, ROC, type='continuous')
	hist.vol = sqrt(252) * bt_apply_matrix(ret.log, runSD, n = 60)
	...
	models$ma.crossover.volatility.weighted.100.monthly = bt_run_share(data, clean.signal=T)
...
dev.off()	
	...
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)		
	bt_prep(data, align='remove.na', dates='1994::')
	...
	#****************************************************************** 
	ret.log = bt_apply_matrix(models$timing$equity, ROC, type='continuous')
	hist.vol = bt_apply_matrix(ret.log, runSD, n = 60)
	...
	data$weight[] = NA
		...
		data$weight[] = data$weight / iif(rs > 1, rs, 1) 				
		data$weight[1:200,] = NA
	models$timing.volatility.weighted.100.monthly = bt_run_share(data, clean.signal=T)
...
png(filename = 'plot.sp500.cor.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										
 	...
 	cols = col_add_alpha(spl('green,red'),50)
	plota_control$col.x.highlight = iif(sma50 > sma200, cols[1], cols[2])
	...
	plota(avg.cor, type='l', ylim=range(avg.cor, avg.cor.spy, na.rm=T), x.highlight = highlight,
		...
		plota_lines(avg.cor.spy, type='l', col='blue')
		plota_legend('Pairwise Correlation,Correlation with SPY,SPY 50-day SMA > 200-day SMA,SPY 50-day SMA < 200-day SMA', 
		c('black,blue',cols))
...
###############################################################################
bt_volatility_quantiles_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data <- new.env()
	getSymbols(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
		#save(data, file='data.sp500_components.Rdata') 
		#load(file='data.sp500_components.Rdata') 	
	...
	bt_prep(data.spy, align='keep.all', dates='1994::')
	...
	data.spy$weight[] = NA
		data.spy$weight[] = 1
	models$spy = bt_run(data.spy)
	
	# Equal Weight
	data$weight[] = NA
		data$weight[] = ntop(prices, 500)
	models$equal.weight = bt_run(data)
...
#	period.ends = endpoints(prices, 'months')
	...
	p = bt_apply_matrix(coredata(prices), ifna_prev)	
	ret = p / mlag(p) - 1		
	sd252 = bt_apply_matrix(ret, runSD, 252)		
	...
	for( i in 1:n.quantiles) {
		...
		data$weight[] = NA
			data$weight[] = temp
		models[[ paste('Q',i,sep='_') ]] = bt_run(data, silent = T)
	}
	...
	#write_xts(make_xts(out, index(prices)), 'report.csv')	
}
...
###############################################################################
bt_fa_value_quantiles_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	factors = get_fama_french_data('F-F_Research_Data_Factors', periodicity = periodicity,download = F, clean = F)
	...
	bt_prep(data.fa, align='remove.na')
	...
	for(i in tickers) {
		...
		obj = factor_rolling_regression(data.fa, i, 36, silent=T)
...
############################################################################### 
factor_rolling_regression <- function(
	data, 
...
) 
{
	...
	# fl.all is the full sample estimates. 		
	# Facto Loadings - fl
	fl.all = list()
...
#' uses some quantmod function for has.Cl
#' @import quantmod 
factor_rolling_regression_detail_plot <- function(obj) {
  ...
  polyprep_xts <- function(ubnd_xts, lbnd_xts, xlabel = "X", ylabel = "Y") {
    ...
    plot(as.Date(index(ubnd_xts)), coredata(ubnd_xts), type = "l", col = "red", 
  ...
  for (i in 1:n) {
    ...
    lines(index(obj$fl$estimate[, i]), obj$fl$estimate[, i])
    abline(h = 0, col = "blue", lty = "dashed")
    
    # Plot Histogram of Rolling Estimates
    hist(obj$fl$estimate[, i], col = "red", border = "gray", las = 1, xlab = "", 
         ylab = "", main = yaxes)
...
factor_rolling_regression_style_plot <- function(obj, 
...
factor_rolling_regression_bt_plot <- function(obj) {
	# setup
	...
	models[[ticker]] = bt_run_share(data, clean.signal = F)

	data$weight[] = NA
		data$weight[,3:(n+2)] = t(repmat(obj$fl.all$estimate[1:n], 1, nperiods))
		data$weight$RF = 1
		data$weight[1:obj$window.len,] = NA
	models$all.alpha = bt_run_share(data, clean.signal = F)
	...
	data$weight[] = NA
		...
		data$weight[1:obj$window.len,] = NA
	models$all = bt_run_share(data, clean.signal = F)

	data$weight[] = NA
		data$weight[,3:(n+2)] = obj$fl$estimate[,1:n]
		data$weight$RF = 1
		data$weight[1:obj$window.len,] = NA
	models$est.alpha = bt_run_share(data, clean.signal = F)
	...
	data$weight[] = NA
		...
		data$weight$alpha = NA	
		data$weight[1:obj$window.len,] = NA
	models$est = bt_run_share(data, clean.signal = F)
...
three_factor_rolling_regression <- function() {
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	load_packages('quantmod')	
	...
	for(i in ls(data)) {
		...
		if(periodicity == 'months') {
			...
			data[[i]] = make_xts(coredata(temp[period.ends,]), monthly.dates)
		} else
	...
	# Fama/French factors
	#****************************************************************** 
	factors = get_fama_french_data('F-F_Research_Data_Factors', periodicity = periodicity,download = T, clean = F)
	...
	# Facto Loadings Regression
	#****************************************************************** 
	obj = factor_rolling_regression(data, tickers, 36)

	#*****************************************************************
	# Reports
	#****************************************************************** 
png(filename = 'plot1.png', width = 600, height = 1200, units = 'px', pointsize = 12, bg = 'white')		
	factor_rolling_regression_detail_plot(obj)
dev.off()

png(filename = 'plot2.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')			
	factor_rolling_regression_style_plot(obj)
dev.off()	

png(filename = 'plot3.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')				
	factor_rolling_regression_bt_plot(obj)
dev.off()	
	...
	factors.extra = get_fama_french_data('F-F_Momentum_Factor', periodicity = periodicity,download = T, clean = F)	
...
png(filename = 'plot6.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')			
	factor_rolling_regression_style_plot(obj, xfactor='HML', yfactor='Mom')
dev.off()	
...
your_own_factor_rolling_regression <- function() {
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	load_packages('quantmod')	
	...
	factors.extra = 100 * read_xts('EEM_SPY.csv')
...
png(filename = 'plot4.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')			
	factor_rolling_regression_style_plot(obj, xfactor='HML', yfactor='EEM_SPY')
dev.off()	
...
bt_one_month_test <- function() 
{
	...
	# Load historical data
	#****************************************************************** 
#save(data, data.spy, tickers, file='data.sp500_components.Rdata') 	
#load(file='data.sp500_components.Rdata') 	
	...
	models = c(models, 
		bt_make_quintiles(one.month, data, period.ends, start.t=1 + n.skip, prefix='M1_'))
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')					
	plotbt_custom_report_part1(models[spl('spy,equal.weight,spread')])
dev.off()		
...
###############################################################################
# compute_various additional stats
factor_rolling_regression_custom_stats <- function(x,y,fit) {
	n = len(y)
...
) 
{
	...
	for( i in 1:n.quantiles) {
		...
		models[[ paste(prefix,'Q',i,sep='') ]] = bt_run(data, silent = T)
	}
	...
	data$weight[] = NA
		data$weight[period.ends,] = temp
	models$spread = bt_run(data, silent = T)	
...
bt_fa_one_month_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	load_packages('quantmod')	
	info = sp500_components()
	...
	# setdiff(index(data.spy$prices), index(data$prices))
	# setdiff(index(data$prices),index(data.spy$prices))
#save(data, data.spy, tickers, sector, file='data.sp500_components.Rdata') 	
#load(file='data.sp500_components.Rdata') 	
	...
	for(i in tickers) {
		...
		# Facto Loadings Regression
		obj = factor_rolling_regression(data.fa, i, 36, silent=T,
			factor_rolling_regression_custom_stats)
	...
	#save(factors, file='data_ff.factors.Rdata') 	
	load(file='data_ff.factors.Rdata') 	
	...
	for(name in names(factors)) {
		cat(name, '\n')
		quantiles[[name]] = bt_make_quintiles(factors[[name]], data, index, start.t =  1+36, prefix=paste(name,'_',sep=''))
	}
	...
	png(filename = 'plot1.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(quantiles$one.month$spread,quantiles$last.e$spread,quantiles$last.e_s$spread)
	dev.off()		
	
	png(filename = 'plot2.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_strategy_sidebyside(quantiles$one.month$spread,quantiles$last.e$spread,quantiles$last.e_s$spread)
	dev.off()	


	png(filename = 'plot3.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(	quantiles$last.e )
	dev.off()		
		
	png(filename = 'plot4.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(	quantiles$last.e_s )
	dev.off()		
...
bt_fa_sector_one_month_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	for(name in names(factors)) {
		cat(name, '\n')
		quantiles.sn[[name]] = bt_make_quintiles_sector(sector, factors[[name]], data, index, start.t =  1+36, prefix=paste(name,'_',sep=''))
	}
	...
	png(filename = 'plot1.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(quantiles$one.month$spread,
	...
	png(filename = 'plot2.png', width = 800, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_strategy_sidebyside(quantiles$one.month$spread,
			quantiles$last.e$spread, quantiles$last.e_s$spread,
			quantiles.sn$one.month$spread.sn,
			quantiles.sn$last.e$spread.sn, quantiles.sn$last.e_s$spread.sn)	
	dev.off()	
	...
	png(filename = 'plot3.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(	quantiles.sn$one.month )
	dev.off()				
	
	png(filename = 'plot4.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		
		plotbt_custom_report_part1(	quantiles.sn$last.e_s )
	dev.off()		
	
	
	
	#*****************************************************************
	# Create Report - bt_one_month_test
	#****************************************************************** 	
	png(filename = 'plot1a.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')					
		plotbt_custom_report_part1(c(models,quantiles$one.month))
	dev.off()	

	png(filename = 'plot2a.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')					
		plotbt_custom_report_part1(c(models,quantiles$one.month$spread))
	dev.off()	

	#*****************************************************************
	# Create Report - bt_fa_one_month_test
	#****************************************************************** 						
...
bt_make_quintiles_sector <- function(
	sector,			# sector data
	position.score,	# position.score is a factor to form Quintiles sampled at the period.ends
	data,			# back-test object
	period.ends,	
...
) 
{
	...
	for( s in 1:n.sectors) {
		...
		models[[ paste(prefix,'spread.',sector.names[s], sep='') ]]	= bt_run(data, silent = T)	
	}
...
if(F) {	
	...
	model.prices = abind(lapply(models, function(m) m$equity), along=2)	
		#model.prices = make_xts(model.prices, index(data$prices)
		model.prices = model.prices[period.ends,]
		model.returns = model.prices / mlag(model.prices)-1
		model.score = bt_apply_matrix(model.returns, SMA, 6) 
		model.vol = bt_apply_matrix(model.returns, runSD, 6) 
	...
	data$weight[] = NA
		data$weight[period.ends,] = long - short
	models$spread.sn.top3 = bt_run(data, silent = T)	
...
#plotbt_strategy_sidebyside(models)
	...
	data$weight[] = NA
		data$weight[period.ends,] = long - short
	models$spread.sn = bt_run(data, silent = T)	
...
forecast_helper <- function(fit, h=10, level = c(80,95)) {
...
forecast2xts <- function(data, forecast) {
	...
	rm.index = date_dayofweek(new.dates) == 6 | date_dayofweek(new.dates) == 0
 	new.dates = new.dates[!rm.index]
 		
 	new.dates = new.dates[1:h] 	
 	return(make_xts(forecast, new.dates))
}

# create forecast_plot
forecast_plot <- function(data, forecast, ...) {
	...
	for(i in n : 1) {
		...
		border=NA, col=col_add_alpha(i+2,150))
	}
	
	plota_lines(out[,1], col='red')
	
	labels = c('Data,Forecast', paste(gsub('Lo.', '', colnames(out)[2*(1:n)]), '%', sep=''))
	plota_legend(labels, fill = c('black,red',col_add_alpha((1:n)+2, 150)))			
}


bt_forecast_dashboard <- function() {
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	load_packages('quantmod')	
	...
	# Create models
	#****************************************************************** 
	load_packages('forecast,fGarch,fArma')
	...
	png(filename = 'plot1.png', width = 800, height = 800, units = 'px', pointsize = 12, bg = 'white')		
		...
		for(i in 1:len(models)) {
			out = forecast_helper(models[[i]], 30, level = c(80,95))	 	
			forecast_plot(sample, out, main = names(models)[i]) 	
		}	
	dev.off()		
	...
	png(filename = 'plot2.png', width = 800, height = 800, units = 'px', pointsize = 12, bg = 'white')		
		layout(matrix(1:9,nr=3))
		for(i in 1:len(models)) {
			out = forecast_helper(models[[i]], 30, level = c(75,85,95,97,99))	 	
			forecast_plot(sample, out, main = names(models)[i]) 	
		}	
	dev.off()		
...
bt_new_60_40_test <- function() 
{
	...
	bt_prep(data.all, align='remove.na')
...
dev.off()		
	...
	models$dollar.w.60.40 = bt_run_share(data, clean.signal=F)
	...
	#****************************************************************** 				
	ret.log = bt_apply_matrix(prices, ROC, type='continuous')
	hist.vol = sqrt(252) * bt_apply_matrix(ret.log, runSD, n = 21)	
	...
	models$risk.w.60.40 = bt_run_share(data, clean.signal=F)
				
	#*****************************************************************
	# Scale Risk Weighted 40% Bonds & 60% Stock strategy to have 6% volatility
	#****************************************************************** 				
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$risk.w.60.40,
						weight.risk, 6/100, 21, 100/100)[period.ends,]
	models$risk.w.60.40.target6 = bt_run_share(data, clean.signal=T)
	
	#*****************************************************************
	# Same, plus invest cash into SHY
	#****************************************************************** 					
	weight = target_vol_strategy(models$risk.w.60.40,
	...
	models$risk.w.60.40.target6.cash = bt_run_share(data.all, clean.signal=T)
...
png(filename = 'plot4.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')			
	plotbt_custom_report_part2(models$risk.w.60.40.target6)		
dev.off()		
	
png(filename = 'plot5.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')			
	plotbt_custom_report_part2(models$risk.w.60.40.target6.cash)		
dev.off()		
...
) 
{
    ...
    ret.log = bt_apply_matrix(prices, ROC, type='continuous')
    hist.vol = bt_apply_matrix(ret.log, runSD, n = n.vol)   
    	adj.vol = 1/hist.vol[period.ends,]
    
    momentum = prices / mlag(prices, n.mom)
    
    weight = ntop_keep(momentum[period.ends,], n.top, n.top.keep) * adj.vol
		n.skip = max(n.mom, n.vol)
   
    data$weight[] = NA
        data$weight[period.ends,] = weight / rowSums(weight, na.rm=T)   
        data$weight[1 : n.skip,] = NA 
    bt_run_share(data, clean.signal=F, silent=T)
}

#' @export 
bt_aaa_minrisk <- function
(
	data,
	period.ends,
	n.top = 5,		# number of momentum positions
...
) 
{
    ...
    for( i in period.ends[period.ends >= n.skip] ) {
		...
		if(n > 0) {					
	        ...
	        weight[i,] = 0        
	        weight[i,index] = min_risk_portfolio(ia, constraints)
        }
    }
...
# Sensitivity Analysis based on the bt_improving_trend_following_test()
bt_aaa_sensitivity_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)							
	bt_prep(data, align='keep.all', dates='2004:12::')
	...
	models$combo = bt_aaa_combo(data, period.ends, n.top = 5,
					n.mom = 180, n.vol = 20)
					
					
	models$aaa = bt_aaa_minrisk(data, period.ends, n.top = 5,
					n.mom = 180, n.vol = 20)
	...
	#*****************************************************************
	# Sensitivity Analysis: bt_aaa_combo / bt_aaa_minrisk
	#****************************************************************** 
	...
	for(n.mom in mom.lens) {
		...
		for(n.vol in vol.lens) {
			...
			models[[ paste('M', n.mom, 'V', n.vol) ]] = 
				bt_aaa_combo(data, period.ends, n.top = 5,
					n.mom = n.mom, n.vol = n.vol)
		}
	}
	
	out = plotbt_strategy_sidebyside(models, return.table=T, make.plot = F)
...
png(filename = 'plot1.png', width = 1000, height = 1000, units = 'px', pointsize = 12, bg = 'white')										
	...
	for(i in names) {
		...
		plot_table(dummy, smain = i, highlight = T, colorbar = F)
...
dev.off()	
	...
	for(n.mom in mom.lens) {
		...
		for(n.vol in vol.lens) {
			...
			models[[ paste('M', n.mom, 'V', n.vol) ]] = 
				bt_aaa_minrisk(data, period.ends, n.top = 5,
					n.mom = n.mom, n.vol = n.vol)
		}
	}
...
bt_aaa_test <- function() 
{
	...
	bt_prep(data.price, align='keep.all', dates='2004:12::')	
    ...
    if(F) {
		...
		plota(y, type='l', ylim=range(y, y.price, na.rm=T))
			plota_lines(y.price, col='red')
		plota_legend('Total,Price', 'black,red')
	}			
    ...
    #******************************************************************
    ret.log = bt_apply_matrix(prices4vol, ROC, type='continuous')
    hist.vol = bt_apply_matrix(ret.log, runSD, n = n.vol)
    ...
    models_volatility.weighted = bt_run_share(data, clean.signal=F)
    ...
    models$momentum = bt_run_share(data, clean.signal=F)
    ...
    data$weight[] = NA
        data$weight[period.ends,] = weight / rowSums(weight, na.rm=T)   
    models$combo = bt_run_share(data, clean.signal=F,trade.summary = TRUE)
    ...
    data$weight[] = NA
        data$weight[period.ends,] = weight[period.ends,]   
    models$aaa = bt_run_share(data, clean.signal=F,trade.summary = TRUE)
...
png(filename = 'plot3.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		    
	plotbt_custom_report_part3(models$combo, trade.summary = TRUE)       
dev.off()		
       
png(filename = 'plot4.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		    
    plotbt_custom_report_part3(models$aaa, trade.summary = TRUE)       
dev.off()               
...
n.top = 4
	...
	models$combo = bt_aaa_combo(data, period.ends, n.top = n.top,
					n.mom = n.mom, n.vol = n.vol)
					
	# bt_aaa_minrisk is equivalent to MV=min_var_portfolio below
	models$aaa = bt_aaa_minrisk(data, period.ends, n.top = n.top,
					n.mom = n.mom, n.vol = n.vol)
			
					
	obj = portfolio_allocation_helper(data$prices, period.ends=period.ends,
		...
		min.risk.fns = list(EW=equal_weight_portfolio,
						...
						MC2=min_corr2_portfolio,
						MCE=min_corr_excel_portfolio,
						RSO.2 = rso_portfolio(equal_weight_portfolio, 2, 100), 
						MS=max_sharpe_portfolio())
	) 
	
	#models = c(models, create_strategies(obj, data)$models)
	models = create_strategies(obj, data)$models
...
png(filename = 'plot3.png', width = 1200, height = 800, units = 'px', pointsize = 12, bg = 'white')		
	plotbt_custom_report_part2(models$MS)
dev.off()	
...
#*****************************************************************
bt_rso_portfolio_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)							
	bt_prep(data, align='keep.all', dates='1998::') 
	...
	obj = portfolio_allocation_helper(data$prices, 
		...
		min.risk.fns = list(
			...
			# MV = min_var_portfolio,
			# RSO.MV = rso_portfolio(min_var_portfolio, 3, 100),
			
			MS = max_sharpe_portfolio(),			
			RSO.MS.2 = rso_portfolio(max_sharpe_portfolio(), 2, 100),
			RSO.MS.3 = rso_portfolio(max_sharpe_portfolio(), 3, 100),
			RSO.MS.4 = rso_portfolio(max_sharpe_portfolio(), 4, 100),
			RSO.MS.5 = rso_portfolio(max_sharpe_portfolio(), 5, 100),
			RSO.MS.6 = rso_portfolio(max_sharpe_portfolio(), 6, 100),
			RSO.MS.7 = rso_portfolio(max_sharpe_portfolio(), 7, 100)
		)
	)
...
###############################################################################
bt_current_quote_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		...
		for(i in ls(data))
			if( last(index(data[[i]])) < as.Date(quotes[i, 'Trade Time']) ) {
				data[[i]] = rbind( data[[i]], make_xts(quotes[i, spl('Open,High,Low,Last,Volume,Last')],
...
png(filename = 'plot1.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')		    	
	...
	plota(temp, ylim=range(temp))
		plota_lines(temp[,1],col=1)
		plota_lines(temp[,2],col=2)
		plota_lines(temp[,3],col=3)
	plota_legend(colnames(temp),1:3)
			
	# Plot correlation table
	temp = cor(temp / mlag(temp)- 1, use='complete.obs', method='pearson')
			temp[] = plota_format(100 * temp, 0, '', '%')
	plot_table(temp)	
dev.off()		
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		...
		data$GLD = extend_GLD(data$GLD)	
		data$DBC = extend_data(data$DBC, get_CRB(), scale=T)
...
bt_extend_DBC_update_test <- function() 
{
    #*****************************************************************
    # Load historical data
    #******************************************************************    
	...
	temp = extract_table_from_webpage( join(readLines("TRJ_CRB")), 'EODValue' )
		temp = join( apply(temp, 1, join, ','), '\n' )
	data$CRB_1 = make_stock_xts( read_xts(temp, format='%m/%d/%y' ) )
    ...
    data$CRB_2 = make_stock_xts( read_xts("prfmdata.csv", format='%m/%d/%Y' ) )
...
png(filename = 'plot1.png', width = 500, height = 500, units = 'px', pointsize = 12, bg = 'white')
	...
	plota_matplot(scale_one(data$prices))
...
###############################################################################
bt_permanent_portfolio_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		...
		# extend_GLD with Gold.PM - London Gold afternoon fixing prices
		data$GLD = extend_GLD(data$GLD)
	...
	models$buy.hold = bt_run_share(data, clean.signal=F)

		
	# Equal Weight
	data$weight[] = NA
		data$weight[period.ends.y,] = ntop(prices[period.ends.y,], n)
	models$equal.weight.y = bt_run_share(data, clean.signal=F)

	# Rebalance only when threshold is broken
	models$threshold.y = bt_max_deviation_rebalancing(data, models$buy.hold, target.allocation, 10/100, 0, period.ends = period.ends.y) 
	...
	models$equal.weight.q = bt_run_share(data, clean.signal=F)

	# Rebalance only when threshold is broken
	models$threshold.q = bt_max_deviation_rebalancing(data, models$buy.hold, target.allocation, 10/100, 0, period.ends = period.ends.q) 
...
###############################################################################
bt_permanent_portfolio2_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data$weight[] = NA
		data$weight[period.ends,] = weight.dollar[period.ends,]
	models$dollar = bt_run_share(data, clean.signal=F)
				
	#*****************************************************************
	# Dollar Weighted + 7% target volatility
	#****************************************************************** 				
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$dollar,
						weight.dollar, 7/100, 21, 100/100)[period.ends,]
	models$dollar.target7 = bt_run_share(data, clean.signal=F)
	...
	data$weight[] = NA
		data$weight[period.ends,] = weight.risk[period.ends,]
	models$risk = bt_run_share(data, clean.signal=F)

	if(F) {
		# risk weighted + 7% target volatility
		data$weight[] = NA
			data$weight[period.ends,] = target_vol_strategy(models$risk,
							weight.risk, 7/100, 21, 100/100)[period.ends,]
		models$risk.target7 = bt_run_share(data, clean.signal=F)
	
		# risk weighted + 5% target volatility
		data$weight[] = NA
			data$weight[period.ends,] = target_vol_strategy(models$risk,
							weight.risk, 5/100, 21, 100/100)[period.ends,]
		models$risk.target5 = bt_run_share(data, clean.signal=F)
	}	
	#*****************************************************************
	...
	models$dollar.tactical = bt_run_share(data, clean.signal=F)

	#*****************************************************************
	# Tactical + 7% target volatility
	#****************************************************************** 				
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$dollar.tactical,
						weight.dollar.tactical, 7/100, 21, 100/100)[period.ends,]
	models$dollar.tactical.target7 = bt_run_share(data, clean.signal=F)
...
###############################################################################
bt_permanent_portfolio3_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data$weight[] = NA
		data$weight[period.ends,] = weight.dollar[period.ends,]
	models$dollar = bt_run_share(data, commission=commission, clean.signal=F)
	...
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$dollar,
						weight.dollar, 7/100, 21, 100/100)[period.ends,]
	models$dollar.target7 = bt_run_share(data, commission=commission, clean.signal=F)
	...
	data$weight[] = NA
		data$weight[period.ends,] = weight.risk[period.ends,]
	models$risk = bt_run_share(data, commission=commission, clean.signal=F)
	...
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$risk,
						weight.risk, 7/100, 21, 100/100)[period.ends,]
	models$risk.target7 = bt_run_share(data, commission=commission, clean.signal=F)
	...
	models$risk.target7.shy = bt_run_share(data, commission=commission, clean.signal=F)
...
dev.off()	   	
	...
	data$weight[] = NA
		data$weight[period.ends,] = weight.dollar.tactical[period.ends,]
	models$dollar.tactical = bt_run_share(data, commission=commission, clean.signal=F)
	...
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$dollar.tactical,
						weight.dollar.tactical, 7/100, 21, 100/100)[period.ends,]
	models$dollar.tactical.target7 = bt_run_share(data, commission=commission, clean.signal=F)
	...
	models$risk.tactical = bt_run_share(data, commission=commission, clean.signal=F)
	
	#*****************************************************************
	# Risk Weighted + Tactical + 7% target volatility + SHY
	#****************************************************************** 				
	data$weight[] = NA
		data$weight[period.ends,] = target_vol_strategy(models$risk.tactical,
						weight.risk.tactical, 7/100, 21, 100/100)[period.ends,]
  		cash = 1-rowSums(data$weight)
	    data$weight$SHY[period.ends,] = cash[period.ends]						
	models$risk.tactical.target7.shy = bt_run_share(data, commission=commission, clean.signal=F)
...
bt_mca_test <- function() 
{
	...
	getSymbols(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)							
	bt_prep(data, align='keep.all', dates='2002:08::')
	
	#write_xts(data$prices, 'data.csv')
	
	#*****************************************************************
	# Code Strategies
	#****************************************************************** 	
	
	obj = portfolio_allocation_helper(data$prices, periodicity = 'weeks',
		min.risk.fns = list(EW=equal_weight_portfolio,
						RP=risk_parity_portfolio(),
						MV=min_var_portfolio,
						MD=max_div_portfolio,
...
png(filename = 'plot2.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')   	
	...
	cdi = custom_composite_diversification_indicator(obj,plot_table = F)	
...
png(filename = 'plot3.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')   				
	performance_barchart_helper(out, 'Sharpe,Cagr,DVR,MaxDD', c(T,T,T,T))
dev.off()	
	
png(filename = 'plot4.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')   		
	performance_barchart_helper(out, 'Volatility,Portfolio Turnover,Composite Diversification Indicator(CDI)', c(F,F,T))
dev.off()	
...
dev.off()	
	...
	plot_table(  sapply(models, function(m) round(100*last(m$weight),1))  )
...
###############################################################################
bt_mca_speed_test <- function() 
{
	#*****************************************************************
	# Setup
	#*****************************************************************
	load_packages('quadprog,corpcor')
	...
	benchmark(
		min_var_portfolio(ia, constraints),
		min_corr_portfolio(ia, constraints),
		min_corr2_portfolio(ia, constraints),
...
bt_crp_test <- function() 
{
	...
	plota(prices$iroqu, col='blue', type='l',   ylim=range(prices), main = '"iroqu" and "kinar"', ylab='')
		plota_lines(prices$kinar, col='red')
		grid()
	plota_legend('iroqu,kinar', 'blue,red')	
	...
	for (i in 1:length(crps)) {
		...
		equity = bt_run(data, silent=T)$equity
	...
	plota(prices$iroqu, col='blue', type='l',   ylim=range(prices, universal), 
		...
		plota_lines(prices$kinar, col='red')
		plota_lines(universal, col='green')
		grid()
	plota_legend('iroqu,kinar,universal', 'blue,red,green')	
	...
	obj = portfolio_allocation_helper(data$prices, periodicity = 'weeks', lookback.len = 460, 
    ...
    plotbt_custom_report_part2( models$CRP )
...
bt_october_gold_test <- function() 
{
    ...
    #****************************************************************** 
png(filename = 'plot1.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')   
	month_year_seasonality(data, ticker)
dev.off()    
    ...
    #****************************************************************** 
    data = bundes_bank_data_gold()

    #*****************************************************************
    # Look at the Month of the Year Seasonality
    #****************************************************************** 
png(filename = 'plot2.png', width = 600, height = 600, units = 'px', pointsize = 12, bg = 'white')   
	month_year_seasonality(data, 'GOLD', lookback.len = nrow(data))
dev.off()    
	...
	write_xts(extend_data(GLD, data / 10), 'GOLD.csv')
...
###############################################################################
	...
	couch_potato_strategy <- function
	(
		data.all,
	...
	{ 
		...
		model = bt_run_share(data, clean.signal=F, commission=commission)
		
		return(model)
	} 	

bt_couch_potato_test <- function() 
{
	#*****************************************************************
	# Canadian  Version	
	#*****************************************************************
	...
	models$classic = couch_potato_strategy(data, 'can.eq,us.eq,can.bond', rep(1/3,3), periodicity, dates)
	models$global = couch_potato_strategy(data, 'can.eq,us.eq,int.eq,can.bond', c(0.2, 0.2, 0.2, 0.4), periodicity, dates)
	models$yield = couch_potato_strategy(data, 'can.div,can.it,us.div,can.bond', c(0.25, 0.25, 0.25, 0.25), periodicity, dates)
	models$growth = couch_potato_strategy(data, 'can.eq,us.eq,int.eq,can.bond', c(0.25, 0.25, 0.25, 0.25), periodicity, dates)
	
	models$complete = couch_potato_strategy(data, 'can.eq,us.eq,int.eq,can.re,can.real.bond,can.bond', c(0.2, 0.15, 0.15, 0.1, 0.1, 0.3), periodicity, dates)	
	
	models$permanent = couch_potato_strategy(data, 'can.eq,can.gold,can.bond', c(0.25,0.25,0.5), periodicity, dates)	
...
dev.off()
	...
	models$classic = couch_potato_strategy(data, 'VIPSX,VTSMX', rep(1/2,2), periodicity, dates)
	models$margarita = couch_potato_strategy(data, 'VIPSX,VTSMX,VGTSX', rep(1/3,3), periodicity, dates)
	models$permanent = couch_potato_strategy(data, 'SPY,TLT,GLD,SHY', rep(1/4,4), periodicity, dates)
...
###############################################################################
bt_regime_detection_test <- function() 
{	
	#*****************************************************************
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	layout(1:3)
		plota_control$col.x.highlight = col_add_alpha(true.states+1, 150)
	plota(data, type='h', plotX=F, x.highlight=T)
		plota_legend('Returns + True Regimes')
	plota(cumprod(1+data/100), type='l', plotX=F, x.highlight=T)
		plota_legend('Equity + True Regimes')
	
		plota_control$col.x.highlight = col_add_alpha(VitPath+1, 150)
	plota(data, type='h', x.highlight=T)
		plota_legend('Returns + Detected Regimes')				
dev.off()	
...
###############################################################################
bt_regime_detection_pitfalls_test <- function() 
{	
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
    ...
    models$states.all = bt_run_share(data)
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')										    	
	layout(1:3)
	col = col_add_alpha('white',210)
	plota(states.all[out.sample.index], type='s', plotX=F)
		plota_legend('Implied States based on all data', x='center', bty='o', bg=col, box.col=col,border=col,fill=col,cex=2)
	plota(states.win1[out.sample.index], type='s')
		plota_legend('Implied States based on rolling window', x='center', bty='o', bg=col, box.col=col,border=col,fill=col,cex=2)
	plota(states.win2[out.sample.index], type='s')
		plota_legend('Implied States based on rolling window(re-fit)', x='center', bty='o', bg=col, box.col=col,border=col,fill=col,cex=2)
dev.off()
	...
	data$weight[] = NA
		...
		data$weight[in.sample.index] = NA
    models$states.win1 = bt_run_share(data)
		
	data$weight[] = NA
		data$weight[] = iif(states.win2 == 1, 0, 1)
		data$weight[in.sample.index] = NA
    models$states.win2 = bt_run_share(data)
...
###############################################################################
bt_financial_turbulence_test <- function() 
{	
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
...
{			
	...
	bt_prep(data, align='keep.all', dates='2012')
	...
	save(data, tickers, sectors, file='bt_pca_test.Rdata')
	#load(file='bt_pca_test.Rdata')
...
{		
	...
	# load data saved in the bt_pca_test() function
	load(file='bt_pca_test.Rdata')
...
bt_pca_trading_test <- function()
{
	...
	components = components / rep_row(colSums(abs(components)),len(tickers))
	...
	#****************************************************************** 	
	library(tseries)
	equity = bt_apply_matrix(1 + ifna(-ret %*% components,0), cumprod)
		equity = make_xts(equity, index(prices))
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')	
	...
	# add Bollinger Bands
	i.comp = 4	
	bbands1 = BBands(rep_col(equity[,i.comp],3), n=200, sd=1)
	bbands2 = BBands(rep_col(equity[,i.comp],3), n=200, sd=2)
	...
	plota_matplot(temp, main=paste(i.comp, 'Principal component'))
	
	barplot_with_labels(sort(components[,i.comp]), 'weights')
dev.off()
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	layout(1:2)
	plota_matplot(prices, plotX=F)
	plota_matplot(equity)
dev.off()
...
###############################################################################
# Details_for the Visual of Current Major Market Clusters post by David Varadi
# http://cssanalytics.wordpress.com/2013/01/10/a-visual-of-current-major-market-clusters/
###############################################################################		
bt_cluster_visual_test <- function()
{
    #*****************************************************************
	# Load historical data for ETFs
	#****************************************************************** 
	...
	# setup period and method to compute_correlations
	dates = '2012::2012'
...
###############################################################################		
bt_cluster_optimal_number_test <- function()
{
    #*****************************************************************
	# Load historical data for ETFs
	#****************************************************************** 
	...
	find_maximum_distance_point(p.exp[-1]) + 1
...
# based on the bt_cluster_optimal_number_test function
###############################################################################
bt_cluster_optimal_number_historical_test <- function()
{
	#*****************************************************************
	# Load historical data for ETFs
	#****************************************************************** 
	...
	bt_prep(data, align='keep.all', dates=dates)

	
	#*****************************************************************
	# Use following 3 methods to determine number of clusters
	# * Minimum number of clusters that explain at least 90% of variance
	#   cluster_group_kmeans_90
	# * Elbow method
	#   cluster_group_kmeans_elbow
	# * Hierarchical clustering tree cut at 1/3 height
	#   cluster_group_hclust
	#****************************************************************** 

	# helper function to compute portfolio allocation additional stats
	portfolio_allocation_custom_stats_clusters <- function(x,ia) {
		return(list(
			ncluster.90 = max(cluster_group_kmeans_90(ia)),
			ncluster.elbow = max(cluster_group_kmeans_elbow(ia)),
			ncluster.hclust = max(cluster_group_hclust(ia))
		))
	}
...
png(filename = paste('plot',i,'.png',sep=''), width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')						
		plota(hist.cluster, type='l', col='gray', main=title)
			plota_lines(SMA(hist.cluster,10), type='l', col='red',lwd=5)
		plota_legend('Number of Clusters,10 period moving average', 'gray,red', x = 'bottomleft')			
dev.off()		
	}
...
###############################################################################
bt_seasonality_january_test <- function()
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	# Find January's with return > 4%
	#****************************************************************** 
	index =  which( date_month(index(ret)) == 1 & ret > 4/100 )
...
png(filename = 'plot1.png', width = 500, height = 500, units = 'px', pointsize = 12, bg = 'white')
	...
	col=col_add_alpha(spl('black,gray'),200)
	...
	pos = barplot(100*out, border=NA, beside=T, axisnames = F, axes = FALSE,
		...
		axis(1, at = colMeans(pos), labels = date_year(index(out)), las=2)
		axis(2, las=1)
	grid(NA, NULL)
	abline(h= 100*mean(out$Year), col='red', lwd=2)		
	plota_legend(spl('January,Annual,Average'),  c(col,'red'))
...
png(filename = 'plot2.png', width = 500, height = 500, units = 'px', pointsize = 12, bg = 'white')
	...
	# plot_table
	plot_table(round(100*as.matrix(out),1))
...
############################################################################### 
bt_cluster_portfolio_allocation_test <- function()
{
	#*****************************************************************
	# Load historical data for ETFs
	#****************************************************************** 
	...
	# Create Portfolio
	#****************************************************************** 		
	fn.name = 'risk_parity_portfolio'	
	fn.name = 'equal_weight_portfolio'	
			
	
	names = c('risk_parity_portfolio', 'equal_weight_portfolio')
...
for(fn.name in names) {
	...
	# create clusters
	group = cluster_group_kmeans_90(ia)
	ngroups = max(group)
	...
	for(g in 1:ngroups) {
		if( sum(group == g) == 1 ) {
			...
			ia.temp = create_ia(ia$hist.returns[, group == g, drop=F]) 
...
# Example of the Cluster Portfolio Allocation method
############################################################################### 
bt_cluster_portfolio_allocation_test1 <- function()
{
	#*****************************************************************
	# Load historical data for ETFs
	#****************************************************************** 
...
load_hist_stock_data <- function()
{
	...
	for(ticker in tickers)
		write_xts(data[[ticker]], paste0(ticker, '.csv'), format='%m/%d/%Y')
	
	# one file
	write_xts(bt_apply(data, Ad), 'adjusted.csv', format='%m/%d/%Y')
	...
	if(data.load.method == 'basic') {		
		...
		getSymbols_sit(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T, stock.folder = stock.folder)
	}else if(data.load.method == 'custom.local') {
		# custom format historical price files
		for(n in tickers) {
			data[[n]] = read_xts(paste(stock.folder, n, '.csv', sep=''), format='%m/%d/%Y')
		}	
	}else if(data.load.method == 'custom.one.file') {
		# read from one csv file, column headers are tickers
		filename = 'adjusted.csv'
		all.data = read_xts(paste(stock.folder, filename, sep=''), format='%m/%d/%Y')
		
		# alternatively reading xls/xlsx
		#load_packages('readxl')
		#all.data = read_xts(read_excel('adjusted.xls'))
	...
	obj = portfolio_allocation_helper(data$prices, period.ends=period.ends, lookback.len = 250, 
		min.risk.fns = list(EW=equal_weight_portfolio,
						RP=risk_parity_portfolio(),
						MV=min_var_portfolio,
						MC=min_corr_portfolio)
...
###############################################################################
john_ehlers_custom_strategy_plot <- function(
	data,
	models,
	name,
	main = name,
...
john_ehlers_filter_test <- function() {
    ...
    # John Ehlers Stochastic
    stoch = roofing_stochastic_indicator(prices)
    
	# Day Stochastic
	stoch14 = bt_apply(data, function(x) stoch(HLC(x),14)[,'slowD'])
...
dev.off()	
    ...
    data$weight[] = NA
        data$weight[] = iif(cross_up(stoch, 0.2), 1, iif(cross_dn(stoch, 0.8), -1, NA))
    models$post = bt_run_share(data, clean.signal=T, trade.summary=T)

    data$weight[] = NA
        data$weight[] = iif(cross_up(stoch, 0.2), 1, iif(cross_dn(stoch, 0.8), 0, NA))
    models$post.L = bt_run_share(data, clean.signal=T, trade.summary=T)
    
    data$weight[] = NA
        data$weight[] = iif(cross_up(stoch, 0.2), 0, iif(cross_dn(stoch, 0.8), -1, NA))
    models$post.S = bt_run_share(data, clean.signal=T, trade.summary=T)
    ...
    data$weight[] = NA
        data$weight[] = iif(cross_dn(stoch, 0.2), 1, iif(cross_up(stoch, 0.8), -1, NA))
    models$pre = bt_run_share(data, clean.signal=T, trade.summary=T)

    data$weight[] = NA
        data$weight[] = iif(cross_dn(stoch, 0.2), 1, iif(cross_up(stoch, 0.8), 0, NA))
    models$pre.L = bt_run_share(data, clean.signal=T, trade.summary=T)

    data$weight[] = NA
        data$weight[] = iif(cross_dn(stoch, 0.2), 0, iif(cross_up(stoch, 0.8), -1, NA))
    models$pre.S = bt_run_share(data, clean.signal=T, trade.summary=T)
...
jpeg(filename = 'plot3.jpg', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	john_ehlers_custom_strategy_plot(data, models, 'post.L', dates = '2013::', layout=T,
		main = 'post.L: Buy When the Indicator Crosses Above 20% and Sell when the Indicator Crosses Below 80%')
	john_ehlers_custom_strategy_plot(data, models, 'pre.L', dates = '2013::', layout=T,
...
dev.off()	
	...
	d = index_xts(prices[dates2index(prices, trades[,'entry.date'])])
		col = col_add_alpha('green', 50)
		segments(d, rep(0.2,len(d)), d, rep(0.25,len(d)), col=col, lwd=5)
		points(d, rep(0.25,len(d)), pch=24, col=col, bg=col, lwd=5)
	
	d = index_xts(prices[dates2index(prices, trades[,'exit.date'])])
		col = col_add_alpha('red', 50)
...
# Switch from FSENX to_cash at the May close.
# Switch from cash to Fidelity Select Gold (FSAGX) at the August close.
# Switch from FSAGX to_cash at the September close.
...
###############################################################################
bt_calendar_based_sector_strategy_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	#proxy_test(proxies)
	#proxy_overlay_plot(proxies)
	#bt_start_dates(data)		

	
	# extend	
	data$BIL = extend_data(data$BIL, TB3M, scale=T)	
	...
	months = date_month(dates[period.ends])
	...
	data$weight[] = NA
		data$weight$VFINX[] = 1
	models$VFINX  = bt_run_share(data, clean.signal=F, dates=dates, do.lag=do.lag) 
	...
	models$VFINX_Cash  = bt_run_share(data, clean.signal=F, dates=dates, do.lag=do.lag) 	
	...
	models$Sector  = bt_run_share(data, clean.signal=F, dates=dates, do.lag=do.lag) 	
...
dev.off()		
	...
	plotbt_custom_report_part2(models$sector, trade.summary=T)
	...
	models$sector1  = bt_run_share(data, clean.signal=F) 	
...
###############################################################################
bt_7twelve_strategy_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data$weight[] = NA
		data$weight$VFINX[] = 1
	models$VFINX  = bt_run_share(data, clean.signal=F) 
		
	#*****************************************************************
	# Code Strategies
	#****************************************************************** 	
	obj = portfolio_allocation_helper(data$prices, periodicity = 'years',
		min.risk.fns = list(EW=equal_weight_portfolio)
	) 	
	models$year = create_strategies(obj, data)$models$EW

	obj = portfolio_allocation_helper(data$prices, periodicity = 'quarters',
		min.risk.fns = list(EW=equal_weight_portfolio)
	) 	
	models$quarter = create_strategies(obj, data)$models$EW
		
	obj = portfolio_allocation_helper(data$prices, periodicity = 'months',
		min.risk.fns = list(EW=equal_weight_portfolio)
	) 	
	models$month = create_strategies(obj, data)$models$EW
...
###############################################################################
bt_mebanefaber_modified_mn_test <- function() 
{	
    #*****************************************************************
    # Load historical data
    #******************************************************************    
	...
	temp = get_fama_french_data('F-F_Research_Data_Factors', periodicity = '',download = T, clean = T)
		ret = temp[[1]]$Mkt.RF + temp[[1]]$RF
		price = bt_apply_matrix(ret / 100, function(x) cumprod(1 + x))
	data$SPY = make_stock_xts( price )
	
	# load historical momentum returns
	temp = get_fama_french_data('10_Portfolios_Prior_12_2', periodicity = '',download = T, clean = T)		
		ret = temp[[1]]
		price = bt_apply_matrix(ret / 100, function(x) cumprod(1 + x))
	data$HI.MO = make_stock_xts( price$High )
	data$LO.MO = make_stock_xts( price$Low )
	...
	# Create Plots
	#*****************************************************************
	# plota_matplot(data$prices, log = 'y')
	...
	data$weight[] = NA
		data$weight$SPY[] = 1
	models$SPY = bt_run_share(data, clean.signal=T)
	
	data$weight[] = NA
		data$weight$HI.MO[] = 1
	models$HI.MO = bt_run_share(data, clean.signal=T)
	
	data$weight[] = NA
		data$weight$LO.MO[] = 1
	models$LO.MO = bt_run_share(data, clean.signal=T)
	
	data$weight[] = NA
		data$weight$HI.MO[] = 1
		data$weight$LO.MO[] = -1
	models$MKT.NEUTRAL = bt_run_share(data, clean.signal=F)
	...
	# (in 20% steps for every 10% decline in stocks)
	#*****************************************************************	
	market.drawdown = -100 * compute_drawdown(data$prices$SPY)
	...
	models$Modified.MN = bt_run_share(data, clean.signal=F)
...
###############################################################################
bt_mebanefaber_f_squared_test <- function() 
{	
    #*****************************************************************
    # Load historical data
    #******************************************************************    
	...
	# load historical market returns
	temp = get_fama_french_data('F-F_Research_Data_Factors', periodicity = '',download = download, clean = T)
		ret = cbind(temp[[1]]$Mkt.RF + temp[[1]]$RF, temp[[1]]$RF)
		price = bt_apply_matrix(ret / 100, function(x) cumprod(1 + x))
	data$SPY = make_stock_xts( price$Mkt.RF )
	data$SHY = make_stock_xts( price$RF )
	
	# load historical momentum returns
	temp = get_fama_french_data('10_Industry_Portfolios', periodicity = '',download = download, clean = T)		
		ret = temp[[1]]
		price = bt_apply_matrix(ret[,1:9] / 100, function(x) cumprod(1 + x))
	for(n in names(price)) data[[n]] = make_stock_xts( price[,n] )
	
	# align dates
	data$symbolnames = c(names(price), 'SHY', 'SPY')
	bt_prep(data, align='remove.na', dates='2000::')
	...
	data$weight[] = NA
		data$weight$SPY[1] = 1
	models$SPY = bt_run_share(data, clean.signal=F, dates=bt.dates)
	...
	models$EW = bt_run_share(data, clean.signal=F, dates=bt.dates)
	
	#*****************************************************************
	# Code Strategies
	# http://www.mebanefaber.com/2013/12/04/square-root-of-f-squared/
	#****************************************************************** 			
	sma = bt_apply_matrix(prices, SMA, 10)
	...
	data$weight[] = NA
		data$weight[] = weight
	models$strategy1 = bt_run_share(data, clean.signal=F, dates=bt.dates)
    ...
    #******************************************************************       	
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')		
	strategy_performance_snapshoot(models, one.page = T)
dev.off()
...
bt_averaged_test <- function() 
{
	...
	data <- new.env()
	getSymbols_extra(tickers, src = 'yahoo', from = '1980-01-01', env = data, set.symbolnames = T, auto.assign = T)
		#bt_start_dates(data)
		for(i in data$symbolnames) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)
	bt_prep(data, align='keep.all', dates=paste(start.date-2,':12::',sep=''), fill.gaps = T)
	...
	#****************************************************************** 	
	lookback.len = 40
	create_ia.fn = create_ia
	
	# input assumptions are averaged on 20, 40, 60 days using 1 day lag
	ia.array = c(20,40,60)
	avg.create_ia.fn = create_ia_averaged(ia.array, 1)
	...
	avg.momentum = momentum_averaged(prices, mom.array, 3)
	...
	min.risk.fns = list(
		...
		MV.RSO = rso_portfolio(min_var_portfolio, 3, 100, const.ub = max.product.exposure),
		MCE.RSO = rso_portfolio(min_corr_excel_portfolio, 3, 100, const.ub = max.product.exposure)
	)

	#*****************************************************************
	# Code Strategies
	#****************************************************************** 	
make.strategy.custom <- function(name, create_ia.fn, lookback.len, universe, env) {
	obj = portfolio_allocation_helper(data$prices, 
		periodicity = periodicity,
		universe = universe,
		lookback.len = lookback.len,
		create_ia.fn = create_ia.fn,
		const.ub = max.product.exposure,
		min.risk.fns = min.risk.fns,
		adjust2positive.definite = F
	)
	env[[name]] = create_strategies(obj, data, prefix=paste(name,'.',sep=''))$models
}


	models <- new.env()	
	make.strategy.custom('ia.none'        , create_ia.fn    , lookback.len, universe       , models)
	make.strategy.custom('ia.mom'         , create_ia.fn    , lookback.len, mom.universe   , models)
	make.strategy.custom('ia.avg_mom'     , create_ia.fn    , lookback.len, avgmom.universe, models)
	make.strategy.custom('avg_ia.none'    , avg.create_ia.fn, 252         , universe       , models)
	make.strategy.custom('avg_ia.mom'     , avg.create_ia.fn, 252         , mom.universe   , models)
	make.strategy.custom('avg_ia.avg_mom' , avg.create_ia.fn, 252         , avgmom.universe, models)
...
strategy.snapshot.custom <-	function(models, n = 0, title = NULL) {
	...
	plotbt(models, plotX = T, log = 'y', LeftMargin = 3, main = title)
		mtext('Cumulative Performance', side = 2, line = 1)
	plotbt_strategy_sidebyside(models)
	barplot_with_labels(sapply(models, compute_turnover, data), 'Average Annual Portfolio Turnover', T)	
}
...
png(filename = 'plot1.png', width = 900, height = 900, units = 'px', pointsize = 12, bg = 'white')		
	...
	models_final = c(models$ia.none, models$ia.mom)
	strategy.snapshot.custom(models_final, len(min.risk.fns), 'Momentum Filter')
dev.off()
...
png(filename = 'plot2.png', width = 900, height = 900, units = 'px', pointsize = 12, bg = 'white')		
	...
	models_final = c(models$ia.none, models$avg_ia.none)
	strategy.snapshot.custom(models_final, len(min.risk.fns), 'Averaged Input Assumptions')
dev.off()
...
png(filename = 'plot3.png', width = 900, height = 900, units = 'px', pointsize = 12, bg = 'white')			
	...
	models_final = c(models$ia.mom, models$ia.avg_mom)
	strategy.snapshot.custom(models_final, len(min.risk.fns), 'Averaged Momentum')
dev.off()

png(filename = 'plot4.png', width = 900, height = 900, units = 'px', pointsize = 12, bg = 'white')				
	# basic + momentum vs avg ia + avg.momentum
	models_final = c(models$ia.mom, models$avg_ia.avg_mom)
	strategy.snapshot.custom(models_final, len(min.risk.fns), 'Averaged vs Base')	
dev.off()
...
###############################################################################
bt_probabilistic_momentum_test <- function()
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data <- new.env()
	getSymbols_extra(tickers, src = 'yahoo', from = '1980-01-01', env = data, auto.assign = T)
		for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)
	bt_prep(data, align='remove.na', dates='::')
	...
	models$Simple  = bt_run_share(data, clean.signal=T) 	
	...
	data$weight[] = NA
		data$weight$EQ[] = iif(cross_up(momentum.p, confidence.level), 1, iif(cross_dn(momentum.p, (1 - confidence.level)), 0,NA))
		data$weight$FI[] = iif(cross_dn(momentum.p, (1 - confidence.level)), 1, iif(cross_up(momentum.p, confidence.level), 0,NA))
	models$Probabilistic  = bt_run_share(data, clean.signal=T) 	

	data$weight[] = NA
		data$weight$EQ[] = iif(cross_up(momentum.p, confidence.level), 1, iif(cross_up(momentum.p, (1 - confidence.level)), 0,NA))
		data$weight$FI[] = iif(cross_dn(momentum.p, (1 - confidence.level)), 1, iif(cross_up(momentum.p, confidence.level), 0,NA))
	models$Probabilistic.Leverage = bt_run_share(data, clean.signal=T) 	
...
pdf(file = 'Probabilistic.Momentum.Report.pdf', width=8.5, height=11)     
   	strategy_performance_snapshoot(bt_trim(models), data = data)
dev.off()
	...
	signal = iif(cross_up(momentum.p, confidence.level), 1, iif(cross_dn(momentum.p, (1 - confidence.level)), 0,NA))
	signal = ifna_prev(signal) == 1
...
bt_load_thebonnotgang_data <- function(Symbols, folder, silent=F, clean=T) 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	for(s in spl(Symbols))
		data[[s]] = read_xts(paste0(folder,s,'_1m.csv'), 
...
bt_intraday_day <- function(dates) 
{
	dates.number = as.double(dates)		
...
bt_intraday_thebonnotgang_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data <- new.env()
	data$SPY = read_xts(paste0(spath,'SPY_1m.csv'), 
		sep = ';', date.column = 3, format='%Y-%m-%d %H:%M:%S', index.class = c("POSIXlt", "POSIXt"))

	data$GLD = read_xts(paste0(spath,'GLD_1m.csv'), 
		sep = ';', date.column = 3, format='%Y-%m-%d %H:%M:%S', index.class = c("POSIXlt", "POSIXt"))
...
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	plota(plota_scale_volume(data$SPY['2012:11:01']), type = 'volume')	
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	plota(plota_scale_volume(data$SPY['2013:11:01']), type = 'volume')	
...
png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	plota(data$GLD, type='l', col='blue', main='GLD')
		plota_lines(data.daily$GLD, type='l', col='red')
	plota_legend('Intraday,Daily', 'blue,red')	
	
dev.off()
png(filename = 'plot4.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	

	plota(data$SPY, type='l', col='blue', main='SPY')
		plota_lines(data.daily$SPY, type='l', col='red')
	plota_legend('Intraday,Daily', 'blue,red')	
...
dev.off()
	...
	# Load historical data
	#****************************************************************** 
	data = bt_load_thebonnotgang_data('SPY,GLD', spath)
	#plota(data$SPY['2013:10:11'], type='candle')
	bt_prep(data, align='keep.all', fill.gaps = T)
	...
	data$weight[] = NA
		data$weight$SPY = 0.5
		data$weight$GLD = 0.5
	models$EW = bt_run_share(data, clean.signal=F)
...
###############################################################################
bt_strategy_intraday_thebonnotgang_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
...
dev.off()    	
    ...
    #******************************************************************    
   	day.stat = bt_intraday_day(dates)
...
bt_pair_strategy_intraday_thebonnotgang_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	spath = 'c:/Desktop/'
spath = 'c:/Documents and Settings/mkapler/Desktop/'	
	data = bt_load_thebonnotgang_data('USO,GLD', spath)
	bt_prep(data, align='keep.all', fill.gaps = T)
	...
	stoch = (prices - bt_apply_matrix(prices, runMin, lookback)) / (bt_apply_matrix(prices, runMax, lookback) - bt_apply_matrix(prices, runMin, lookback))	
		stoch = bt_apply_matrix(stoch, ifna_prev)
	...
	data$weight[] = NA
		...
		data$weight[as.vector(0:(lookback-1) + rep_row(day.stat$day.start,lookback)),] = 0
		
	models$P = bt_run_share(data, clean.signal=T, do.lag = 1)
	...
	models$P1 = bt_run_share(data, clean.signal=T, do.lag = 1)
...
###############################################################################
bt_calendar_strategy_month_end_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	key.date.index = date_month_ends(data$dates, F)
	key.date = NA * prices
	...
	models$T0 = bt_run_share(data, do.lag = 0, trade.summary=T, clean.signal=T)  

	#*****************************************************************
	# Add helper functions
	#*****************************************************************
	calendar_strategy <- function(data, signal, universe = data$prices > 0) {
		data$weight[] = NA
			data$weight[] = ifna(universe & signal, F)
		bt_run_share(data, do.lag = 0, trade.summary=T, clean.signal=T)  	
	}
	
	calendar_signal <- function(key.date, offsets = 0) {
	...
	# Trade on key.date
	models$T0 = calendar_strategy(data, key.date)

	# Trade next day after key.date
	models$N1 = calendar_strategy(data, mlag(key.date,1))
	# Trade two days next(after) key.date
	models$N2 = calendar_strategy(data, mlag(key.date,2))

	# Trade a day prior to key.date
	models$P1 = calendar_strategy(data, mlag(key.date,-1))
	# Trade two days prior to key.date
	models$P2 = calendar_strategy(data, mlag(key.date,-2))
	...
	models$P2N2 = calendar_strategy(data, signal)

	# same, but using helper function above	
	models$P2N2 = calendar_strategy(data, calendar_signal(key.date, -2:2))
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	strategy_performance_snapshoot(models, control=list(comparison=T), sort.performance=F)
...
dev.off()	
	...
	# Using new functions
	#*****************************************************************
	signals = calendar_signal(key.date, T0=0, N1=1, N2=2, P1=-1, P2=-2, P2N2=-2:2)
	models = calendar_strategy(data, signals, universe = universe)
	
	strategy_performance_snapshoot(models, control=list(main=T))
	
	strategy_performance_snapshoot(models, control=list(comparison=T), sort.performance=F)
		
	strategy_performance_snapshoot(models["P2N2"], control=list(monthly=T))
	
	strategy_performance_snapshoot(models, control=list(transition=T))
...
###############################################################################
bt_calendar_strategy_option_expiry_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	second.friday = third_friday_month(years[1]:years[2], 12) - 7
	...
	signals = calendar_signal(key.date, signals)
	models = calendar_strategy(data, signals, universe = universe)
		names(models)
	
png(filename = 'plot1.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
    
    strategy_performance_snapshoot(models, T, sort.performance=F)
...
png(filename = 'plot2.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    	
	...
	out = sapply(models, function(x) list(
		CAGR = 100*compute_cagr(x$equity),
		MD = 100*compute_max_drawdown(x$equity),
		Win = x$trade.summary$stats['win.prob', 'All'],
		Profit = x$trade.summary$stats['profitfactor', 'All']
		))	
	performance_barchart_helper(out, sort.performance = F)
	
dev.off()	

png(filename = 'plot3.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    		

	strategy_performance_snapshoot(models$N15, control=list(main=T))
	
dev.off()		
	
	#strategy_performance_snapshoot(models['N15'], control=list(transition=T))
	
	#strategy_performance_snapshoot(models['N15'], control=list(monthly=T))
		
png(filename = 'plot4.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    		
	
	last_trades(models$N15)
...
png(filename = 'plot5.png', width = 600, height = 500, units = 'px', pointsize = 12, bg = 'white')    		
	...
	trades = models$N15$trade.summary$trades
		trades = make_xts(parse_number(trades[,'return']), as.Date(trades[,'entry.date']))
	layout(1:2)
...
dev.off()			
	...
	#plotbt_custom_report_part2(models$N15, trade.summary=F)
...
###############################################################################
bt_calendar_strategy_fed_days_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
...
###############################################################################
bt_adjusted_momentum_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	bt_prep_remove_symbols(data, 'VIX')
	...
	data$weight[] = NA
		data$weight[] = 1
	models$buy.hold = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	models$ma200 = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	models$roc200 = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	models_vix.mom = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	models$er.mom = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	data$weight[] = NA
		data$weight[] = iif(cross_up(sma, upper.band), 1, iif(cross_dn(sma, lower.band), 0, NA))
	models$er.mom.cushioned = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
...
if(F) {		
	...
	data$weight[] = NA
		...
		#data$weight[] = iif(cross_up(sma, upper.band), 1, iif(cross_dn(sma, lower.band), 0, NA))
	models$ea = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
	...
	data$weight[] = NA
		data$weight[] = iif(cross_up(sma, upper.band), 1, iif(cross_dn(sma, lower.band), 0, NA))
	models$ea.cushioned = bt_run_share(data, clean.signal=T, commission=commission, trade.summary=T)
}	
	#*****************************************************************
	# Report
	#****************************************************************** 
...
#*****************************************************************
bt_signal_execution_lag_test <- function()
{
	do.lag = 2
	calc.offset = -1
	...
	data <- new.env()
		...
		data$TEST = make_stock_xts(make_xts(1:len(dates), dates))
	bt_prep(data)
...
###############################################################################
bt_execution_price_high_low_test <- function
(
...
) 
{
	...
	for(name in spl('Cl,Op,Hi,Lo')) {
		...
		exec.prices = bt_apply(data, fun)
		
		data$weight[] = NA
			data$execution.price[] = NA
		  	data$execution.price[frequency,] = exec.prices[frequency,]
		  	data$weight[frequency,] = weight[frequency,]
		models[[name]] = bt_run_share(data, trade.summary=T, dates=dates, silent=T, clean.signal=F)
	}	

	#*****************************************************************
	# Code Strategies
	#****************************************************************** 
	low.prices = bt_apply(data, Lo)
	high.prices = bt_apply(data, Hi)
	...
	data$weight[] = NA
		data$execution.price[] = NA		
	  	...
	  	data$weight[frequency,] = weight[frequency,]
	models$Buy.Low.Sell.High = bt_run_share(data, trade.summary=T, dates=dates, silent=T, clean.signal=F)
	...
	data$weight[] = NA
		data$execution.price[] = NA		
	  	data$execution.price[frequency,] = execution.price	  		  	
	  	data$weight[frequency,] = weight[frequency,]
	models$Buy.High.Sell.Low = bt_run_share(data, trade.summary=T, dates=dates, silent=T, clean.signal=F)
...
print(plotbt_strategy_sidebyside(models, make.plot=F, return.table=T))

print('Monthly Results for', m, ':')
print(plotbt_monthly_table(models[[m]]$equity, make.plot = F))
}
...
#
###############################################################################
bt_dual_momentum_test <- function() 
{
	#*****************************************************************
	# Load historical data
	#****************************************************************** 
	...
	data = env()
	getSymbols_extra(tickers, src = 'yahoo', from = '1980-01-01', env = data, set.symbolnames = T, auto.assign = T)
		#print(bt_start_dates(data))
		for(i in data$symbolnames) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)
	bt_prep(data, align='keep.all', fill.gaps = T)
	data = bt_prep_trim(data, '2006::')
	...
	# plot asset history
	#plota_matplot(scale_one(data$prices),main='Asset Performance')
	...
	period.ends = date_ends(prices, 'month')
	...
	custom.weight.portfolio <- function(mom, ntop)
	{
		...
		{
			ntop_helper(mom[ia$nperiod, ia$index], ntop)			
		}	
	}	
	...
	obj = portfolio_allocation_helper(prices, 
		period.ends = period.ends, 
		universe = universe,
		min.risk.fns = list(
			EW=equal_weight_portfolio,
			DM=distribute_weights(
				static_weight_portfolio(rep(1,4)), # there are 4 clusters
				static_group(risk.groups), # predefined groups
...
if(T) {
	...
	for(i in 1:nrow(test)) {
		for(g in 1:4) {
			index = risk.groups == g
			test[i, index] = ntop_helper(mom[period.ends[i], index], 1) * 0.25 # there are 4 sectors, each one gets 1/4
		}
	}
