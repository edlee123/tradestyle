...
#' @export 
############################################################################### 
Rglpk_read_model <- function(file, type = c("MPS_fixed", "MPS_free", "CPLEX_LP", "MathProg"), ignore_first_row = FALSE, verbose = FALSE){
...
###############################################################################
# Create constraints structure from model data (Rglpk_read_model)
#' @export 
############################################################################### 
Rglpk_create_constraints <- function( prob ) 
{       
	#--------------------------------------------------------------------------
	# Create constraints
	#--------------------------------------------------------------------------
	...
	constraints = new_constraints(n, lb = lb, ub = ub)
	...
	if(len(index)>0) constraints = add_constraints(prob$constraint_matrix[,index], type = '=', b = prob$right_hand_side[index], constraints)
...
#' @export 
############################################################################### 
parse_views = function(symbolnames, views) {
	load_packages('Rglpk')

	views = parse_expr(views)
	if (len(views)==0)
		return(list(
...
###############################################################################
	...
	#--------------------------------------------------------------------------	
	# read model
	model = Rglpk_read_model(model.file,type = 'MathProg') 	

	# convert GNU MathProg model to constraint used in solve.QP
	temp = Rglpk_create_constraints(model)$constraints	
...
#' @export 
############################################################################### 
min_var_portfolio_gmpl <- function(ia, constraints)
{
	#--------------------------------------------------------------------------
...
###############################################################################
portopt_mathprog_test <- function( ) 
{
	#*****************************************************************
	# Load packages
	#****************************************************************** 
	load_packages('quantmod,quadprog,corpcor')
			
	#--------------------------------------------------------------------------
	# Create historical input assumptions
	#--------------------------------------------------------------------------
	tickers = dow_jones_components()
	ia = aa_test_create_ia_custom(tickers, dates = '2000::2010')
	...
	# Solve QP problem
	x = min_var_portfolio_gmpl(ia, constraints)	
...
###############################################################################
	...
	# convert GNU MathProg model to constraint used in solve.QP
	constraints = Rglpk_create_constraints(model)$constraints	
...
###############################################################################
	...
	x = min_var_portfolio_gmpl(aa_test_ia_add_short(ia), constraints)	
...
dev.off()	
	...
	# reduce problem size
	ia = aa_test_create_ia_custom(tickers[1:15], dates = '2000::2010')
...
# reduce problem size
ia = aa_test_create_ia_custom(tickers[1:10], dates = '2000::2010')
