...
#' @export 
cross_up <- function( array1, array2 ) { cross( array1, array2 ) }


#' @export 
cross_dn <- function( array1, array2 ) { cross( array2, array1 ) }

#' @export 
cross_up_eq <- function( array1, array2 ) { cross( array1, array2, T ) }

#' @export 
cross_dn_eq <- function( array1, array2 ) { cross( array2, array1, T ) }
...
{	
	# simple percent_rank function
    pctRank <- function(x,i) sum(x[i,1] >= x[(i- (n-1) ):i,])    
        
    out = data
    
    # Apply the percent_rank function to the coredata of our results
    data = coredata(data)
...
#' @export 
############################################################################### 
percent_rankM <- function
(
	..., 	# data
	n = 252	# window length
)
{	
	data = variable_number_arguments( ... )
...
)
{
	# Calculate each day's high/low mean
  	...
  	if(bounded) res = percent_rank(res, 252)
...
)
{
	...
	DVI.magnitude = percent_rank( ColumnE , n )
	DVI.stretch = percent_rank( ColumnH, n )
...
# http://cssanalytics.wordpress.com/2010/06/02/rolling-ev_ratio-a-trend-indicator-or-performance-measurement-statistic/    
...
#' @export 
bt_rank <- function(x, dirMaxMin = TRUE, do.sort = F )
{	
res = NA * x
res[] = {
	if(!do.sort) {
...
#' @export 
############################################################################### 
super_smoother_filter <- function(x) {
...
roofing_filter <- function(x) {
    ...
    super_smoother_filter(HP)
}

# My Stochastic Indicator 2013 John F. Ehlers
#' @export 
roofing_stochastic_indicator  <- function(x, lookback = 20) {
    Filt = roofing_filter(x)
    ...
    super_smoother_filter(Stoc)
}

# Quantile over moving window
#' @export 
runQuantile = function(x, k, probs) {
	load_packages('caTools')
	temp = rep_col(x * NA, len(probs))
