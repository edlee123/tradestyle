...
#' Split columns, it can be populated with bt_unadjusted_add_div_split function
#'
...
(
	...
	do.lag = 1, 	   # lag logic, to be back compatible with bt_run_share
	...
	lot.size = 0.01,
	control = list(
		round_lot = default_round_lot_control()
	),
	
	# cashflow are treated differently based on type
	# the regular cashflows do not affect portfolio_returns
	#
	# the fee.rebate cashflows affect portfolio_returns
	...
	# regular withdrawals should not affect portfolio_returns
	#
	cashflow.control = list(
		...
		#	cashflow.fn = function(info, i, i.prev) round_to((info$cash[i] + sum(info$shares[i,] * prices[i,]))*(5/100), 100), # if specified, cashflows are computed as a function of total equity
	...
	tax.control = NULL
	#tax.control = default_tax_control()
) 
{
	#---------------------------------------------------------
	# check inputs
	...
	dividend.control = verify_control('dividend.control', default.dividend.control, dividend.control)	
	...
	weight[!is.na(weight) & is.na(prices)] = 0
		...
		# lag logic, to be back compatible with bt_run_share
	...
	check.non.positive.prices = function(p, name) {
		if(any(p<=0, na.rm=T)) {
			index = lookup_index(open,  which(p <= 0))
			stop('bt_run_share_ex detected non positive ', name, ' for ' , join(colnames(p)[index$icol], ' , '))
		}
	}
	...
	prices[] = ifna( bt_apply_matrix(prices, ifna_prev), 1)
	...
	#---------------------------------------------------------
	# execute internal version of bt_run_share_ex
	#---------------------------------------------------------
	bt = bt_run_share_ex_internal(b, prices, capital, commission, weight,
	...
	bt$ret = make_xts(bt$ret, index(b$prices))
	bt$weight = make_xts(bt$weight, index(b$prices))
	...
	# prepare output
	bt = bt_run_trim_helper(bt, bt$dates.index)
...
#' @export 
###############################################################################
verify_control = function(name, control, inputs) {
...
#' @export 
###############################################################################
default_tax_control = function(nonqualified = c()) {
	list(
		capital.gain = list(
...
bt_run_share_ex_internal <- function(b, prices, capital, commission, weight,
		lot.size, control, cashflows, adjusted, dividend.control, tax.control
)			
{
	#---------------------------------------------------------
	...
	#---------------------------------------------------------
	# back filled weights
	weight1 = ifna( bt_apply_matrix(weight, ifna_prev), 0)
	...
	#---------------------------------------------------------
	if(!adjusted) {
		dividends = coredata(bt_apply(b, function(x) x[,'Dividend']))
			dividends[is.na(dividends)] = 0
		splits = coredata(bt_apply(b, function(x) x[,'Split']))		
	...
	# i.e. just put dividends to_cash and go forward
	...
	for(i in event.index) {		
		...
		if(last.trade > 0) {
			...
			index = (last.trade + 1) : i
				n.index = len(index)
			share.wt[index,] = rep_row(info$share[last.trade,], n.index)
			info$share[index,] = rep_row(info$share[last.trade,], n.index)
		...
		if(!adjusted) {
			if( trade.dividend[i] ) {			
				if( !is.null(tax.control) ) {
					tax_update_dividends(tax.control, dividend.control, holdings, tax, info$share[i,], dividends[i,], i, b$dates)					
				}
			...
			if( trade.split[i] ) {
				...
				if( !is.null(tax.control) ) {
					for(a in which(info$share[i,] !=0 & splits[i,] > 0)) {
						holdings_split(holdings, a, splits[i,a])
					...
					for(a in which(wash.sale$n.trades > 0 & splits[i,] > 0)) {
						holdings_split(wash.sale, a, splits[i,a])
		...
		if( trade.index[i] ) {	
			...
			if(trade.invest.type == invest.type.def$rebalance)
				out = bt_run_share_ex_allocate(weight.new = weight1[i,], weight.prev = weight.last,
					weight.change.index = weight.change.index,
					price = prices[i,], share = info$share[i,], cash = info$cash[i],
					commission, lot.size, control = control$round_lot,
			...
			if(trade.invest.type == invest.type.def$update) {
				...
				out = bt_run_share_ex_invest(weight.new = weight1[i,], weight.prev = weight.last,
					weight.change.index = weight.change.index,
					price = prices[i,], share = info$share[i,], cash = info$cash[i], 
					cashflow = cashflow[i] + fee.rebate[i] + div[i],
					commission, lot.size, control = control$round_lot)
	...
	if( any(!equal_check(info$share[i,], sapply(1:n, function(a) sum(iif(holdings$n.trades[a] > 0, holdings$share[1:holdings$n.trades[a],a], 0))  )) )	)
	{
		cat('Wrong Holdings', info$share[i,][5], '\n')
		

	}		
				tax_update_holdings(tax.control, holdings, tax, wash.sale, 
...
# 
holdings_split = function(holdings, a, split) {
	n.trades = 1:holdings$n.trades[a]
...
bt_run_share_ex_n_days = function(index.today, index.hist, dates) {
...
tax_update_dividends = function(tax.control, dividend.control, holdings, tax, share, dividend, index, dates) 
{
	# NOTES:
	#
	...
	for(a in which(pos.cashflow & !tax.control$dividend$nonqualified)) {
		n.trades = 1:holdings$n.trades[a]							
		trade.days = bt_run_share_ex_n_days(index, holdings$date[n.trades,a], dates)
...
# record_wash_sale, called every time there is a loosing trade
record_wash_sale = function(a, n.trades, pnl, price, trigger, holdings, wash.sale, tax.control) {
...
check_wash_sale = function(a, dates, tax, tax.control, holdings, wash.sale) {
	# no losses recorded
	...
	n.trades = 1:wash.sale$n.trades[a]	
	trade.days = bt_run_share_ex_n_days(index , wash.sale$date[n.trades,a], dates)
...
tax_update_holdings = function(tax.control, holdings, tax, wash.sale, share0, share1, price, index, dates) 
{				
	n = len(price)
	...
	if( any(!equal_check(share0, sapply(1:n, function(a) sum(iif(holdings$n.trades[a] > 0, holdings$share[1:holdings$n.trades[a],a], 0))  )) )	)
	...
	for(a in (1:n)[share0 != share1]) {				
		...
		if( (share0[a] * share1[a]) <= 0) {
			...
			if(share0[a] != 0) {
				...
				record_wash_sale(a, n.trades, pnl, price, trigger, holdings, wash.sale, tax.control)
...
# wash sale - check	
check_wash_sale(a, dates, tax, tax.control, holdings, wash.sale)
...
check_wash_sale(a, dates, tax, tax.control, holdings, wash.sale)
			...
			if( abs(share1[a]) < abs(share0[a]) ) {
				...
				if( run.share[n1] == remove.share ) {
					...
					record_wash_sale(a, 1:n1, pnl, price, trigger, holdings, wash.sale, tax.control)
					n1 = n1 + 1	
				} else {
				# split trade
		...
		if( !equal_check(share1[a], sum(iif(holdings$n.trades[a] > 0, holdings$share[1:holdings$n.trades[a],a], 0))))
			cat('a', a, index, '\n')
	}
	
	if( any(!equal_check(share1, sapply(1:n, function(a) sum(iif(holdings$n.trades[a] > 0, holdings$share[1:holdings$n.trades[a],a], 0))  )) )	)
		cat('Mismatch holding shares', index, '\n')
...
equal_check = function(a,b,eps=1e-10) abs(a - b) < eps
		

###############################################################################
#' default_round_lot_control	
#' @export 
###############################################################################
default_round_lot_control = function() {
	list(
...
#' @export 
###############################################################################
bt_run_share_ex_invest = function
(
	weight.new,
	weight.prev,
	weight.change.index,
	...
	lot.size,
	silent=T,
	# control allocation if round_lot is enabled
	control = default_round_lot_control()
) {
...
# does not work for negative cashflows because we need cash to pay commissions
	n = len(share)
	out = bt_run_share_ex_allocate(weight.new = weight.new, weight.prev = rep(0, n),
...
# for cashflow(-) freeze weights that has abs(share.new) > abs(share) and repeat
	out = bt_run_share_ex_allocate(weight.new = weight.new, weight.prev = weight.prev,
		weight.change.index = weight.change.index,
...
#' @export 
###############################################################################
bt_run_share_ex_allocate = function
(
	weight.new,
	weight.prev,
	weight.change.index,
...
compute.commission = function(share.prev, share.new, price, commission) {	
	...
	share.prev = rep_row(share.prev, nrow(share.new))
	price = rep_row(price, nrow(share.new))
...
compute.cash = function(value, share, price, com) {
	if(is.null(dim(share)))
		...
		price = rep_row(price, nrow(share))
...
compute.weight.diff = function(target, share, cash) {
	if(is.null(dim(share)))
		...
		price = rep_row(price, nrow(share))
		target = rep_row(target, nrow(share))
		rowSums(abs(
...
allocate.lot = function(value, share, lot.size) {
	if( len(lot.size) == 0 || all(lot.size == 0) )
		return(rep_row(allocate(value, share), 3))
	
	new.total.weight = sum(abs(weight.new[weight.change.index]))
	if(new.total.weight == 0) {
		shares = rep_row(share, 2)
		...
		shares = rep_row(share, 3)
		shares[2, weight.change.index] = round_lot_basic(w, p, allocate.value, lot.size)
		shares[3, weight.change.index] = round_lot_basic_base(w, p, allocate.value, lot.size)
	}
	shares	
}
...
# Tests
bt_run_share_ex_allocate_test = function() {
	...
	a = bt_run_share_ex_allocate(weight.new,weight.prev,weight.change.index,price,share,cash,commission,lot.size,F)
	...
	weight.new = c(2,8) / 10
	weight.change.index = c(T, T)
	a = bt_run_share_ex_allocate(weight.new,weight.prev,weight.change.index,price,share,cash,commission,lot.size)
...
###############################################################################	
#' Round Lot
#' a helper function for round_lot_basic
#' @export 
###############################################################################
round_lot = function(weight, price, capital, lot.size) {
	...
	round_lot_basic(weight, price, capital, lot.size)
}

###############################################################################	
#' Round Lot Basic Base
#' round_lot to the nearest lot.size
#' @export 
###############################################################################
round_lot_basic_base = function(weight, price, capital, lot.size) {
...
###############################################################################	
#' Round Lot Basic
#' round_lot to the nearest lot.size and next try to reallocate remaining cash
#' @export 
###############################################################################
round_lot_basic = function(weight, price, capital, lot.size) {
...
round_lot_basic_test = function() {
	weight = c(1, 1, 1, 1) / 4
	price = c(1.345, 2.4, 3.5, 4.6)
	capital = 100
	lot.size = c(1, 1, 1, 1)
	
	w = round_lot_basic(weight, price, capital, lot.size)
	w
	sum(abs(w * price))
...
###############################################################################	
#' Test for bt_run_share_ex functionality
#' @export 
###############################################################################
bt_run_share_ex_test = function() {
	#*****************************************************************
	...
	data <- new.env()
	getSymbols_extra(tickers, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T, set.symbolnames=T)
	...
	models$test = bt_run_share(data, clean.signal=F, silent=F, commission=commission)
	
	data$weight[] = NA
	  #data$weight[1,] = 1
	  data$weight[period.ends,] = ntop(prices[period.ends,], n)
	models$test.ex = bt_run_share_ex(data, clean.signal=F, silent=F, commission=commission)

	data$weight[] = NA
	  #data$weight[1,] = 1
	  data$weight[period.ends,] = ntop(prices[period.ends,], n)
	models$test.ex.lot = bt_run_share_ex(data, clean.signal=F, silent=F, commission=commission, lot.size=50)
	
		
	data$weight[] = NA
	  #data$weight[1,] = 1
	  data$weight[period.ends,] = ntop(prices[period.ends,], n)
	models$test.ex.lot.turnover = bt_run_share_ex(data, clean.signal=F, silent=F, commission=commission, 
		lot.size=50, control = list(round_lot = list(select = 'minimum.turnover', diff.target = 5/100))
	)	
	...
	layout(1:3)
		plotbt_transition_map(models$test$weight, 'BuyHold')
		plotbt_transition_map(models$test.ex$weight, 'BuyHold.ex')
		plotbt_transition_map(models$test.ex.lot$weight, 'BuyHold.ex')
	...
	pdf(file = 'report1.pdf', width=8.5, height=11)
		...
		strategy_performance_snapshoot(models, data=data)
	...
	obj = portfolio_allocation_helper(data$prices, 
		...
		min.risk.fns = list(
			...
			RP=risk_parity_portfolio(function(ia) ia$risk),
			MV=min_var_portfolio,
			Sharpe.RP=risk_parity_portfolio(function(ia) ia$risk / ia$expected.return)
		)
	)
	...
	for(i in names(obj$weights)) {
		...
		models[[paste0(i)]] = bt_run_share(data, clean.signal=F, silent=T, commission=commission)
	
		data$weight[] = NA
		  data$weight[period.ends,] = obj$weights[[i]]
		models[[paste0(i,'.ex')]] = bt_run_share_ex(data, clean.signal=F, silent=T, commission=commission)
	
		data$weight[] = NA
		  data$weight[period.ends,] = obj$weights[[i]]
		models[[paste0(i,'.ex.lot')]] = bt_run_share_ex(data, clean.signal=F, silent=T, commission=commission, lot.size=50)
		
		data$weight[] = NA
		  data$weight[period.ends,] = obj$weights[[i]]
		models[[paste0(i,'.ex.lot.turnover')]] = bt_run_share_ex(data, clean.signal=F, silent=T, commission=commission, 
			lot.size=50, control = list(round_lot = list(select = 'minimum.turnover', diff.target = 5/100)))
	...
	#*****************************************************************
	# Example of using round_lot externally
	#******************************************************************
	weight = rep(1/n, n)
	...
	bt_run_share_ex_allocate(weight, weight, rep(T, n),
...
#' @export 
###############################################################################
bt_unadjusted_add_div_split = function(
	data.raw, 
	yahoo.round.up.nearest.cents=F,
	infer.div.split.from.adjusted=F # caution, please see example in test_implied_div_split
) {
...
##all(x == adjRatios(splits=merge(split, index(dividend)))[,1])
		...
		if(yahoo.round.up.nearest.cents) {
			map.col = unlist(find_names('Close,Open,High,Low,Adjusted', price, F))
...
#Implied Dividend: (Pt1+Dt1)/Pt0 = At1/At0 => Dt1 = Pt0 * At1/At0 - Pt1
#Implied Split: St1 * Pt1/Pt0 = At1/At0 => St1 = Pt1/Pt0 * At0/At1
test_implied_div_split = function(ticker) {
	#*****************************************************************
	# Load historical data
	#*****************************************************************
	load_packages('quantmod')
	...
	data <- new.env()
	getSymbols_extra(ticker, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T, set.symbolnames=T)
...
for(ticker in tickers) {
	...
	data <- new.env()
	getSymbols_extra(ticker, src = 'yahoo', from = '1970-01-01', env = data, auto.assign = T)
...
###############################################################################	
#' Test for bt_run_share.unadjusted functionality
#' @export 
###############################################################################
bt_run_share_unadjusted_test_data = function() {
	#*****************************************************************
	# Load historical data
	#*****************************************************************
	load_packages('quantmod')
	...
	# For each asset, append dividend and split columns
	#****************************************************************** 	
	bt_unadjusted_add_div_split(data.raw, yahoo.round.up.nearest.cents = T)
	...
	plota_matplot(cbind(cumprod(1 + adjusted),cumprod(1 + unadjusted)))
	...
	yhist = read_xts(hist_quotes_url('IBM', '1992-11-01', '1992-11-30', 'yahoo'))
	ghist = read_xts(hist_quotes_url('IBM', '1992-11-01', '1992-11-30', 'google'), format='%d-%b-%y')
	qhist = read_xts(hist_quotes_url('IBM', '1992-11-01', '1992-11-30', 'quotemedia'))	
...
IBM = read_xts(txt,sep=' ', format='%b-%d-%Y')
	IBM = IBM[,'Close']
...
# looks like dividends are split adjusted by yahoo	
	...
	plota_matplot(cbind(cumprod(1 + temp.adjusted),cumprod(1 + temp.unadjusted)))

}
	

###############################################################################	
#' Summarize bt_run_share_ex events
#' @export 
###############################################################################
bt_make_trade_event_summary_table = function(bt, to.text=F) {
...
#' @export 
###############################################################################
bt_make_cashflow_event_summary_table = function(bt, to.text=F) {
...
#' @export 
###############################################################################
bt_run_share_unadjusted_test = function() {
	#*****************************************************************
	# Load historical data
	#*****************************************************************
	load_packages('quantmod')
	...
	bt_prep(data.raw, align='remove.na', fill.gaps = T)
	...
	period.ends = date_ends(data.raw$prices,'months')
	...
	models$test.unadjusted = bt_run_share_ex(data.raw, clean.signal=F, silent=F, commission=commission,
	...
	#****************************************************************** 	
data.raw = data.raw1	
	bt_unadjusted_add_div_split(data.raw, yahoo.round.up.nearest.cents = T, infer.div.split.from.adjusted=T)
	...
	data.raw$weight[] = NA
		#data.raw$weight[1,] = 1
		data.raw$weight[period.ends,] = 1		
	models$test.unadjusted1 = bt_run_share_ex(data.raw, clean.signal=F, silent=F, commission=commission,
		lot.size=50, adjusted = F)
	...
	layout(1:2)
		plotbt_transition_map(models$test.ex$weight)
		plotbt_transition_map(models$test.unadjusted$weight)

		
	# create table
	mlast(bt_make_trade_event_summary_table(models$test.unadjusted), 20)
...
#****************************************************************** 	
	...
	#data.raw = data.raw1
	#bt_unadjusted_add_div_split(data.raw, infer.div.split.from.adjusted=T)
	bt_unadjusted_add_div_split(data.raw)
	...
	layout(1:2)
		plotbt_transition_map(models$test.unadjusted$weight)
		plotbt_transition_map(models$test.unadjusted1$weight)
...
###############################################################################	
#' Test for bt_run_share_ex cashflow functionality
#' @export 
###############################################################################
bt_run_share_ex_test_cashflow = function() {
	#*****************************************************************
	# Load historical data
	#*****************************************************************
	load_packages('quantmod')
	...
	data$weight[] = NA
	  data$weight[period.ends,] = ntop(prices[period.ends,], n)
	models$test.ex.lot.cashflow = bt_run_share_ex(data, clean.signal=F, silent=F, commission=commission, 
		lot.size=50,
		cashflow.control = list(
			monthly.income = list(
				cashflows = event_at(prices, 'quarter', 1000, offset=0),
				invest = 'cash',
				type = 'regular'
			)
		)
	...
	# info tables
	mlast(bt_make_trade_event_summary_table(models$test.ex.lot.cashflow), 20)
	
	mlast(bt_make_cashflow_event_summary_table(models$test.ex.lot.cashflow), 20)
...
# Tests
bt_run_share_ex_invest_test = function() {
	...
	a = bt_run_share_ex_invest(weight.new,weight.prev,weight.change.index,price,share,cash,cashflow,commission,lot.size,F)
...
###############################################################################	
#' Test for bt_run_share_ex tax functionality
#' @export 
###############################################################################
bt_run_share_ex_test_tax = function() {
	#*****************************************************************
	# Load historical data
	#*****************************************************************
	load_packages('quantmod')
...
#source('../bt.share.r')
	...
	data$weight[] = NA
	  data$weight[period.ends,] = weights
	models$test.ex.lot.tax = bt_run_share_ex(data, clean.signal=F, silent=F, commission=commission, 
		lot.size=50,
		control = list(round_lot = list(select = 'minimum.turnover', diff.target = 5/100)),
		...
		cashflow.control = list(
			taxes = list(
				cashflows = event_at(prices, 'year', offset=60),
				cashflow.fn = tax_cashflows,
				invest = 'cash',
				type = 'fee.rebate'
			)
		)
	...
	# info tables
	mlast(bt_make_trade_event_summary_table(models$test.ex.lot.tax), 20)
	
	mlast(bt_make_cashflow_event_summary_table(models$test.ex.lot.tax), 20)
	...
	data$weight[] = NA
	  data$weight[period.ends,] = weights
	models$test.unadjusted.tax = bt_run_share_ex(data.raw, clean.signal=F, silent=F, commission=commission, 
		lot.size=50,
		control = list(round_lot = list(select = 'minimum.turnover', diff.target = 5/100)),
	...
	# info tables
	mlast(bt_make_trade_event_summary_table(models$test.unadjusted.tax), 20)
	
	mlast(bt_make_cashflow_event_summary_table(models$test.unadjusted.tax), 20)
	...
	look_at_taxes(models$test1)['2007']
	look_at_taxes(models$test2)['2007']
		
	tax_summary(models$test1)
	tax_summary(models$test2)
	
	tax_summary(models$test1, function(x) as.numeric(format(x,'%Y%m')))[1:14,]	
	tax_summary(models$test2, function(x) as.numeric(format(x,'%Y%m')))[1:14,]	

	data$prices[period.ends,][1:14,1:3]
	data.raw$prices[period.ends,][1:14,1:3]
	
	bt_make_trade_event_summary_table(models$test1)[1:14,]
	bt_make_trade_event_summary_table(models$test2)[1:36,]
...
#' @export 
###############################################################################
event_at = function(x, period = 'month', amount = 1, period.ends = date_ends(x,period), offset = 1) {
	nperiods = nrow(x)
...
#' @export 	
look_at_taxes = function(m) {
	temp = data.frame(m[spl('long.term.cap,short.term.cap,qualified.div,non.qualified.div')])
	temp = make_xts(cbind(temp, total=rowSums(temp)), data$dates)
	temp[temp$total!=0]
}	

#' @export 	
tax_summary = function(m, by.fn=date_year) {
	temp = aggregate(
...
#' @export 
###############################################################################
tax_cashflows = function(info, index, last.index) {
...
#matrix(1,1,1)[1,20]
	...
	# index of last year
	ii = date_year(info$dates) == date_year(info$dates[index]) - 1
	...
	tax_cashflows.cap.helper = function(neg, pos) {
		if( -neg > pos) {
			neg = neg + pos
			pos = 0
		} else {
	...
	if(long.term.cap < 0 && short.term.cap > 0) {
		temp = tax_cashflows.cap.helper(long.term.cap, short.term.cap)
		long.term.cap = temp$neg
		short.term.cap = temp$pos
	} else if(long.term.cap > 0 && short.term.cap < 0) {
		temp = tax_cashflows.cap.helper(short.term.cap, long.term.cap)
...
#while un-adjusted prices back-test deposits dividends into_cash account that is allocated 
...
#
#bt_run_share vs bt_run_share_ex - different assumptions in calculations of shares
#=========================================
#the bt_run_share function scales number of shares to the original capital at each rebalance. 
#To glance at SIT back-test logic for bt_run_share, please have a look at
#  https://systematicinvestor.wordpress.com/2013/11/05/commissions/
#
#This problem with bt_run_share function is one of the reason i'm working on 
#bt_run_share_ex. bt_run_share_ex function properly tracks capital evolution 
#and is not causing artificial turnover.
#
#Following simple example that should clarify the logic in bt_run_share.  
#bt_run_share function computes number of shares at each rebalance using following formula, 
...
#above introduces artificial turnover that you see with bt_run_share function
#
...
#
#> mlast(bt_make_trade_event_summary_table(models$test.unadjusted.tax), 25)
#            Type  GOOGL     Cash   Com
...
if(F) {	
	...
	for(i in trade.index) {
		if(last.trade > 0) {
			...
			index = (last.trade + 1) : i
				n.index = len(index)
			share.wt[index,] = rep_row(share[last.trade,], n.index)
			cash.wt[index] = cash[last.trade]
			
			share[index,] = rep_row(share[last.trade,], n.index)
		...
		#share[i,] = round_lot_basic_base(w, p, value, lot.size)
		share[i,] = round_lot_basic(w, p, value, lot.size)
