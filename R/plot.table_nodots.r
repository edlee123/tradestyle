...
###############################################################################
# draw_cell - draw_cell at location (r,c)
###############################################################################
draw_cell <- function
(
...
###############################################################################
# plot_table_helper_auto_adjust_cex - determine how to auto-adjust text size
#  code is based on discussion at
#  http://www.mail-archive.com/r-help@r-project.org/msg04577.html			
###############################################################################
plot_table_helper_auto_adjust_cex <- function
(
...
###############################################################################
# plot_table_param - plot_table with user specified parameters
###############################################################################
plot_table_param <- function
(
...
{	
	...
	for(p in 1:(len(pages)-1)) {	
		...
		oldpar = make_table(nr, nc)

		text.cex = plot.matrix.cex[c(1, 1 + rindex), ]		
			text.cex = plot_table_helper_auto_adjust_cex(temp.table, keep.all.same.cex)
		bg.col = plot.matrix_bg.col[c(1, 1 + rindex), ]

		for(r in 1:nr) {
			for(c in 1:nc) {
				draw_cell( paste('', temp.table[r,c], '', sep=' '), r, c, 
...
###############################################################################
# plot_table_helper_color - default coloring scheme for highlight
###############################################################################
plot_table_helper_color <- function
(
	temp	# matrix to plot 
){
...
###############################################################################
# plot_table_helper_colorbar - plot colorbar
###############################################################################
plot_table_helper_colorbar <- function
(
	plot.matrix		# matrix to plot 
)
{
...
)
{
	...
	if(is.logical(highlight)) { 
		...
		if(highlight) highlight = plot_table_helper_color(plot.matrix)
	}
	...
	if(!is_highlight) {
		...
		plot_table_param( plot.matrix, smain, plot.matrix.cex, plot.matrix_bg.col, 
	...
	if(colorbar) plot_table_helper_colorbar(plot.matrix);
}
...
###############################################################################
# Examples of plot_table function
###############################################################################
plot_table_test <- function()
{
	# basic plot_table
		...
		plot_table(temp, format(as.Date(Sys.time()), '%d %b %Y'))
	
		dev.off()
	
	# plot_table with colorbar
		# generate 1,000 random numbers from Normal(0,1) distribution 
		data =  matrix(rnorm(1000), nc=10)
			colnames(data) = paste('data', 1:10, sep='')
...
###############################################################################
plot_periodic_table1 <- function(hist.returns)
{	
	n = ncol(hist.returns)
	
	# create temp matrix with data you want to plot
	temp = t(coredata(hist.returns))
		colnames(temp) = format(index_xts(hist.returns), '%Y')
		rownames(temp) = 1:n
	...
	plot_table(temp, highlight = highlight)			
	plota_legend(colnames(hist.returns), col)	# , cex=1.5
}

plot_periodic_table2 <- function(hist.returns)
{	
	# create temp matrix with data you want to plot
	temp = t(coredata(hist.returns))
		colnames(temp) = format(index_xts(hist.returns), '%Y')
	...
	plot_table(temp, highlight = highlight, colorbar = TRUE)	
}	
