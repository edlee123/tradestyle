...
#' @export 
###############################################################################
consecutive_changes <- function
(
	data, 		# data series
	positive=T	# count positive consecutive_changes
) 
{ 
	if(positive) dir = diff(data) > 0 else dir = diff(data) < 0
					
	temp = cumsum(iif(dir, 1, 0))
	temp - ifna_prev(iif(dir, NA, coredata(temp)))
}
...
#' @export 
factor_avgcor <- function(data, next.month.ret, name) { 
	load_packages('abind')
	# create matrix
	...
	# plot
	temp = t(compute_avgcor(temp, 'spearman')[,-1])
		temp[] = plota_format(100 * temp, 0, '', '%')
		plot_table(temp, smain=paste(name,'Correlation',sep=' \n '))
}
...
#' @export 
###############################################################################
compute_avgcor <- function
(
	data, 		# matrix with data: [rows,cols_factors]
...
{ 
	...
	for(i in names(data)) {
		...
		data[[i]] = (data[[i]] - cap_weighted_mean(data[[i]], MKVAL)) / 
...
#' @export 
###############################################################################
normal_transform <- function(data) 
{	
...
{ 
	# normalize (convert to z scores) cross sectionaly all factors
	for(i in names(data)) {
		data[[i]][] = t(apply(data[[i]], 1, normal_transform))
	}
	return(data)	
}
...
{ 
	...
	sapply(1:len(data), function(i)
		compute_quantiles(data[[i]], next.month.ret, paste(names(data)[i],smain))
	)	
}
...
{ 
	out = compute_quantiles(factors, next.month.ret, plot=F)	
...
{ 
	...
	plot(x, y, pch=20, xlab=name, ylab='Next Month Return')
		...
		plota_legend(paste('Pearson =',cor.p,',Spearman =', cor.s))
}		 		
